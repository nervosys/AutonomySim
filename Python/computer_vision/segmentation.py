...
AutonomySim.wait_key('Press any key to set all object IDs to 0')
found = client.simSetSegmentationObjectID("[\w]*", 0, True);
print("Done: %r" % (found))

#for block environment

AutonomySim.wait_key('Press any key to change one ground object ID')
found = client.simSetSegmentationObjectID("Ground", 20);
print("Done: %r" % (found))

#regex are case insensitive
AutonomySim.wait_key('Press any key to change all ground object ID')
...
AutonomySim.wait_key('Press any key to set Landscape object ID to 128')
found = client.simSetSegmentationObjectID("[\w]*", 128, True);
print("Done: %r" % (found))

#get segmentation image in various formats
responses = client.simGetImages([
    AutonomySim.ImageRequest("0", AutonomySim.ImageType.Segmentation, True), #depth in perspective projection
    AutonomySim.ImageRequest("0", AutonomySim.ImageType.Segmentation, False, False)])  #scene vision image in uncompressed RGBA array
