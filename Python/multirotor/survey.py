...
class SurveyNavigator:
    ...
    def start(self):
        ...
        while x < self.boxsize:
            ...
            path.append(AutonomySim.Vector3r(x, self.boxsize, z))
            x += self.stripewidth            
        ...
        try:
            result = self.client.moveOnPathAsync(path, self.velocity, trip_time, AutonomySim.DrivetrainType.ForwardOnly, 
                AutonomySim.YawMode(False,0), self.velocity + (self.velocity/2), 1).join()
        except:
