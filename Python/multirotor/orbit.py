...
class OrbitNavigator:
    def __init__(self, radius = 2, altitude = 10, speed = 2, iterations = 1, center = [1,0], snapshots = None):
        ...
        self.client = AutonomySim.MultirotorClient()
    ...
    def start(self):
        ...
        if not self.takeoff and landed == AutonomySim.LandedState.Landed: 
            self.takeoff = True
            print("taking off...")
        ...
        while count < self.iterations:
            ...
            self.client.moveByVelocityZAsync(vx, vy, z, 1, AutonomySim.DrivetrainType.MaxDegreeOfFreedom, AutonomySim.YawMode(False, camera_heading))
    ...
    def take_snapshot(self):
        ...
        pos = self.client.getMultirotorState().kinematics_estimated.position
        self.client.moveToPositionAsync(pos.x_val, pos.y_val, self.z, 0.5, 10, AutonomySim.DrivetrainType.MaxDegreeOfFreedom, 
            AutonomySim.YawMode(False, self.camera_heading)).join()
        responses = self.client.simGetImages([AutonomySim.ImageRequest(1, AutonomySim.ImageType.Scene)]) #scene vision image in png format
