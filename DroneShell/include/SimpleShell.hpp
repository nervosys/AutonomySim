...
template <class ExecContext> class SimpleShell {
  public:
    ...
    struct ShellCommandSwitch {
        ...
        float toFloat() {
            try {
                ...
                throw std::invalid_argument(Utils::stringf("expecting float value for switch '%s', but found '%s'",
        ...
        TTimeDelta toTimeDelta() {
            try {
                ...
                throw std::invalid_argument(Utils::stringf("expecting TTimeDelta value for switch '%s', but found '%s'",
                                                           name.c_str(), value.c_str()));
            }
        }
        int toInt() {
            try {
                ...
                throw std::invalid_argument(Utils::stringf("expecting integer value for switch '%s', but found '%s'",
                                                           name.c_str(), value.c_str()));
            }
        }
    };
    ...
    class ShellCommand {
      ...
      public:
        ShellCommand(const std::string &name, const std::string &help) : name_(name), help_(help) {}
        ...
        ShellCommandSwitch &getSwitch(const std::string &name) {
            ...
            throw std::invalid_argument(
                common_utils::Utils::stringf("switch %s is not defined on this command", name.c_str()));
        }
      ...
      private:
        ShellCommand() {}
        ShellCommand(ShellCommand &other) { unused(other); }
    };

    typedef std::function<std::string(const ShellCommandParameters &params, std::string script_filepath)>
        BeforeScriptStartCallback;
    BeforeScriptStartCallback _beforeScriptStartCallback;
    void beforeScriptStartCallback(BeforeScriptStartCallback value) { _beforeScriptStartCallback = value; }
    typedef std::function<bool(const ShellCommandParameters &params, std::string script_filepath)>
        AfterScriptEndCallback;
    AfterScriptEndCallback _afterScriptEndCallback;
    void afterScriptEndCallback(AfterScriptEndCallback value) { _afterScriptEndCallback = value; }
    typedef std::function<std::string(const ShellCommandParameters &params, std::string command_line)>
        BeforeScriptCommandStartCallback;
    ...
    typedef std::function<bool(const ShellCommandParameters &params, std::string command_line, bool commandReturnValue)>
        AfterScriptCommandEndCallback;
    AfterScriptCommandEndCallback _afterScriptCommandEndCallback;
    void afterScriptCommandEndCallback(AfterScriptCommandEndCallback value) { _afterScriptCommandEndCallback = value; }

    typedef std::function<void(const ShellCommandParameters &params, std::string command_line)>
        BeforeCommandStartCallback;
    BeforeCommandStartCallback _beforeCommandStartCallback;
    void beforeCommandStartCallback(BeforeCommandStartCallback value) { _beforeCommandStartCallback = value; }
    typedef std::function<bool(const ShellCommandParameters &params, std::string command_line, bool commandReturnValue)>
        AfterCommandEndCallback;
    AfterCommandEndCallback _afterCommandEndCallback;
    void afterCommandEndCallback(AfterCommandEndCallback value) { _afterCommandEndCallback = value; }
  ...
  public: // default commands
    ...
    class HelpCommand : public ShellCommand {
      ...
      public:
        ...
        void bind(SimpleShellCommand helpMethod) { helpMethod_ = helpMethod; }

        bool execute(const ShellCommandParameters &params) { return helpMethod_(params); }
    };
    ...
    bool helpMethod(const ShellCommandParameters &params) {
        ...
        std::sort(sorted.begin(), sorted.end(),
                  [&](const std::string &p1, const std::string &p2) { return p1.compare(p2) < 0; });
    ...
    class RunCommand : public ShellCommand {
      ...
      public:
        ...
        void bind(SimpleShellCommand runMethod) { runMethod_ = runMethod; }

        bool execute(const ShellCommandParameters &params) { return runMethod_(params); }
    };
  ...
  public:
    SimpleShell(std::string prompt = "> ") {
        ...
        linenoise::SetCompletionCallback(
            std::bind(&SimpleShell::commandCompletitionCallBack, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    void showMessage(const std::string &message) { std::cout << prompt_ << message << std::endl; }
    ...
    virtual bool execute(const ShellCommandParameters &params) {
        ...
        if (command_infos_.find(command) != command_infos_.end()) {
            ...
            if (command_infos_.find(command) != command_infos_.end()) {
                ...
                std::cout << "Alias '" << params.args[0] << "' referred to unknown command '" << command << "'"
                          << std::endl;
                return false;
            }
        } else {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (!arg.empty() && (arg.at(0) == '-' || arg.at(0) == '/')) {
                    ...
                    if (option == "-?" || option == "-help" || option == "-h") {
                        ...
                        throw std::invalid_argument(
                            common_utils::Utils::stringf("switch %s is missing a value", arg.c_str()));
                    }
                } else {
