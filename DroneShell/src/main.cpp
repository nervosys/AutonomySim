...
#include "common/utils/AsyncTasker.hpp"
...
struct CommandContext {
  public:
    CommandContext(const std::string &server_address = "localhost") : client(server_address) {}

    MultirotorRpcLibClient client;
    AsyncTasker tasker;

    void sleep_for(TTimeDelta wall_clock_dt) { clock_->sleep_for(wall_clock_dt); }
...
class DroneCommand : public SimpleShell<CommandContext>::ShellCommand {
  public:
    DroneCommand(std::string name, std::string help) : ShellCommand(name, help) {}
    ...
    void addRelativeSwitch() {
        this->addSwitch({"-r", "0",
                         "specify x,y,z is relative to current position rather than home position, 0 means absolute, 1 "
                         "means relative"});
    }
    ...
    void addDriveTrainSwitch() {
        this->addSwitch(
            {"-drivetrain", "0", "type of drive mode (1=forward only, 0= max degree of freedom) (default is 1)"});
    }
};
...
class GoHomeCommand : public DroneCommand {
  public:
    GoHomeCommand() : DroneCommand("GoHome", "Go back to takeoff point and land") {
        this->addSwitch(
            {"-timeout_sec", "30000", "specify time to wait after issuing command (default 30000 seconds)"});
    }
...
class RotateByYawRateCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() { context->client.rotateByYawRateAsync(yaw_rate, duration); });
...
class RotateToYawCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() { context->client.rotateToYawAsync(yaw, duration, margin); });
...
class HoverCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        CommandContext *context = params.context;
        context->tasker.execute([=]() { context->client.hoverAsync(); });
  ...
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() {
            ...
            std::cout << "Global position: lat=" << gps.latitude << ", lon=" << gps.longitude
                      << ", alt=" << gps.altitude << std::endl;
        });
...
class MoveOnPathCommand : public DroneCommand {
  public:
    MoveOnPathCommand()
        ...
        this->addSwitch({"-path", "",
                         "a series of x,y,z coordinates separated by commas, e.g. "
                         "0,0,-10,100,0,-10,100,100,-10,0,100,-10,0,0,-10 will fly a square box pattern"});
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() {
            context->client.moveOnPathAsync(path, velocity, duration, drivetrain, yawMode, lookahead,
                                            adaptive_lookahead);
        });
...
class MoveToPositionCommand : public DroneCommand {
  public:
    MoveToPositionCommand() : DroneCommand("MoveToPosition", "Move to x,y,z with specified velocity") {
        ...
        this->addSwitch(
            {"-velocity", "0.5", "the velocity to approach the position in meters per second (default 0.5)"});
    ...
    void MoveToPosition(CommandContext *context, float x, float y, float z, float velocity, float duration,
                        DrivetrainType drivetrain, YawMode yawMode, float lookahead, float adaptive_lookahead) {
        context->client.moveToPositionAsync(x, y, z, velocity, duration, drivetrain, yawMode, lookahead,
                                            adaptive_lookahead);
    }
...
class MoveByManualCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() { context->client.moveByManualAsync(vx, vy, z, duration, drivetrain, yawMode); });
...
class MoveByAngleZCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() { context->client.moveByRollPitchYawZAsync(roll, pitch, yaw, z, duration); });
        ...
        if (!s_origin.empty()) {
            ...
            if (parts.size() == 3) {
                ...
                throw std::invalid_argument(
                    "-origin argument is expecting 'x,y,z' (separated by commas and no spaces in between)");
            }
        }

        context->tasker.execute([=]() {
            context->client.setSafety(SafetyEval::SafetyViolationType(safety_flags), obs_clearance,
                                      SafetyEval::ObsAvoidanceStrategy(obs_strategy), obs_avoidance_vel, origin,
                                      xy_length, min_z, max_z);
        });
...
class BackForthByPositionCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute(
            [=]() {
                context->client.moveToPositionAsync(length, 0, z, velocity, 0, drivetrain, yawMode, lookahead,
                                                    adaptive_lookahead);
                if (!context->client.waitOnLastTask()) {
...
class SquareByPositionCommand : public DroneCommand {
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute(
            [=]() {
                context->client.moveToPositionAsync(x + length, y - length, z, velocity, 0, drivetrain, yawMode,
...
class CircleByPositionCommand : public DroneCommand {
  public:
    CircleByPositionCommand() : DroneCommand("CircleByPosition", "Make drone go in square using position commands") {
        ...
        this->addSwitch({"-z", "-2.5", "z position in meters (default -2.5)"});
        this->addSwitch(
            {"-seg_length", "0.2", "circle is approximated using strait segments (default length .2 meters)"});
        this->addSwitch({"-duration", "0", "the duration of this command in seconds (default no wait)"});
        this->addSwitch({"-pause_time", "0", "pause time between each run back and forth in seconds (default 0)"});
        this->addSwitch({"-iterations", "10000", "number of times to repeat the task (default 10000)"});
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        if (std::isnan(angle))
            throw std::invalid_argument(common_utils::Utils::stringf(
                "radius=%f and seg_length=%f doesn't form valid circle", radius, seg_length));
  ...
  public:
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        const Vector3r origin =
            plane != "xy" && plane != "yx" ? Vector3r(cx, cy, z_path + radius) : Vector3r(cx, cy, z_path);
...
class RecordPoseCommand : public DroneCommand {
  public:
    RecordPoseCommand()
        : DroneCommand("RecordPose",
                       "Append a single pose snapshot to a log file named 'rec_pos.log' in your $HOME folder\n\
...
// shell.addCommand("PlayPose", &playPoseCommand, "Play position, quaternion and GPS coordinates of drone from log
// file");
...
class PlayPoseCommand : public DroneCommand {
  public:
    PlayPoseCommand()
        : DroneCommand(
              "PlayPose",
              "Fly the drone through each recorded pose found in log file named 'rec_pos.log' in your $HOME folder\n\
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        context->tasker.execute([=]() {
            ...
            std::string file_path_name =
                FileSystem::getLogFileNamePath(FileSystem::getAppDataFolder(), "rec_pos", "", ".log", false);
            ...
            while (!file.eof()) {
                ...
                if (line.length() > 0) {
                    ...
                    line_ss >> gps_point.latitude >> gps_point.longitude >>
                        gps_point.altitude; // >> gps_point.height >> gps_point.health;
                    ...
                    context->client.moveToPositionAsync(local_point.x(), local_point.y(), local_point.z(), velocity, 0,
                                                        DrivetrainType::MaxDegreeOfFreedom, YawMode(false, yaw),
                                                        lookahead, adaptive_lookahead);
...
class GetImageCommand : public DroneCommand {
  ...
  public:
    ...
    void getImages(CommandContext *context, ImageCaptureBase::ImageType imageType, std::string baseName, int iterations,
                   TTimeDelta pause_time) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            const char *typeName = "";
            switch (imageType) {
            case nervosys::autonomylib::ImageCaptureBase::ImageType::Scene:
                typeName = "scene";
                break;
            case nervosys::autonomylib::ImageCaptureBase::ImageType::DepthVis:
                typeName = "depth";
                break;
            case nervosys::autonomylib::ImageCaptureBase::ImageType::Segmentation:
                typeName = "seg";
                break;
            case nervosys::autonomylib::ImageCaptureBase::ImageType::SurfaceNormals:
                typeName = "normals";
                break;
            case nervosys::autonomylib::ImageCaptureBase::ImageType::DisparityNormalized:
                typeName = "disparity";
                break;
            default:
                break;
    ...
    bool execute(const DroneCommandParameters &params) {
        ...
        if (type == "depth") {
            ...
            std::cout << "Error: Invalid image type '" << type
                      << "', expecting either 'depth', 'scene' or 'segmentation'" << std::endl;
            return true;
        }

        context->tasker.execute([=]() { getImages(context, imageType, name, iterations, pause_time); });
...
void printUsage() {
    ...
    std::cout << "The default server address is 127.0.0.1, but use the -server option to specify a different address "
                 "for the server"
              << std::endl;
}
...
int main(int argc, const char *argv[]) {
    ...
    // Shell callbacks
    //  shell.beforeScriptStartCallback(std::bind(&beforeScriptStartCallback, std::placeholders::_1,
    //  std::placeholders::_2)); shell.afterScriptEndCallback(std::bind(&afterScriptEndCallback, std::placeholders::_1,
    //  std::placeholders::_2)); shell.afterScriptCommandEndCallback(std::bind(&afterScriptCommandEndCallback,
    //  std::placeholders::_1, std::placeholders::_2));
    // shell.beforeCommandStartCallback(std::bind(&beforeCommandStartCallback, std::placeholders::_1,
    // std::placeholders::_2));
