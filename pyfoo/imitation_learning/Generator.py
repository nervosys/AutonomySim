...
class DriveIterator(image.Iterator):
    ...
    def __get_indexes(self, index_array):
        ...
        if self.x_prev_states is not None:
            batch_x_images = np.zeros(tuple([self.cmdch_size]+ list(self.x_images.shape)[1:]),
                                      dtype=K.floatx())
            batch_x_prev_states = np.zeros(tuple([self.cmdch_size]+list(self.x_prev_states.shape)[1:]), dtype=K.floatx())
        else:
            batch_x_images = np.zeros(tuple([self.cmdch_size] + list(self.x_images.shape)[1:]), dtype=K.floatx())
        ...
        if self.save_to_dir:
            for i in range(0, self.cmdch_size, 1):
