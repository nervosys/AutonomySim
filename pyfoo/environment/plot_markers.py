import setup_path
import autonomysim
from autonomysim import Vector3r, Quaternionr, Pose
from autonomysim.utils import to_quaternion
...
client.simPlotArrows(
    points_start=[
        Vector3r(x, y, 0) for x, y in zip(np.linspace(0, 10, 20), np.linspace(0, 0, 20))
    ],
    points_end=[
        Vector3r(x, y, 0)
        for x, y in zip(np.linspace(0, 10, 20), np.linspace(10, 10, 20))
    ],
...
client.simPlotArrows(
    points_start=[
        Vector3r(x, y, -3)
        for x, y in zip(np.linspace(0, 10, 20), np.linspace(0, 0, 20))
    ],
    points_end=[
        Vector3r(x, y, -5)
        for x, y in zip(np.linspace(0, 10, 20), np.linspace(10, 20, 20))
    ],
...
client.simPlotArrows(
    points_start=[
        Vector3r(x, y, -2)
        for x, y in zip(np.linspace(0, 10, 20), np.linspace(0, 20, 20))
    ],
    points_end=[
        Vector3r(x, y, -5)
        for x, y in zip(np.linspace(3, 17, 20), np.linspace(5, 28, 20))
    ],
...
client.simPlotPoints(
    points=[
        Vector3r(x, y, z)
        for x, y, z in zip(
            np.linspace(0, -10, 20), np.linspace(0, -20, 20), np.linspace(0, -5, 20)
        )
    ],
...
client.simPlotPoints(
    points=[
        Vector3r(x, y, z)
        for x, y, z in zip(
            np.linspace(0, 10, 20), np.linspace(0, -20, 20), np.linspace(0, -7, 20)
        )
    ],
    color_rgba=[1.0, 0.0, 1.0, 1.0],
    size=15,
...
client.simPlotLineStrip(
    points=[
        Vector3r(x, y, -5)
        for x, y in zip(np.linspace(0, -10, 10), np.linspace(0, -20, 10))
    ],
    color_rgba=[1.0, 0.0, 0.0, 1.0],
    thickness=5,
    duration=30.0,
...
client.simPlotStrings(
    strings=["Nervosys AutonomySim" for i in range(5)],
    positions=[
        Vector3r(x, y, -1) for x, y in zip(np.linspace(0, 5, 5), np.linspace(0, 0, 5))
    ],
    scale=1,
    color_rgba=[1.0, 1.0, 1.0, 1.0],
    duration=1200.0,
