...
cameraTypeMap = {
    "depth": autonomysim.ImageType.DepthVis,
    "segmentation": autonomysim.ImageType.Segmentation,
    "seg": autonomysim.ImageType.Segmentation,
    "scene": autonomysim.ImageType.Scene,
    "disparity": autonomysim.ImageType.DisparityNormalized,
    "normals": autonomysim.ImageType.SurfaceNormals,
}
...
class ImageBenchmarker:
    def __init__(
        ...
        self.autonomysim_client = autonomysim.VehicleClient()
        self.autonomysim_client.confirmConnection()
    ...
    def update_benchmark_results(self):
        ...
        if self.image_benchmark_num_images % 10 == 0:
            print(
                f"Result: {self.avg_fps} avg_fps for {self.image_benchmark_num_images} images"
            )

    def image_callback_benchmark_simGetImage(self):
        self.image_benchmark_num_images += 1
        image = self.autonomysim_client.simGetImage(CAM_NAME, self.img_type)
    ...
    def image_callback_benchmark_simGetImages(self):
        self.image_benchmark_num_images += 1
        request = [autonomysim.ImageRequest(CAM_NAME, self.img_type, False, False)]
        responses = self.autonomysim_client.simGetImages(request)
