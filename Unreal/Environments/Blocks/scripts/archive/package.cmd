...
if "%OutPath%"=="" set "OutPath=D:\AutonomySimBuilds"
IF NOT EXIST "%ToolPath%" (
...
for %%f in (*.uproject) do (
		...
		@echo off
		echo start %%~nf -windowed> "%OutPath%\%%~nf\run.cmd"
		if ERRORLEVEL 1 goto :failed
)
...
echo "Usage: package.cmd <path\to\output> <path to Engine\Build\BatchFiles> <UE Version>"
exit /b 1
