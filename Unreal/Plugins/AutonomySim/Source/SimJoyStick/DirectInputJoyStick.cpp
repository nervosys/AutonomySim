  ...
  public:
    ...
    void setAutoCenterStrength(double magnitude) {
        ...
        if (g_pAutoCenterHandle) {
            g_pAutoCenterHandle->SetParameters(&g_sAutoCenterConfig,
...
#define FFWRMAX 0.08
    ...
    void setWheelRumbleStrength(double strength) {
        ...
        if (g_pWheelRumbleHandle) {
            g_pWheelRumbleHandle->SetParameters(&g_sWheelRumbleConfig,
                                                DIEP_DIRECTION | DIEP_TYPESPECIFICPARAMS | DIEP_START);
        }
    }
    ...
    const Capabilities &getCapabilities() { return caps; }

    const JoystickInfo &getJoystickInfo() { return joystick_info; }

    ~impl() { FreeDirectInput(); }

  private:
    std::string toString(const std::wstring &wstr) { return std::string(wstr.begin(), wstr.end()); }
    ...
    HRESULT InitForceFeedback() {
        ...
        if (FAILED(hr = g_pJoystick->CreateEffect(GUID_ConstantForce, &g_sAutoCenterConfig, &g_pAutoCenterHandle,
                                                  nullptr)))
            return hr;
    ...
    HRESULT InitDirectInput(unsigned int joystick_index) {
        ...
        if (FAILED(hr = DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION, IID_IDirectInput8,
                                           (VOID **)&g_pDI, nullptr)))
            return hr;
        ...
        if (SUCCEEDED(pJoyConfig->GetConfig(
                joystick_index, &PreferredJoyCfg,
                DIJC_GUIDINSTANCE))) { // This function is expected to fail if no joystick is attached
        ...
        if (FAILED(hr = g_pDI->EnumDevices(DI8DEVCLASS_GAMECTRL, DirectInputJoyStick::impl::EnumJoysticksCallback, this,
                                           DIEDFL_ATTACHEDONLY))) {
        ...
        if (FAILED(hr = g_pJoystick->EnumObjects(DirectInputJoyStick::impl::EnumObjectsCallback, (VOID *)this,
                                                 DIDFT_ALL))) {
    ...
    HRESULT SetupForIsXInputDevice() {
        ...
        // Create WMI
        hr = CoCreateInstance(__uuidof(WbemLocator), nullptr, CLSCTX_INPROC_SERVER, __uuidof(IWbemLocator),
        ...
        (void)CoSetProxyBlanket(pIWbemServices, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL,
                                RPC_C_IMP_LEVEL_IMPERSONATE, nullptr, 0);
...
DirectInputJoyStick::DirectInputJoyStick() { pimpl_.reset(new impl()); }

DirectInputJoyStick::~DirectInputJoyStick() {
    // nop
}

bool DirectInputJoyStick::initialize(int joystick_index) { return pimpl_->initialize(joystick_index); }
...
const DirectInputJoyStick::Capabilities &DirectInputJoyStick::getCapabilities() { return pimpl_->getCapabilities(); }

const DirectInputJoyStick::JoystickInfo &DirectInputJoyStick::getJoystickInfo() { return pimpl_->getJoystickInfo(); }
