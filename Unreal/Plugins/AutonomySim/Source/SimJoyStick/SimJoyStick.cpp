...
struct SimJoyStick::impl {
  ...
  private:
    float getMappedValue(AxisMap::AxisType axis_type, const AxisMap &map,
                         const DirectInputJoyStick::JoystickState &di_state, const std::string &device_pid_vid) {
    ...
    float getAxisValue(AxisMap::AxisType axis_type, const AxisMap &map,
                       const DirectInputJoyStick::JoystickState &di_state, const std::string &device_pid_vid) {
...
SimJoyStick::SimJoyStick() { pimpl_.reset(new impl()); }
...
void SimJoyStick::setAutoCenter(int index, double strength) { pimpl_->setAutoCenter(index, strength); }

void SimJoyStick::setWheelRumble(int index, double strength) { pimpl_->setWheelRumble(index, strength); }