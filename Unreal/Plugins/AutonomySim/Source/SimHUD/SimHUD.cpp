...
ASimHUD::ASimHUD() {
    static ConstructorHelpers::FClassFinder<UUserWidget> hud_widget_class(
        TEXT("WidgetBlueprint'/AutonomySim/Blueprints/BP_SimHUDWidget'"));
...
void ASimHUD::BeginPlay() {
    ...
    try {
        UAutonomyBlueprintLib::OnBeginPlay();
        initializeSettings();
        loadLevel();
        ...
        UAutonomyBlueprintLib::LogMessageString("Error at startup: ", ex.what(), LogDebugLevel::Failure);
        ...
        UAutonomyBlueprintLib::ShowMessage(EAppMsgType::Ok, std::string("Error at startup: ") + ex.what(), "Error");
    }
}
...
void ASimHUD::EndPlay(const EEndPlayReason::Type EndPlayReason) {
    ...
    UAutonomyBlueprintLib::OnEndPlay();

    Super::EndPlay(EndPlayReason);
}

void ASimHUD::toggleRecordHandler() { simmode_->toggleRecording(); }

void ASimHUD::inputEventToggleRecording() { toggleRecordHandler(); }
...
void ASimHUD::inputEventToggleHelp() { widget_->toggleHelpVisibility(); }

void ASimHUD::inputEventToggleTrace() { simmode_->toggleTraceAll(); }

void ASimHUD::updateWidgetSubwindowVisibility() {
    for (int window_index = 0; window_index < AutonomySimSettings::kSubwindowCount; ++window_index) {
        ...
        if (camera != nullptr) {
            ...
            // sub-window captures don't count as a request, set bCaptureEveryFrame and bCaptureOnMovement to display so
            // we can show correctly the subwindow
            camera->setCameraTypeUpdate(camera_type, false);
        }

        widget_->setSubwindowVisibility(window_index, is_visible,
                                        is_visible ? camera->getRenderTarget(camera_type, false) : nullptr);
    }
}

bool ASimHUD::isWidgetSubwindowVisible(int window_index) { return widget_->getSubwindowVisibility(window_index) != 0; }
...
void ASimHUD::inputEventToggleSubwindow0() { toggleSubwindowVisibility(0); }

void ASimHUD::inputEventToggleSubwindow1() { toggleSubwindowVisibility(1); }

void ASimHUD::inputEventToggleSubwindow2() { toggleSubwindowVisibility(2); }
...
void ASimHUD::createMainWidget() {
    ...
    widget_->setRecordButtonVisibility(AutonomySimSettings::singleton().is_record_ui_visible);
    updateWidgetSubwindowVisibility();
}
...
void ASimHUD::initializeSettings() {
    ...
    if (getSettingsText(settingsText))
        AutonomySimSettings::initializeSettings(settingsText);
    else
        AutonomySimSettings::createDefaultSettingsFile();

    AutonomySimSettings::singleton().load(std::bind(&ASimHUD::getSimModeFromUser, this));
    for (const auto &warning : AutonomySimSettings::singleton().warning_messages) {
        UAutonomyBlueprintLib::LogMessageString(warning, "", LogDebugLevel::Failure);
    }
    for (const auto &error : AutonomySimSettings::singleton().error_messages) {
        UAutonomyBlueprintLib::ShowMessage(EAppMsgType::Ok, error, "settings.json");
    }
}

const std::vector<ASimHUD::AutonomySimSettings::SubwindowSetting> &ASimHUD::getSubWindowSettings() const {
    return AutonomySimSettings::singleton().subwindow_settings;
}

std::vector<ASimHUD::AutonomySimSettings::SubwindowSetting> &ASimHUD::getSubWindowSettings() {
    return AutonomySimSettings::singleton().subwindow_settings;
}
...
void ASimHUD::loadLevel() {
    UAutonomyBlueprintLib::RunCommandOnGameThread(
        [&]() {
            this->map_changed_ = UAutonomyBlueprintLib::loadLevel(
                this->GetWorld(), FString(AutonomySimSettings::singleton().level_name.c_str()));
        },
        true);
}
...
void ASimHUD::createSimMode() {
    ...
    if (simmode_name == AutonomySimSettings::kSimModeTypeMultirotor)
        simmode_ = this->GetWorld()->SpawnActor<ASimModeWorldMultirotor>(FVector::ZeroVector, FRotator::ZeroRotator,
                                                                         simmode_spawn_params);
    else if (simmode_name == AutonomySimSettings::kSimModeTypeCar)
        simmode_ =
            this->GetWorld()->SpawnActor<ASimModeCar>(FVector::ZeroVector, FRotator::ZeroRotator, simmode_spawn_params);
    else if (simmode_name == AutonomySimSettings::kSimModeTypeComputerVision)
...
void ASimHUD::initializeSubWindows() {
    ...
    for (const auto &setting : getSubWindowSettings()) {
        APIPCamera *camera = simmode_->getCamera(
            nervosys::autonomylib::CameraDetails(setting.camera_name, setting.vehicle_name, setting.external));
        if (camera)
...
// Example (Path): AutonomySim.exe -settings="C:\path\to\settings.json"
// Example (Text): AutonomySim.exe -settings={"foo":"bar"} -> settingsText will be set to {"foo":"bar"}
