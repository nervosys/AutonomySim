...
void AComputerVisionPawn::NotifyHit(class UPrimitiveComponent *MyComp, class AActor *Other,
...
void AComputerVisionPawn::initializeForBeginPlay() {
    ...
    camera_bottom_center_ = this->GetWorld()->SpawnActor<APIPCamera>(
        pip_camera_class_, FTransform(FRotator(-90, 0, 0), FVector::ZeroVector), camera_spawn_params);
    camera_bottom_center_->AttachToComponent(camera_bottom_center_base_,
                                             FAttachmentTransformRules::KeepRelativeTransform);
...
void AComputerVisionPawn::BeginPlay() { Super::BeginPlay(); }
