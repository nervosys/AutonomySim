...
ACarPawn::ACarPawn() {
    static ConstructorHelpers::FClassFinder<APIPCamera> pip_camera_class(
    ...
    const auto &car_mesh_paths = AutonomySimSettings::singleton().pawn_paths["DefaultCar"];
    auto slippery_mat = Cast<UPhysicalMaterial>(UAutonomyBlueprintLib::LoadObject(car_mesh_paths.slippery_mat));
    auto non_slippery_mat = Cast<UPhysicalMaterial>(UAutonomyBlueprintLib::LoadObject(car_mesh_paths.non_slippery_mat));
    if (slippery_mat)
        slippery_mat_ = slippery_mat;
    else
        UAutonomyBlueprintLib::LogMessageString("Failed to load Slippery physics material", "", LogDebugLevel::Failure);
    if (non_slippery_mat)
    ...
    ConstructorHelpers::FObjectFinder<USoundCue> SoundCue(
        TEXT("/AutonomySim/VehicleAdv/Sound/Engine_Loop_Cue.Engine_Loop_Cue"));
...
void ACarPawn::NotifyHit(class UPrimitiveComponent *MyComp, class AActor *Other, class UPrimitiveComponent *OtherComp,
                         bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse,
                         const FHitResult &Hit) {
    pawn_events_.getCollisionSignal().emit(MyComp, Other, OtherComp, bSelfMoved, HitLocation, HitNormal, NormalImpulse,
                                           Hit);
}

UChaosVehicleMovementComponent *ACarPawn::getVehicleMovementComponent() const { return GetVehicleMovement(); }
...
void ACarPawn::initializeForBeginPlay(bool engine_sound) {
    ...
    camera_front_center_ =
        this->GetWorld()->SpawnActor<APIPCamera>(pip_camera_class_, camera_transform, camera_spawn_params);
    camera_front_center_->AttachToComponent(camera_front_center_base_,
    ...
    camera_back_center_ = this->GetWorld()->SpawnActor<APIPCamera>(
        pip_camera_class_, FTransform(FRotator(0, -180, 0), FVector::ZeroVector), camera_spawn_params);
...
void ACarPawn::updateHUDStrings() {
    ...
    float speed_unit_factor = AutonomySimSettings::singleton().speed_unit_factor;
    FText speed_unit_label = FText::FromString(FString(AutonomySimSettings::singleton().speed_unit_label.c_str()));
...
void ACarPawn::setupInputBindings() {
    ...
    // UAutonomyBlueprintLib::BindActionToKey("Reverse", EKeys::Down, this, &ACarPawn::onReverseReleased, false);
}
...
void ACarPawn::onMoveRight(float Val) { keyboard_controls_.steering = Val; }

void ACarPawn::onHandbrakePressed() { keyboard_controls_.handbrake = true; }

void ACarPawn::onHandbrakeReleased() { keyboard_controls_.handbrake = false; }

void ACarPawn::onFootBrake(float Val) { keyboard_controls_.brake = Val; }
