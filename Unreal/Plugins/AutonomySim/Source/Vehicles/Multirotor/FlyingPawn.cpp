...
void AFlyingPawn::BeginPlay() { Super::BeginPlay(); }
...
void AFlyingPawn::NotifyHit(class UPrimitiveComponent *MyComp, class AActor *Other,
...
void AFlyingPawn::setRotorSpeed(const std::vector<MultirotorPawnEvents::RotorActuatorInfo> &rotor_infos) {
    for (auto rotor_index = 0; rotor_index < rotor_infos.size(); ++rotor_index) {
        ...
        if (comp != nullptr) {
            comp->RotationRate.Yaw = rotor_infos.at(rotor_index).rotor_speed *
                                     rotor_infos.at(rotor_index).rotor_direction * 180.0f / M_PIf * RotatorFactor;
        }
    }
}
...
void AFlyingPawn::initializeRotors(const std::vector<MultirotorPawnEvents::RotorActuatorInfo> &rotor_infos) {
    for (auto i = 0; i < rotor_infos.size(); ++i) {
        rotating_movements_.Add(UAutonomyBlueprintLib::GetActorComponent<URotatingMovementComponent>(
