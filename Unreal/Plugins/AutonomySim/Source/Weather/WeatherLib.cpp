        ...
        if (WeatherParameterCollection) {
            ...
            if (Instance) {
                ...
                UE_LOG(LogTemp, Warning,
                       TEXT("Warning, WeatherAPI could NOT get WeatherParameterCollectionInstance1!"));
            }
        } else {
    ...
    if (World) {
        ...
        if (WeatherActorClass) {
            for (int32 i = 0; i < ActorsToAttachTo.Num(); i++) {
                ...
                AActor *SpawnedWeatherActor =
                    World->SpawnActor(WeatherActorClass, &Location, &Rotation, WeatherActorSpawnInfo);

                SpawnedWeatherActor->AttachToActor(ActorsToAttachTo[i],
    ...
    if (World && MenuWidgetClass) {
        ...
        if (FoundWidgets.Num() > 0) {
            for (int32 i = 0; i < FoundWidgets.Num(); i++) {
                // hacky test to make sure we are getting the right class. for some reason cast above doesn't work, so
                // we use this instead to test for class
                if (FoundWidgets[i] &&
                    FoundWidgets[i]->GetClass()->GetFName().ToString() == getWeatherMenuClassName()) {
...
void UWeatherLib::setWeatherFog(AExponentialHeightFog *fog) { weather_fog_ = fog; }