...
APIPCamera::APIPCamera(const FObjectInitializer &ObjectInitializer)
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> mat_finder(
        TEXT("Material'/AutonomySim/HUDAssets/CameraSensorNoise.CameraSensorNoise'"));
    ...
    if (dist_mat_finder.Succeeded()) {
        ...
        distortion_param_collection_ = Cast<UMaterialParameterCollection>(
    ...
    image_type_to_pixel_format_map_.Add(
        Utils::toNumeric(ImageType::DepthPlanar),
        EPixelFormat::PF_DepthStencil); // not used. init_auto_format is called in setupCameraFromSettings()
    image_type_to_pixel_format_map_.Add(Utils::toNumeric(ImageType::DepthPerspective),
                                        EPixelFormat::PF_DepthStencil); // not used for same reason as above
    image_type_to_pixel_format_map_.Add(Utils::toNumeric(ImageType::DepthVis),
                                        EPixelFormat::PF_DepthStencil); // not used for same reason as above
    image_type_to_pixel_format_map_.Add(Utils::toNumeric(ImageType::DisparityNormalized),
                                        EPixelFormat::PF_DepthStencil); // not used for same reason as above
...
void APIPCamera::PostInitializeComponents() {
    ...
    // CinemAutonomySim
    camera_ = UAutonomyBlueprintLib::GetActorComponent<UCineCameraComponent>(this, TEXT("CameraComponent"));
    ...
    captures_[Utils::toNumeric(ImageType::Scene)] =
        UAutonomyBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("SceneCaptureComponent"));
    ...
    captures_[Utils::toNumeric(ImageType::Infrared)] =
        UAutonomyBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("InfraredCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::SurfaceNormals)] =
        UAutonomyBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(this, TEXT("NormalsCaptureComponent"));
...
void APIPCamera::BeginPlay() {
    ...
    // This improves performance because the capture components are no longer updating every frame and only update while
    // requesting an image
...
nervosys::autonomylib::ProjectionMatrix APIPCamera::getProjectionMatrix(const APIPCamera::ImageType image_type) const {
    nervosys::autonomylib::ProjectionMatrix mat;
    ...
    if (capture) {
        ...
        if (capture->ProjectionType == ECameraProjectionMode::Orthographic) {
            ...
            proj_mat_transpose = FReversedZOrthoMatrix(OrthoWidth, OrthoHeight, ZScale, ZOffset);
        } else {
            float halfFov = Utils::degreesToRadians(capture->FOVAngle) / 2;
            if ((int32)ERHIZBuffer::IsInverted) {
                proj_mat_transpose = FReversedZPerspectiveMatrix(halfFov, halfFov, x_axis_multiplier, y_axis_multiplier,
                ...
                proj_mat_transpose = FPerspectiveMatrix(halfFov, halfFov, x_axis_multiplier, y_axis_multiplier,
                                                        GNearClippingPlane, GNearClippingPlane);
            }
        }

        // Takes a vector from NORTH-EAST-DOWN coordinates (AutonomySim) to EAST-UP-SOUTH coordinates (Unreal). Leaves W
        // coordinate unchanged.
        FMatrix coordinateChangeTranspose =
            FMatrix(FPlane(0, 0, -1, 0), FPlane(1, 0, 0, 0), FPlane(0, -1, 0, 0), FPlane(0, 0, 0, 1));

        FMatrix projMatTransposeInAutonomySim = coordinateChangeTranspose * proj_mat_transpose;

        // Copy the result to an airlib::ProjectionMatrix while taking transpose.
        for (auto row = 0; row < 4; ++row)
            for (auto col = 0; col < 4; ++col)
                mat.matrix[col][row] = projMatTransposeInAutonomySim.M[row][col];
    } else
...
void APIPCamera::Tick(float DeltaTime) {
    if (gimbal_stabilization_ > 0) {
        ...
        if (!std::isnan(gimbald_rotator_.Pitch))
            rotator.Pitch =
                gimbald_rotator_.Pitch * gimbal_stabilization_ + rotator.Pitch * (1 - gimbal_stabilization_);
        if (!std::isnan(gimbald_rotator_.Roll))
            rotator.Roll = gimbald_rotator_.Roll * gimbal_stabilization_ + rotator.Roll * (1 - gimbal_stabilization_);
        if (!std::isnan(gimbald_rotator_.Yaw))
            rotator.Yaw = gimbald_rotator_.Yaw * gimbal_stabilization_ + rotator.Yaw * (1 - gimbal_stabilization_);
...
unsigned int APIPCamera::imageTypeCount() { return Utils::toNumeric(ImageType::Count); }
...
void APIPCamera::showToScreen() {
    ...
    UAutonomyBlueprintLib::LogMessage(TEXT("Camera: "), GetName(), LogDebugLevel::Informational);
}
...
bool APIPCamera::getCameraTypeEnabled(ImageType type) const { return camera_type_enabled_[Utils::toNumeric(type)]; }

void APIPCamera::setCameraTypeEnabled(ImageType type, bool enabled) { enableCaptureComponent(type, enabled); }
...
void APIPCamera::setCameraPose(const nervosys::autonomylib::Pose &relative_pose) {
...
nervosys::autonomylib::CameraInfo APIPCamera::getCameraInfo() const {
...
void APIPCamera::setupCameraFromSettings(const APIPCamera::CameraSetting &camera_setting,
    ...
    for (int image_type = -1; image_type < image_count; ++image_type) {
        ...
        if (image_type >= 0) { // scene capture components
            ...
            pixel_format =
                (pixel_format == EPixelFormat::PF_Unknown ? image_type_to_pixel_format_map_[image_type] : pixel_format);

            switch (Utils::toEnum<ImageType>(image_type)) {
            case ImageType::Scene:
            case ImageType::Infrared:
                updateCaptureComponentSetting(captures_[image_type], render_targets_[image_type], false, pixel_format,
                                              capture_setting, ned_transform, false);
                break;

            case ImageType::Segmentation:
            case ImageType::SurfaceNormals:
                updateCaptureComponentSetting(captures_[image_type], render_targets_[image_type], true, pixel_format,
                                              capture_setting, ned_transform, true);
                break;
            ...
            setNoiseMaterial(image_type, captures_[image_type], captures_[image_type]->PostProcessSettings,
                             noise_setting);
            copyCameraSettingsToSceneCapture(camera_, captures_[image_type]); // CinemAutonomySim
            ...
            copyCameraSettingsToAllSceneCapture(camera_); // CinemAutonomySim
        }
    }
}
...
// CinemAutonomySim
void APIPCamera::updateCameraSetting(UCineCameraComponent *camera, const CaptureSetting &setting,
                                     const NedTransform &ned_transform) {
    // if (!std::isnan(setting.target_gamma))
...
void APIPCamera::setNoiseMaterial(int image_type, UObject *outer, FPostProcessSettings &obj,
...
float APIPCamera::getFocalLength() const { return camera_->CurrentFocalLength; }
...
float APIPCamera::getFocusDistance() const { return camera_->FocusSettings.ManualFocusDistance; }
...
float APIPCamera::getFocusAperture() const { return camera_->CurrentAperture; }
