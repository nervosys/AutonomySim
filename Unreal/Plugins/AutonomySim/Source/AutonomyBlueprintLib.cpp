...
void UAutonomyBlueprintLib::enableWorldRendering(AActor *context, bool enable) {
...
void UAutonomyBlueprintLib::setSimulatePhysics(AActor *actor, bool simulate_physics) {
...
bool UAutonomyBlueprintLib::loadLevel(UObject *context, const FString &level_name) {
...
bool UAutonomyBlueprintLib::spawnPlayer(UWorld *context) {
...
std::vector<UPrimitiveComponent *> UAutonomyBlueprintLib::getPhysicsComponents(AActor *actor) {
...
void UAutonomyBlueprintLib::resetSimulatePhysics(AActor *actor) {
    TInlineComponentArray<UPrimitiveComponent *> components;
    actor->GetComponents(components);
...
void UAutonomyBlueprintLib::enableViewportRendering(AActor *context, bool enable) {
...
void UAutonomyBlueprintLib::OnBeginPlay() {
    image_wrapper_module_ = &FModuleManager::LoadModuleChecked<IImageWrapperModule>(FName("ImageWrapper"));
}

void UAutonomyBlueprintLib::OnEndPlay() {
    // nothing to do for now
    image_wrapper_module_ = nullptr;
}

IImageWrapperModule *UAutonomyBlueprintLib::getImageWrapperModule() { return image_wrapper_module_; }

void UAutonomyBlueprintLib::setLogMessagesVisibility(bool is_visible) {
...
void UAutonomyBlueprintLib::LogMessage(const FString &prefix, const FString &suffix, LogDebugLevel level,
    ...
    case LogDebugLevel::Failure:
        ...
        // UE_LOG(LogAutonomySim, Error, TEXT("%s%s"), *prefix, *suffix);
        break;
...
void UAutonomyBlueprintLib::GenerateAssetRegistryMap(const UObject *context, TMap<FString, FAssetData> &asset_map) {
    UAutonomyBlueprintLib::RunCommandOnGameThread(
        [context, &asset_map]() {
            ...
            // Find mesh in /Game and /AutonomySim asset registry. When more plugins are added this function will have
            // to change
...
void UAutonomyBlueprintLib::GenerateActorMap(const UObject *context, TMap<FString, AActor *> &scene_object_map) {
    auto world = context->GetWorld();
...
void UAutonomyBlueprintLib::setUnrealClockSpeed(const AActor *context, float clock_speed) {
    UAutonomyBlueprintLib::RunCommandOnGameThread(
        [context, clock_speed]() {
...
float UAutonomyBlueprintLib::GetWorldToMetersScale(const AActor *context) {
    float w2m = 100.f;
...
template <typename T> T *UAutonomyBlueprintLib::GetActorComponent(AActor *actor, FString name) {
    TArray<T *> components;
    actor->GetComponents(components);
    T *found = nullptr;
...
template UChildActorComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template USceneCaptureComponent2D *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template UStaticMeshComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template URotatingMovementComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template UCameraComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template UDetectionComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);
template UCineCameraComponent *UAutonomyBlueprintLib::GetActorComponent(AActor *, FString);

bool UAutonomyBlueprintLib::IsInGameThread() { return ::IsInGameThread(); }

void UAutonomyBlueprintLib::RunCommandOnGameThread(TFunction<void()> InFunction, bool wait, const TStatId InStatId) {
    if (IsInGameThread())
        InFunction();
    else {
        FGraphEventRef task = FFunctionGraphTask::CreateAndDispatchWhenReady(MoveTemp(InFunction), InStatId, nullptr,
...
template <> std::string UAutonomyBlueprintLib::GetMeshName<USkinnedMeshComponent>(USkinnedMeshComponent *mesh) {
    switch (mesh_naming_method_) {
    case nervosys::autonomylib::AutonomySimSettings::SegmentationSetting::MeshNamingMethodType::OwnerName:
        if (mesh->GetOwner())
    ...
    case nervosys::autonomylib::AutonomySimSettings::SegmentationSetting::MeshNamingMethodType::StaticMeshName:
...
std::string UAutonomyBlueprintLib::GetMeshName(ALandscapeProxy *mesh) {
    return std::string(TCHAR_TO_UTF8(*(mesh->GetName())));
}

void UAutonomyBlueprintLib::InitializeMeshStencilIDs(bool override_existing) {
...
bool UAutonomyBlueprintLib::SetMeshStencilID(const std::string &mesh_name, int object_id, bool is_name_regex) {
...
int UAutonomyBlueprintLib::GetMeshStencilID(const std::string &mesh_name) {
    // Takes a UStaticMeshComponent, USkinnedMeshComponent or ALandscapeProxy and returns their custom stencil ID if
    // their meshes's name or their owner's name (depending on the naming method in mesh_naming_method_) equals
    // mesh_name
...
std::vector<nervosys::autonomylib::MeshPositionVertexBuffersResponse> UAutonomyBlueprintLib::GetStaticMeshComponents() {
    std::vector<nervosys::autonomylib::MeshPositionVertexBuffersResponse> meshes;
    int num_meshes = 0;
    for (TObjectIterator<UStaticMeshComponent> comp; comp; ++comp) {
        *comp;

        std::string name = common_utils::Utils::toLower(GetMeshName(*comp));
        // The skybox is ignored here as it is huge, and really is of no use to the end user typically. Also the
        // associated meshes with the cameras
        if (name == "" || common_utils::Utils::startsWith(name, "default_") ||
            common_utils::Utils::startsWith(name, "sky") || common_utils::Utils::startsWith(name, "camera")) {
            continue;
        }
        ...
        nervosys::autonomylib::MeshPositionVertexBuffersResponse mesh;
        mesh.name = name;
        ...
        FPositionVertexBuffer *vertex_buffer =
            &comp->GetStaticMesh()->GetRenderData()->LODResources[0].VertexBuffers.PositionVertexBuffer;
#else
        FPositionVertexBuffer *vertex_buffer =
            &comp->GetStaticMesh()->RenderData->LODResources[0].VertexBuffers.PositionVertexBuffer;
#endif
        if (vertex_buffer) {
            ...
            ([vertex_buffer, data](FRHICommandListImmediate &RHICmdList) {
                FVector *indices = (FVector *)RHILockBuffer(vertex_buffer->VertexBufferRHI, 0,
                                                            vertex_buffer->VertexBufferRHI->GetSize(), RLM_ReadOnly);
            ...
            if (IndexBuffer->IndexBufferRHI->GetStride() == 2) {
                ...
                ([IndexBuffer, data_ptr](FRHICommandListImmediate &RHICmdList) {
                    uint16_t *indices = (uint16_t *)RHILockBuffer(IndexBuffer->IndexBufferRHI, 0,
                                                                  IndexBuffer->IndexBufferRHI->GetSize(), RLM_ReadOnly);
                    memcpy(data_ptr, indices, IndexBuffer->IndexBufferRHI->GetSize());
                    RHIUnlockBuffer(IndexBuffer->IndexBufferRHI);
                });

                // Need to force the render command to go through cause on the next iteration the buffer no longer
                // exists
                ...
                ENQUEUE_RENDER_COMMAND(GetIndexBuffer)
                ([IndexBuffer, data_ptr](FRHICommandListImmediate &RHICmdList) {
                    uint32_t *indices = (uint32_t *)RHILockBuffer(IndexBuffer->IndexBufferRHI, 0,
                                                                  IndexBuffer->IndexBufferRHI->GetSize(), RLM_ReadOnly);
                    memcpy(data_ptr, indices, IndexBuffer->IndexBufferRHI->GetSize());
                    RHIUnlockBuffer(IndexBuffer->IndexBufferRHI);
                });
            ...
            // Unreal stores more vertices than triangles. So here we find the highest referenced vertex and ignore any
            // after that
...
TArray<FName> UAutonomyBlueprintLib::ListWorldsInRegistry() {
    FARFilter Filter;
...
UObject *UAutonomyBlueprintLib::GetMeshFromRegistry(const std::string &load_object) {
    FARFilter Filter;
...
bool UAutonomyBlueprintLib::RunConsoleCommand(const AActor *context, const FString &command) {
...
void UAutonomyBlueprintLib::FollowActor(AActor *follower, const AActor *followee, const FVector &offset, bool fixed_z,
    ...
    FRotator next_rot =
        UKismetMathLibrary::FindLookAtRotation(follower->GetActorLocation(), followee->GetActorLocation());
...
UClass *UAutonomyBlueprintLib::LoadClass(const std::string &name) {
    FString str(name.c_str());
...
void UAutonomyBlueprintLib::CompressImageArray(int32 width, int32 height, const TArray<FColor> &src, TArray<uint8> &dest) {
    ...
    // at some point. At the moment, manually swapping Red and Blue
...
bool UAutonomyBlueprintLib::CompressUsingImageWrapper(const TArray<uint8> &uncompressed, const int32 width,
    ...
    if (uncompressed.Num() > 0) {
        IImageWrapperModule *ImageWrapperModule = UAutonomyBlueprintLib::getImageWrapperModule();
        TSharedPtr<IImageWrapper> ImageWrapper = ImageWrapperModule->CreateImageWrapper(EImageFormat::PNG);
        if (ImageWrapper.IsValid() &&
            ImageWrapper->SetRaw(&uncompressed[0], uncompressed.Num(), width, height, ERGBFormat::RGBA, 8)) {
