...
void ACameraDirector::BeginPlay() { Super::BeginPlay(); }
...
void ACameraDirector::Tick(float DeltaTime) {
    ...
    if (mode_ == ECameraDirectorMode::CAMERA_DIRECTOR_MODE_MANUAL) {
        ...
        UAutonomyBlueprintLib::FollowActor(ExternalCamera, follow_actor_, initial_ground_obs_offset_, ext_obs_fixed_z_);
    }
}

ECameraDirectorMode ACameraDirector::getMode() { return mode_; }

void ACameraDirector::initializeForBeginPlay(ECameraDirectorMode view_mode, AActor *follow_actor,
                                             APIPCamera *fpv_camera, APIPCamera *front_camera,
    ...
    initial_ground_obs_offset_ =
        camera_start_location_ - (follow_actor_ ? follow_actor_->GetActorLocation() : FVector::ZeroVector);
...
void ACameraDirector::setMode(ECameraDirectorMode mode) {
    { // first remove any settings done by previous mode
        ...
        if (mode_ == ECameraDirectorMode::CAMERA_DIRECTOR_MODE_NODISPLAY &&
            ...
            UAutonomyBlueprintLib::enableViewportRendering(this, true);
        }
    ...
    { // perform any settings to enter in to this mode
        ...
        case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_SPRINGARM_CHASE:
            ...
            break;
        case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_NODISPLAY:
            UAutonomyBlueprintLib::enableViewportRendering(this, false);
            break;
        default:
...
APIPCamera *ACameraDirector::getFpvCamera() const { return fpv_camera_; }

APIPCamera *ACameraDirector::getExternalCamera() const { return ExternalCamera; }

APIPCamera *ACameraDirector::getBackupCamera() const { return backup_camera_; }
...
void ACameraDirector::inputEventSpringArmChaseView() {
    if (ExternalCamera) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Camera is not available: ", "ExternalCamera", LogDebugLevel::Failure);
...
void ACameraDirector::inputEventBackupView() {
    if (backup_camera_) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Camera is not available: ", "backup_camera", LogDebugLevel::Failure);
...
void ACameraDirector::inputEventFlyWithView() {
    if (ExternalCamera) {
        ...
        if (follow_actor_)
            ExternalCamera->SetActorLocationAndRotation(follow_actor_->GetActorLocation() + initial_ground_obs_offset_,
                                                        camera_start_rotation_);
        disableCameras(true, true, false, true);
...
void ACameraDirector::inputEventFpvView() {
    if (fpv_camera_) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Camera is not available: ", "fpv_camera", LogDebugLevel::Failure);
