...
NedTransform::Vector3r NedTransform::toLocalNed(const FVector &position) const {
    return NedTransform::toVector3r(position - local_ned_offset_, 1 / world_to_meters_, true);
}
...
NedTransform::Vector3r NedTransform::toGlobalNed(const FVector &position) const {
    return NedTransform::toVector3r(position - global_transform_.GetLocation(), 1 / world_to_meters_, true);
}
NedTransform::Quaternionr NedTransform::toNed(const FQuat &q) const { return Quaternionr(q.W, -q.X, -q.Y, q.Z); }
float NedTransform::toNed(float length) const { return length / world_to_meters_; }
...
float NedTransform::fromNed(float length) const { return length * world_to_meters_; }
...
FQuat NedTransform::fromNed(const Quaternionr &q) const { return FQuat(-q.x(), -q.y(), q.z(), q.w()); }
...
FQuat NedTransform::fromUUtoFLU(const FQuat &q) const { return FQuat(-q.X, q.Y, -q.Z, q.W); }
// todo unused. need to manually plots tf axes' line in right handed FLU instead of using DrawDebugCoordinateSystem
FQuat NedTransform::fromGlobalNedToFLU(const Quaternionr &q) const { return fromUUtoFLU(fromNed(q)); }

FVector NedTransform::getGlobalOffset() const { return global_transform_.GetLocation(); }
FVector NedTransform::getLocalOffset() const { return local_ned_offset_; }
FTransform NedTransform::getGlobalTransform() const { return global_transform_; }
