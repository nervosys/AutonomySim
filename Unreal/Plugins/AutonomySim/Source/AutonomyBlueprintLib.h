 ...
 */
UCLASS()
class UAutonomyBlueprintLib : public UBlueprintFunctionLibrary {
    GENERATED_BODY()

  public:
    static void OnBeginPlay();
    static void OnEndPlay();
    static void LogMessageString(const std::string &prefix, const std::string &suffix, LogDebugLevel level,
    ...
    template <typename T> static T *GetActorComponent(AActor *actor, FString name);

    template <typename T> static T *FindActor(const UObject *context, FString name) {
    ...
    template <typename T> static void FindAllActor(const UObject *context, TArray<AActor *> &foundActors) {
    ...
    UFUNCTION(BlueprintCallable, Category = "AutonomySim|LevelAPI")
    static bool loadLevel(UObject *context, const FString &level_name);
    UFUNCTION(BlueprintCallable, Category = "AutonomySim|LevelAPI")
    static bool spawnPlayer(UWorld *context);
    UFUNCTION(BlueprintPure, Category = "AutonomySim|LevelAPI")
    ...
    static bool GetObstacle(const AActor *actor, const FVector &start, const FVector &end, FHitResult &hit,
                            const AActor *ignore_actor = nullptr, ECollisionChannel collision_channel = ECC_Visibility);
    static bool GetLastObstaclePosition(const AActor *actor, const FVector &start, const FVector &end, FHitResult &hit,
                                        const AActor *ignore_actor = nullptr,
                                        ECollisionChannel collision_channel = ECC_Visibility);
    static void FollowActor(AActor *follower, const AActor *followee, const FVector &offset, bool fixed_z = false,
                            float fixed_z_val = 2.0f);

    static bool SetMeshStencilID(const std::string &mesh_name, int object_id, bool is_name_regex = false);
    ...
    template <class T> static std::string GetMeshName(T *mesh) {
    ...
    static FInputActionBinding &
    BindActionToKey(const FName action_name, const FKey in_key, UserClass *actor,
                    typename FInputActionHandlerSignature::TUObjectMethodDelegate<UserClass>::FMethodPtr func,
                    bool on_press_or_release = false, bool shift_key = false, bool control_key = false,
                    bool alt_key = false, bool command_key = false) {
        ...
        return controller->InputComponent->BindAction(action_name, on_press_or_release ? IE_Pressed : IE_Released,
                                                      actor, func);
    }

    template <class UserClass>
    static FInputAxisBinding &
    BindAxisToKey(const FName axis_name, const FKey in_key, AActor *actor, UserClass *obj,
        ...
        return UAutonomyBlueprintLib::BindAxisToKey(axis, actor, obj, func);
    }

    template <class UserClass>
    static FInputAxisBinding &
    BindAxisToKey(const FInputAxisKeyMapping &axis, AActor *actor, UserClass *obj,
                  typename FInputAxisHandlerSignature::TUObjectMethodDelegate<UserClass>::FMethodPtr func) {
        APlayerController *controller = actor->GetWorld()->GetFirstPlayerController();
    ...
    static void RunCommandOnGameThread(TFunction<void()> InFunction, bool wait = false,
                                       const TStatId InStatId = TStatId());

    static float GetDisplayGamma();

    static EAppReturnType::Type ShowMessage(EAppMsgType::Type MessageType, const std::string &message,
                                            const std::string &title);

    static bool getLogMessagesHidden() { return log_messages_hidden_; }
    static void setLogMessagesVisibility(bool is_visible);

    static void
    SetMeshNamingMethod(nervosys::autonomylib::AutonomySimSettings::SegmentationSetting::MeshNamingMethodType method) {
    ...
    static std::vector<nervosys::autonomylib::MeshPositionVertexBuffersResponse> GetStaticMeshComponents();

  private:
    template <typename T> static void InitializeObjectStencilID(T *mesh, bool override_existing = true) {
    ...
    template <typename T>
    static void SetObjectStencilIDIfMatch(T *mesh, int object_id, const std::string &mesh_name, bool is_name_regex,
    ...
    template <typename T> static void SetObjectStencilID(T *mesh, int object_id) {
        if (object_id < 0) {
    ...
    template <typename T> static void SetRenderCustomDepth(T *mesh, bool enable) { mesh->SetRenderCustomDepth(enable); }
    ...
    static bool CompressUsingImageWrapper(const TArray<uint8> &uncompressed, const int32 width, const int32 height,
  ...
  private:
    ...
    static nervosys::autonomylib::AutonomySimSettings::SegmentationSetting::MeshNamingMethodType mesh_naming_method_;
