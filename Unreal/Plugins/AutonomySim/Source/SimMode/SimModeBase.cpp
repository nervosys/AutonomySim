...
// it to AutonomyLib and directly implement WorldSimApiBase interface
#include "WorldSimApi.h"

ASimModeBase *ASimModeBase::SIMMODE = nullptr;

ASimModeBase *ASimModeBase::getSimMode() { return SIMMODE; }

ASimModeBase::ASimModeBase() {
    SIMMODE = this;

    static ConstructorHelpers::FClassFinder<APIPCamera> external_camera_class(
        TEXT("Blueprint'/AutonomySim/Blueprints/BP_PIPCamera'"));
    external_camera_class_ = external_camera_class.Succeeded() ? external_camera_class.Class : nullptr;
    static ConstructorHelpers::FClassFinder<ACameraDirector> camera_director_class(
        TEXT("Blueprint'/AutonomySim/Blueprints/BP_CameraDirector'"));
    ...
    static ConstructorHelpers::FObjectFinder<UParticleSystem> collision_display(
        TEXT("ParticleSystem'/AutonomySim/StarterContent/Particles/P_Explosion.P_Explosion'"));
    ...
    static ConstructorHelpers::FClassFinder<UUserWidget> loading_screen_class_find(
        TEXT("WidgetBlueprint'/AutonomySim/Blueprints/BP_LoadingScreenWidget'"));
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> domain_rand_mat_finder(
        TEXT("Material'/AutonomySim/HUDAssets/DomainRandomizationMaterial.DomainRandomizationMaterial'"));
...
void ASimModeBase::BeginPlay() {
    ...
    global_ned_transform_.reset(
        new NedTransform(player_start_transform, UAutonomyBlueprintLib::GetWorldToMetersScale(this)));

    UAutonomyBlueprintLib::GenerateAssetRegistryMap(this, asset_map);

    world_sim_api_.reset(new WorldSimApi(this));
    api_provider_.reset(new nervosys::autonomylib::ApiProvider(world_sim_api_.get()));

    UAutonomyBlueprintLib::setLogMessagesVisibility(getSettings().log_messages_visible);
    ...
    initializeTimeOfDay();
    AutonomySimSettings::TimeOfDaySetting tod_setting = getSettings().tod_setting;
    ...
    UAutonomyBlueprintLib::LogMessage(TEXT("Press F1 to see help"), TEXT(""), LogDebugLevel::Informational);
    ...
    UAutonomyBlueprintLib::GenerateActorMap(this, scene_object_map);

    loading_screen_widget_->AddToViewport();
    loading_screen_widget_->SetVisibility(ESlateVisibility::Hidden);
}

const NedTransform &ASimModeBase::getGlobalNedTransform() { return *global_ned_transform_; }
    ...
    for (auto &api : api_provider_->getVehicleApis()) {
        if (api) { // sim-only vehicles may have api as null
            ...
            if (!api->isReady(message)) {
                UAutonomyBlueprintLib::LogMessage("Vehicle was not initialized", "", LogDebugLevel::Failure);
                if (message.size() > 0) {
...
void ASimModeBase::setStencilIDs() {
    UAutonomyBlueprintLib::SetMeshNamingMethod(getSettings().segmentation_setting.mesh_naming_method);

    if (getSettings().segmentation_setting.init_method ==
        AutonomySimSettings::SegmentationSetting::InitMethodType::CommonObjectsRandomIDs) {
        UAutonomyBlueprintLib::InitializeMeshStencilIDs(getSettings().segmentation_setting.override_existing);
    }
    // else don't init
}
...
bool ASimModeBase::isPaused() const { return UGameplayStatics::IsGamePaused(this->GetWorld()); }

void ASimModeBase::pause(bool is_paused) { UGameplayStatics::SetGamePaused(this->GetWorld(), is_paused); }
...
void ASimModeBase::setWind(const nervosys::autonomylib::Vector3r &wind) const {
    // should be overridden by derived class
    unused(wind);
    throw std::domain_error("setWind not implemented by SimMode");
}

void ASimModeBase::setExtForce(const nervosys::autonomylib::Vector3r &ext_force) const {
    // should be overridden by derived class
    unused(ext_force);
    throw std::domain_error("setExtForce not implemented by SimMode");
}

std::unique_ptr<nervosys::autonomylib::ApiServerBase> ASimModeBase::createApiServer() const {
...
std::string ASimModeBase::getDebugReport() { return debug_reporter_.getOutput(); }

void ASimModeBase::setupInputBindings() {
    UAutonomyBlueprintLib::EnableInput(this);

    UAutonomyBlueprintLib::BindActionToKey("InputEventResetAll", EKeys::BackSpace, this, &ASimModeBase::reset);
}
...
void ASimModeBase::initializeCameraDirector(const FTransform &camera_transform, float follow_distance) {
    ...
    UAutonomyBlueprintLib::FindAllActor<ACameraDirector>(this, camera_dirs);
    if (camera_dirs.Num() == 0) {
        ...
        CameraDirector = this->GetWorld()->SpawnActor<ACameraDirector>(camera_director_class_, camera_transform,
                                                                       camera_spawn_params);
        ...
        CameraDirector->ExternalCamera =
            this->GetWorld()->SpawnActor<APIPCamera>(external_camera_class_, camera_transform, camera_spawn_params);
    } else {
...
void ASimModeBase::initializeExternalCameras() {
    ...
    for (const auto &camera_setting_pair : getSettings().external_cameras) {
        ...
        APIPCamera *camera =
            this->GetWorld()->SpawnActor<APIPCamera>(pip_camera_class, camera_transform, camera_spawn_params);
...
void ASimModeBase::stopRecording() { FRecordingThread::stopRecording(); }

void ASimModeBase::startRecording() {
    FRecordingThread::startRecording(getSettings().recording_setting, getApiProvider()->getVehicleSimApis());
}

bool ASimModeBase::isRecording() const { return FRecordingThread::isRecording(); }
...
const APIPCamera *ASimModeBase::getCamera(const nervosys::autonomylib::CameraDetails &camera_details) const {
    return camera_details.external
               ? getExternalCamera(camera_details.camera_name)
...
void ASimModeBase::startApiServer() {
    if (getSettings().enable_rpc) {
        ...
        try {
            ...
            UAutonomyBlueprintLib::LogMessageString("Cannot start RpcLib Server", ex.what(), LogDebugLevel::Failure);
        }
    } else
        UAutonomyBlueprintLib::LogMessageString("API server is disabled in settings", "", LogDebugLevel::Informational);
}
...
bool ASimModeBase::isApiServerStarted() { return api_server_ != nullptr; }

void ASimModeBase::updateDebugReport(nervosys::autonomylib::StateReporterWrapper &debug_reporter) {
    ...
    if (debug_reporter.canReport()) {
        ...
        for (auto &api : getApiProvider()->getVehicleSimApis()) {
            ...
            nervosys::autonomylib::StateReporter &reporter = *debug_reporter.getReporter();
...
FRotator ASimModeBase::toFRotator(const nervosys::autonomylib::AutonomySimSettings::Rotation &rotation,
                                  const FRotator &default_val) {
...
APawn *ASimModeBase::createVehiclePawn(const AutonomySimSettings::VehicleSetting &vehicle_setting) {
    ...
    auto vehicle_bp_class =
        UAutonomyBlueprintLib::LoadClass(getSettings().pawn_paths.at(getVehiclePawnPathName(vehicle_setting)).pawn_bp);
...
std::unique_ptr<PawnSimApi> ASimModeBase::createVehicleApi(APawn *vehicle_pawn) {
    ...
    PawnSimApi::Params pawn_sim_api_params(vehicle_pawn, &getGlobalNedTransform(), getVehiclePawnEvents(vehicle_pawn),
                                           getVehiclePawnCameras(vehicle_pawn), pip_camera_class,
...
bool ASimModeBase::createVehicleAtRuntime(const std::string &vehicle_name, const std::string &vehicle_type,
    ...
    if (!isVehicleTypeSupported(vehicle_type_lower)) {
        Utils::log(Utils::stringf("Vehicle type %s is not supported in this game mode", vehicle_type.c_str()),
                   Utils::kLogLevelWarn);
        return false;
    }
    ...
    // Retroactively adjust AutonomySimSettings, so it's like we knew about this vehicle all along
    AutonomySimSettings::singleton().addVehicleSetting(vehicle_name, vehicle_type_lower, pose, pawn_path);
...
void ASimModeBase::setupVehiclesAndCamera() {
    ...
    FVector camera_director_position_uu =
        uu_origin.GetLocation() + getGlobalNedTransform().fromLocalNed(camera_director_setting.position);
...
void ASimModeBase::registerPhysicsBody(nervosys::autonomylib::VehicleSimApiBase *physicsBody) {
...
std::string ASimModeBase::getVehiclePawnPathName(const AutonomySimSettings::VehicleSetting &vehicle_setting) const {
    // derived class should override this method to retrieve types of pawns they support
    return "";
}
...
void ASimModeBase::drawLidarDebugPoints() {
    ...
    for (auto &sim_api : getApiProvider()->getVehicleSimApis()) {
        ...
        nervosys::autonomylib::VehicleApiBase *api = getApiProvider()->getVehicleApi(vehicle_name);
        if (api != nullptr) {
            nervosys::autonomylib::uint count_lidars = api->getSensors().size(SensorType::Lidar);
            ...
            for (nervosys::autonomylib::uint i = 0; i < count_lidars; i++) {
                ...
                if (lidar != nullptr && lidar->getParams().draw_debug_points) {
                    ...
                    nervosys::autonomylib::LidarData lidar_data = lidar->getOutput();

                    if (lidar_data.point_cloud.size() < 3)
                        return;

                    for (int j = 0; j < lidar_data.point_cloud.size(); j = j + 3) {
                        Vector3r point(lidar_data.point_cloud[j], lidar_data.point_cloud[j + 1],
                                       lidar_data.point_cloud[j + 2]);

                        FVector uu_point;

                        if (lidar->getParams().data_frame ==
                            AutonomySimSettings::LidarSetting::DataFrame::VehicleInertialFrame) {
                            uu_point = pawn_sim_api->getNedTransform().fromLocalNed(point);
                        } else if (lidar->getParams().data_frame ==
                                   AutonomySimSettings::LidarSetting::DataFrame::SensorLocalFrame) {
                        ...
                        DrawDebugPoint(this->GetWorld(), uu_point,
                                       5, // size
                                       FColor::Green,
...
void ASimModeBase::drawDistanceSensorDebugPoints() {
    ...
    for (auto &sim_api : getApiProvider()->getVehicleSimApis()) {
        ...
        if (api != nullptr) {
            nervosys::autonomylib::uint count_distance_sensors = api->getSensors().size(SensorType::Distance);
            ...
            for (nervosys::autonomylib::uint i = 0; i < count_distance_sensors; i++) {
                ...
                if (distance_sensor != nullptr && distance_sensor->getParams().draw_debug_points) {
                    ...
                    // order of Pose addition is important here because it also adds quaternions which is not
                    // commutative!
                    Pose distance_sensor_pose = distance_sensor_data.relative_pose + vehicle_pose;
                    Vector3r start = distance_sensor_pose.position;
                    Vector3r point =
                        start + VectorMath::rotateVector(VectorMath::front(), distance_sensor_pose.orientation, true) *
