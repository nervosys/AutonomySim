...
FObjectFilter::FObjectFilter()
    : static_mesh_(nullptr), skeletal_mesh_(nullptr), actor_class_(nullptr), component_class_(nullptr), actor_tag_(),
      component_tag_(), actor_instance_(nullptr) {}
...
bool FObjectFilter::matchesActor(AActor *actor) const {
    ...
    for (UActorComponent *actor_component : actor_components) {
        if (static_mesh_ || wildcard_mesh_names_.Num() != 0) {
            UStaticMeshComponent *static_mesh_component = Cast<UStaticMeshComponent>(actor_component);
            if (static_mesh_component) {
                if (static_mesh_ && static_mesh_component->GetStaticMesh() == static_mesh_) {
                    return true;
                }
                if (wildcard_mesh_names_.Num() != 0 && static_mesh_component->GetStaticMesh() != nullptr &&
        ...
        if (skeletal_mesh_ || wildcard_mesh_names_.Num() != 0) {
            ...
            if (skeletal_mesh_component) {
                if (skeletal_mesh_ && skeletal_mesh_component->SkeletalMesh == skeletal_mesh_) {
                    return true;
                }
                if (wildcard_mesh_names_.Num() != 0 && skeletal_mesh_component->SkeletalMesh &&
...
bool FObjectFilter::matchesComponent(UActorComponent *actor_component) const {
    ...
    if (static_mesh_ || wildcard_mesh_names_.Num() != 0) {
        ...
        if (StaticMeshComponent) {
            ...
            if (wildcard_mesh_names_.Num() != 0 && StaticMeshComponent->GetStaticMesh()->IsValidLowLevel() &&
    ...
    if (skeletal_mesh_ || wildcard_mesh_names_.Num() != 0) {
        ...
        if (SkeletalMeshComponent) {
            ...
            if (wildcard_mesh_names_.Num() != 0 && isMatchAnyWildcard(SkeletalMeshComponent->SkeletalMesh->GetName())) {
                bMatchesWildcardMeshName = true;
            }
        }
    }
    ...
    if (actor_class_) {
        if (actor_component->GetOwner() && actor_component->GetOwner()->GetClass()->IsChildOf(actor_class_)) {
            bMatchesActorClass = true;
        }
    }

    if (!actor_tag_.IsNone()) {
        if (actor_component->GetOwner() && actor_component->GetOwner()->ActorHasTag(actor_tag_)) {
            bMatchesActorTag = true;
        }
    }

    return bMatchesStaticMesh && bMatchesSkeletalMesh && bMatchesWildcardMeshName && bMatchesActorClass &&
           bMatchesActorTag && bMatchesComponentClass && bMatchesComponentClass && bMatchesComponentTag;
}
