...
nervosys::autonomylib::WorkerThreadSignal FRecordingThread::finishing_signal_;
bool FRecordingThread::first_ = true;

FRecordingThread::FRecordingThread() : stop_task_counter_(0), recording_file_(nullptr), is_ready_(false) {
    thread_.reset(FRunnableThread::Create(this, TEXT("FRecordingThread"), 0,
                                          TPri_BelowNormal)); // Windows default, possible to specify more priority
}
...
void FRecordingThread::startRecording(
    ...
    for (const auto &vehicle_sim_api : vehicle_sim_apis) {
        ...
        running_instance_->last_poses_[vehicle_name] = nervosys::autonomylib::Pose();
    }
...
void FRecordingThread::init() { first_ = true; }

bool FRecordingThread::isRecording() { return running_instance_ != nullptr; }
...
bool FRecordingThread::Init() {
    ...
    if (recording_file_) {
        UAutonomyBlueprintLib::LogMessage(TEXT("Initiated recording thread"), TEXT(""), LogDebugLevel::Success);
    }
    return true;
}
...
void FRecordingThread::Stop() { stop_task_counter_.Increment(); }
