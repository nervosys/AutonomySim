...
void RecordingFile::appendRecord(const std::vector<nervosys::autonomylib::ImageCaptureBase::ImageResponse> &responses,
    ...
    for (auto i = 0; i < responses.size(); ++i) {
        ...
        image_file_name << "img_" << vehicle_sim_api->getVehicleName() << "_" << response.camera_name << "_"
    ...
    if (save_success || (responses.size() == 0)) {
        ...
        // UAutonomyBlueprintLib::LogMessage(TEXT("Screenshot saved to:"), filePath, LogDebugLevel::Success);
    }
}
...
bool RecordingFile::isFileOpen() const { return log_file_handle_ != nullptr; }
...
void RecordingFile::startRecording(nervosys::autonomylib::VehicleSimApiBase *vehicle_sim_api, const std::string &folder) {
    try {
        ...
        std::string log_filepath =
            common_utils::FileSystem::getLogFileNamePath(log_folderpath, record_filename, "", ".txt", false);
        if (log_filepath != "")
...
void RecordingFile::stopRecording(bool ignore_if_stopped) {
    ...
    if (!isFileOpen()) {
        ...
        UAutonomyBlueprintLib::LogMessage(TEXT("Recording Error"), TEXT("File was not open"), LogDebugLevel::Failure);
    } else
        closeFile();

    UAutonomyBlueprintLib::LogMessage(TEXT("Recording: "), TEXT("Stopped"), LogDebugLevel::Success);
    UAutonomyBlueprintLib::LogMessage(TEXT("Data saved to: "), FString(image_path_.c_str()), LogDebugLevel::Success);
}

bool RecordingFile::isRecording() const { return is_recording_; }
