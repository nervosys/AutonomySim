...
void RenderRequest::getScreenshot(std::shared_ptr<RenderParams> params[],
                                  std::vector<std::shared_ptr<RenderResult>> &results, unsigned int req_size,
    ...
    if (use_safe_method) {
        ...
        AsyncTask(ENamedThreads::GameThread, [this]() {
            ...
            end_draw_handle_ = game_viewport_->OnEndDraw().AddLambda([this] {
                ...
                ([This](FRHICommandListImmediate &RHICmdList) { This->ExecuteTask(); });
...
FReadSurfaceDataFlags RenderRequest::setupRenderResource(const FTextureRenderTargetResource *rt_resource,
                                                         const RenderParams *params, RenderResult *result,
                                                         FIntPoint &size) {
    ...
    if (params_ != nullptr && req_size_ > 0) {
        for (unsigned int i = 0; i < req_size_; ++i) {
            ...
            if (rt_resource != nullptr) {
                ...
                // https://github.com/nervosys/AutonomySim/pull/162/commits/63e80c43812300a8570b04ed42714a3f6949e63f#diff-56b790f9394f7ca1949ddbb320d8456fR64
                if (!params_[i]->pixels_as_float) {
                    // below is undocumented method that avoids flushing, but it seems to segfault every 2000 or so
                    // calls
                    RHICmdList.ReadSurfaceData(rhi_texture, FIntRect(0, 0, size.X, size.Y), results_[i]->bmp, flags);
                } else {
                    RHICmdList.ReadSurfaceFloatData(rhi_texture, FIntRect(0, 0, size.X, size.Y), results_[i]->bmp_float,
                                                    CubeFace_PosX, 0, 0);
                }
            }

            results_[i]->time_stamp = nervosys::autonomylib::ClockFactory::get()->nowNanos();
        }
