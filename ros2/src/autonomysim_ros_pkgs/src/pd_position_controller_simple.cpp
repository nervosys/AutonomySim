...
PIDPositionController::PIDPositionController(const std::shared_ptr<rclcpp::Node> nh)
    : use_eth_lib_for_geodetic_conv_(true), nh_(nh), has_home_geo_(false), reached_goal_(false), has_goal_(false),
      has_odom_(false), got_goal_once_(false) {
...
void PIDPositionController::initialize_ros() {
    ...
    // ROS timers
    update_control_cmd_timer_ =
        nh_->create_wall_timer(std::chrono::duration<double>(update_control_every_n_sec),
...
bool PIDPositionController::local_position_goal_srv_cb(
    ...
    if (has_goal_ && !reached_goal_) {
        ...
        RCLCPP_ERROR_STREAM(nh_->get_logger(),
                            "denying position goal request-> I am still following the previous goal");
        return false;
    }
    ...
    if (!has_goal_) {
        ...
        RCLCPP_INFO_STREAM(nh_->get_logger(), "got goal: x=" << target_position_.x << " y=" << target_position_.y
                                                             << " z=" << target_position_.z
                                                             << " yaw=" << target_position_.yaw);
...
bool PIDPositionController::local_position_goal_srv_override_cb(
    ...
    target_position_.z = request->z;
    target_position_.yaw = request->yaw;
    RCLCPP_INFO_STREAM(nh_->get_logger(), "got goal: x=" << target_position_.x << " y=" << target_position_.y << " z="
...
void PIDPositionController::home_geopoint_cb(const autonomysim_interfaces::msg::GPSYaw::SharedPtr gps_msg) {
    ...
    has_home_geo_ = true;
    RCLCPP_INFO_STREAM(nh_->get_logger(), "GPS reference initializing " << gps_msg->latitude << ", "
                                                                        << gps_msg->longitude << ", "
                                                                        << gps_msg->altitude);
...
bool PIDPositionController::gps_goal_srv_cb(
    ...
    if (!has_goal_) {
        ...
        if (use_eth_lib_for_geodetic_conv_) {
            ...
            // RCLCPP_INFO_STREAM("geodetic_converter_ GPS reference initialized correctly (lat long in radians) " <<
        ...
        {
            RCLCPP_INFO_STREAM(nh_->get_logger(), "home geopoint: lat=" << gps_home.latitude
        ...
        RCLCPP_INFO_STREAM(nh_->get_logger(), "got GPS goal: lat=" << goal_gps_point.latitude
                                                                   << " long=" << goal_gps_point.longitude
                                                                   << " alt=" << goal_gps_point.altitude
                                                                   << " yaw=" << target_position_.yaw);
        RCLCPP_INFO_STREAM(nh_->get_logger(), "converted NED goal is: x="
                                                  << target_position_.x << " y=" << target_position_.y
                                                  << " z=" << target_position_.z << " yaw=" << target_position_.yaw);
...
bool PIDPositionController::gps_goal_srv_override_cb(
    ...
    RCLCPP_INFO_STREAM(nh_->get_logger(),
                       "got GPS goal: lat=" << goal_gps_point.latitude << " long=" << goal_gps_point.longitude
                                            << " alt=" << goal_gps_point.altitude << " yaw=" << target_position_.yaw);
    RCLCPP_INFO_STREAM(nh_->get_logger(),
                       "converted NED goal is: x=" << target_position_.x << " y=" << target_position_.y
                                                   << " z=" << target_position_.z << " yaw=" << target_position_.yaw);
...
void PIDPositionController::update_control_cmd_timer_cb() {
    ...
    if (has_goal_) {
        ...
        if (reached_goal_) {
            ...
            has_goal_ = false;
            // dear future self, this function doesn't return coz we need to keep on actively hovering at last goal
            // pose. don't act smart
        } else {
...
void PIDPositionController::enforce_dynamic_constraints() {
    double vel_norm_horz =
        sqrt((vel_cmd_.twist.linear.x * vel_cmd_.twist.linear.x) + (vel_cmd_.twist.linear.y * vel_cmd_.twist.linear.y));
    ...
    if (std::fabs(vel_cmd_.twist.linear.z) > constraints_.max_vel_vert_abs) {
        ...
        vel_cmd_.twist.linear.z =
            (vel_cmd_.twist.linear.z / std::fabs(vel_cmd_.twist.linear.z)) * constraints_.max_vel_vert_abs;
    }
    // todo yaw limits
    if (std::fabs(vel_cmd_.twist.linear.z) > constraints_.max_yaw_rate_degree) {
        ...
        // template <typename T> double sgn(T val) { return (T(0) < val) - (val < T(0)); }
        vel_cmd_.twist.linear.z =
            (vel_cmd_.twist.linear.z / std::fabs(vel_cmd_.twist.linear.z)) * constraints_.max_yaw_rate_degree;
    }
}
