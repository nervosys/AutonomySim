...
constexpr char AutonomySimROSWrapper::CAM_YML_NAME[];
constexpr char AutonomySimROSWrapper::WIDTH_YML_NAME[];
constexpr char AutonomySimROSWrapper::HEIGHT_YML_NAME[];
constexpr char AutonomySimROSWrapper::K_YML_NAME[];
constexpr char AutonomySimROSWrapper::D_YML_NAME[];
constexpr char AutonomySimROSWrapper::R_YML_NAME[];
constexpr char AutonomySimROSWrapper::P_YML_NAME[];
constexpr char AutonomySimROSWrapper::DMODEL_YML_NAME[];

const std::unordered_map<int, std::string> AutonomySimROSWrapper::image_type_int_to_string_map_ = {
    {0, "Scene"},        {1, "DepthPlanar"},    {2, "DepthPerspective"}, {3, "DepthVis"}, {4, "DisparityNormalized"},
    {5, "Segmentation"}, {6, "SurfaceNormals"}, {7, "Infrared"}};
...
void AutonomySimROSWrapper::create_ros_pubs_from_settings_json() {
    ...
    for (const auto &curr_vehicle_elem : AutonomySimSettings::singleton().vehicles) {
        ...
        for (auto &curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto &curr_capture_elem : camera_setting.capture_settings) {
                ...
                // todo why does AutonomySimSettings::loadCaptureSettings calls
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    if (curr_image_type == ImageType::Scene || curr_image_type == ImageType::Segmentation ||
                        curr_image_type == ImageType::SurfaceNormals || curr_image_type == ImageType::Infrared) {
                    ...
                    cam_info_pub_vec_.push_back(
                        nh_->create_publisher<sensor_msgs::msg::CameraInfo>(camera_topic + "/camera_info", 10));
            ...
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Imu: {
                    SensorPublisher<sensor_msgs::msg::Imu> sensor_publisher =
                        create_sensor_publisher<sensor_msgs::msg::Imu>("Imu", sensor_setting->sensor_name,
                                                                       sensor_setting->sensor_type,
                ...
                case SensorBase::SensorType::Gps: {
                    SensorPublisher<sensor_msgs::msg::NavSatFix> sensor_publisher =
                        create_sensor_publisher<sensor_msgs::msg::NavSatFix>(
                            "Gps", sensor_setting->sensor_name, sensor_setting->sensor_type,
                ...
                case SensorBase::SensorType::Magnetometer: {
                    SensorPublisher<sensor_msgs::msg::MagneticField> sensor_publisher =
                        create_sensor_publisher<sensor_msgs::msg::MagneticField>(
                            "Magnetometer", sensor_setting->sensor_name, sensor_setting->sensor_type,
                ...
                case SensorBase::SensorType::Distance: {
                    SensorPublisher<sensor_msgs::msg::Range> sensor_publisher =
                        create_sensor_publisher<sensor_msgs::msg::Range>(
                            "Distance", sensor_setting->sensor_name, sensor_setting->sensor_type,
                ...
                case SensorBase::SensorType::Lidar: {
                    ...
                    nervosys::autonomylib::LidarSimpleParams params;
                    ...
                    SensorPublisher<sensor_msgs::msg::PointCloud2> sensor_publisher =
                        create_sensor_publisher<sensor_msgs::msg::PointCloud2>(
                            "Lidar", sensor_setting->sensor_name, sensor_setting->sensor_type,
        ...
        vehicle_name_ptr_map_.emplace(
            curr_vehicle_name,
            std::move(vehicle_ros)); // allows fast lookup in command callbacks in case of a lot of drones
    }
...
template <typename T>
const SensorPublisher<T> AutonomySimROSWrapper::create_sensor_publisher(const std::string &sensor_type_name,
...
tf2::Quaternion AutonomySimROSWrapper::get_tf2_quat(const nervosys::autonomylib::Quaternionr &airlib_quat) const {
...
void AutonomySimROSWrapper::vel_cmd_group_body_frame_cb(const autonomysim_interfaces::msg::VelCmdGroup::SharedPtr msg) {
    ...
    for (const auto &vehicle_name : msg->vehicle_names) {
        auto drone = static_cast<MultirotorROS *>(vehicle_name_ptr_map_[vehicle_name].get());
        drone->vel_cmd_ =
            get_airlib_body_vel_cmd(msg->vel_cmd, drone->curr_drone_state_.kinematics_estimated.pose.orientation);
        drone->has_vel_cmd_ = true;
    }
}
...
void AutonomySimROSWrapper::gimbal_angle_quat_cmd_cb(
    ...
    try {
        ...
        gimbal_cmd_.target_quat = get_airlib_quat(quat_control_cmd); // AutonomySim uses wxyz
...
nav_msgs::msg::Odometry AutonomySimROSWrapper::get_odom_msg_from_kinematic_state(
    const nervosys::autonomylib::Kinematics::State &kinematics_estimated) const {
...
AutonomySimROSWrapper::get_odom_msg_from_car_state(const nervosys::autonomylib::CarApiBase::CarState &car_state) const {
...
sensor_msgs::msg::MagneticField AutonomySimROSWrapper::get_mag_msg_from_autonomysim(
    ...
    std::copy(std::begin(mag_data.magnetic_field_covariance), std::end(mag_data.magnetic_field_covariance),
...
void AutonomySimROSWrapper::publish_odom_tf(const nav_msgs::msg::Odometry &odom_msg) {
...
nervosys::autonomylib::GeoPoint AutonomySimROSWrapper::get_origin_geo_point() const {
    nervosys::autonomylib::HomeGeoPoint geo_point = AutonomySimSettings::singleton().origin_geopoint;
...
VelCmd AutonomySimROSWrapper::get_airlib_world_vel_cmd(const autonomysim_interfaces::msg::VelCmd &msg) const {
    ...
    vel_cmd.drivetrain = nervosys::autonomylib::DrivetrainType::MaxDegreeOfFreedom;
...
VelCmd AutonomySimROSWrapper::get_airlib_body_vel_cmd(const autonomysim_interfaces::msg::VelCmd &msg,
    ...
    vel_cmd.x = (msg.twist.linear.x * cos(yaw)) - (msg.twist.linear.y * sin(yaw)); // body frame assuming zero pitch
                                                                                   // roll
    vel_cmd.y = (msg.twist.linear.x * sin(yaw)) + (msg.twist.linear.y * cos(yaw)); // body frame
    vel_cmd.z = msg.twist.linear.z;
    vel_cmd.drivetrain = nervosys::autonomylib::DrivetrainType::MaxDegreeOfFreedom;
    vel_cmd.yaw_mode.is_rate = true;
    // AutonomySim uses degrees
    vel_cmd.yaw_mode.yaw_or_rate = math_common::rad2deg(msg.twist.angular.z);
...
void AutonomySimROSWrapper::drone_state_timer_cb() {
    try {
...
void AutonomySimROSWrapper::update_and_publish_static_transforms(VehicleROS *vehicle_ros) {
...
rclcpp::Time AutonomySimROSWrapper::update_state() {
    ...
    for (auto &vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        // get drone state from AutonomySim
        ...
        // convert AutonomySim drone state to ROS msgs
...
void AutonomySimROSWrapper::publish_vehicle_state() {
    for (auto &vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        auto &vehicle_ros = vehicle_name_ptr_pair.second;
...
void AutonomySimROSWrapper::update_commands() {
    for (auto &vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        auto &vehicle_ros = vehicle_name_ptr_pair.second;
...
// AutonomySim uses nans for zeros in settings.json. we set them to zeros here for handling tfs in ROS
void AutonomySimROSWrapper::set_nans_to_zeros_in_pose(VehicleSetting &vehicle_setting) const {
...
void AutonomySimROSWrapper::set_nans_to_zeros_in_pose(const VehicleSetting &vehicle_setting,
...
void AutonomySimROSWrapper::convert_tf_msg_to_enu(geometry_msgs::msg::TransformStamped &tf_msg) {
...
AutonomySimROSWrapper::get_camera_optical_tf_from_body_tf(const geometry_msgs::msg::Transform &body_tf) const {
    ...
    if (isENU_)
        opticalQ *= nervosys::autonomylib::Quaternionr(
            ...
            0); // CamOptical in CamBodyENU is rmat[1,0,0;0,0,-1;0,1,0]==xyzw[-0.7071068,0,0,0.7071068]
    else
        opticalQ *= nervosys::autonomylib::Quaternionr(
            0.5, 0.5, 0.5, 0.5); // CamOptical in CamBodyNED is rmat[0,0,1;1,0,0;0,1,0]==xyzw[0.5,0.5,0.5,0.5]
...
void AutonomySimROSWrapper::append_static_vehicle_tf(VehicleROS *vehicle_ros, const VehicleSetting &vehicle_setting) {
...
void AutonomySimROSWrapper::append_static_lidar_tf(VehicleROS *vehicle_ros, const std::string &lidar_name,
...
void AutonomySimROSWrapper::append_static_camera_tf(VehicleROS *vehicle_ros, const std::string &camera_name,
...
void AutonomySimROSWrapper::lidar_timer_cb() {
    try {
        for (auto &vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
...
AutonomySimROSWrapper::get_img_msg_from_response(const ImageResponse &img_response, const rclcpp::Time curr_ros_time,
...
std::shared_ptr<sensor_msgs::msg::Image>
AutonomySimROSWrapper::get_depth_img_msg_from_response(const ImageResponse &img_response,
                                                       const rclcpp::Time curr_ros_time, const std::string frame_id) {
    unused(curr_ros_time);
...
sensor_msgs::msg::CameraInfo AutonomySimROSWrapper::generate_cam_info(const std::string &camera_name,
                                                                      const CameraSetting &camera_setting,
    ...
    // todo focal length in Y direction should be same as X it seems. this can change in future a scene capture
    // component which exactly correponds to a cine camera float f_y = (capture_setting.height / 2.0) /
...
void AutonomySimROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse> &img_response_vec,
                                                             const int img_response_idx,
                                                             const std::string &vehicle_name) {
    // todo add option to use AutonomySim time (image_response.TTimePoint) like Gazebo /use_sim_time param
    rclcpp::Time curr_ros_time = nh_->now();
    int img_response_idx_internal = img_response_idx;

    for (const auto &curr_img_response : img_response_vec) {
        // todo publishing a tf for each capture type seems stupid. but it foolproofs us against render thread's async
        // stuff, I hope. Ideally, we should loop over cameras and then captures, and publish only one tf.
...
// camera poses are obtained from AutonomySim's client API which are in (local) NED frame.
// We first do a change of basis to camera optical frame (Z forward, X right, Y down)
void AutonomySimROSWrapper::publish_camera_tf(const ImageResponse &img_response, const rclcpp::Time &ros_time,
...
void AutonomySimROSWrapper::convert_yaml_to_simple_mat(const YAML::Node &node, SimpleMatrix &m) const {
    int rows, cols;
...
void AutonomySimROSWrapper::read_params_from_yaml_and_fill_cam_info_msg(const std::string &file_name,
