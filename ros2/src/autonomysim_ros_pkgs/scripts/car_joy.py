#!/usr/bin/env python
#
# Capture joystick events using ROS and convert to AutonomySim Car API commands
# to enable: `rosrun joy joy_node`
#
...
import autonomysim_ros_pkgs as air
import autonomysim_ros_pkgs.msg
...
class CarCommandTranslator(object):
    def __init__(self):
        ...
        car_control_topic = rospy.get_param(
            "~car_control_topic", "/autonomysim_node/drone_1/car_cmd"
        )

        self.joy_msg = None

        self.joy_sub = rospy.Subscriber("joy", sensor_msgs.msg.Joy, self.handle_joy)

        self.command_pub = rospy.Publisher(
            car_control_topic, air.msg.CarControls, queue_size=0
        )

        self.update_time = rospy.Timer(
            rospy.Duration(1.0 / update_rate_hz), self.handle_update_timer
        )
    ...
    def handle_update_timer(self, ignored):
        ...
        controls = autonomysim_ros_pkgs.msg.CarControls()
        ...
        controls.manual = (
            True  # set to False for automatic transmission along with manual_gear > 1
        )
        if not self.last_nuetral_btn and nuetral_btn:
            ...
            not self.last_shift_down_btn
            and shift_down_btn
            and self.last_gear > 1
            and self.manual_transmission
        ):
            self.last_gear -= 1
            self.parked = False
            self.shift_mode_manual = True
        elif (
            not self.last_shift_up_btn
            and shift_up_btn
            and self.last_gear >= 1
            and self.manual_transmission
        ):
