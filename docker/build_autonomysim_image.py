# Build AutonomySim Docker image

import argparse
import subprocess

def main():
    parser = argparse.ArgumentParser(description='AutonomySim docker image builder')
    parser.add_argument('--source', action='store_true', help='compile unreal and AutonomySim from source') # default is false
...
def build_docker_image(args):
    ...
    if args.source:
        ...
        if not args.target_image:
            args.target_image = 'AutonomySim_source' + ':' + target_image_tag
    else:
        ...
        target_image_tag = args.base_image.split(":")[1] # take tag from base image
        if not args.target_image:
            args.target_image = 'AutonomySim_binary' + ':' + target_image_tag
