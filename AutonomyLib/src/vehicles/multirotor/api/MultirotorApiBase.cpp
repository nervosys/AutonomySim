...
bool MultirotorApiBase::takeoff(float timeout_sec) {
    ...
    if (kinematics.twist.linear.norm() > approx_zero_vel_) {
        throw VehicleMoveException(
            Utils::stringf("Cannot perform takeoff because vehicle is already moving with velocity %f m/s",
                           kinematics.twist.linear.norm()));
    }

    bool ret = moveToPosition(kinematics.pose.position.x(), kinematics.pose.position.y(),
                              kinematics.pose.position.z() + getTakeoffZ(), 0.5f, timeout_sec,
                              DrivetrainType::MaxDegreeOfFreedom, YawMode::Zero(), -1, 1);
...
bool MultirotorApiBase::moveByVelocityBodyFrame(float vx, float vy, float vz, float duration, DrivetrainType drivetrain,
...
bool MultirotorApiBase::moveByVelocityZBodyFrame(float vx, float vy, float z, float duration, DrivetrainType drivetrain,
                                                 const YawMode &yaw_mode) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByMotorPWMs(float front_right_pwm, float rear_left_pwm, float front_left_pwm,
...
bool MultirotorApiBase::moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle,
                                                       float duration) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle,
                                                 float duration) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByVelocity(float vx, float vy, float vz, float duration, DrivetrainType drivetrain,
                                       const YawMode &yaw_mode) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain,
                                        const YawMode &yaw_mode) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveOnPath(const vector<Vector3r> &path, float velocity, float timeout_sec,
                                   DrivetrainType drivetrain, const YawMode &yaw_mode, float lookahead,
    ...
    if (lookahead == 0) {
        ...
        if (command_period_dist > lookahead) {
            throw std::invalid_argument(
                Utils::stringf("lookahead value %f is too small for velocity %f. It must be at least %f", lookahead,
                               velocity, command_period_dist));
        }
        if (getDistanceAccuracy() > lookahead) {
            throw std::invalid_argument(Utils::stringf(
                "lookahead value %f is smaller than drone's distance accuracy %f.", lookahead, getDistanceAccuracy()));
        }
    } else {
    ...
    if (velocity > getMultirotorApiParams().breaking_vel) {
        breaking_dist =
            Utils::clip(velocity * getMultirotorApiParams().vel_to_breaking_dist,
    ...
    while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 ||
                                   goal_dist > 0)) { // current position is approximately at the last end point
        ...
        moveToPathPosition(next_path_loc.position, seg_velocity, drivetrain, yaw_mode,
            ...
            if (adaptive_lookahead) {
                ...
                if (error > lookahead_error) {
                    ...
                    if (lookahead_error_increasing > 1E5) {
                        throw std::runtime_error("lookahead error is continually increasing so we do not have safe "
                                                 "control, aborting moveOnPath operation");
                    }
                } else {
...
bool MultirotorApiBase::moveToGPS(float latitude, float longitude, float altitude, float velocity, float timeout_sec,
                                  DrivetrainType drivetrain, const YawMode &yaw_mode, float lookahead,
                                  float adaptive_lookahead) {
    SingleTaskCall lock(this);
    GeoPoint target;
    ...
    if (!std::isnan(getHomeGeoPoint().latitude) && !std::isnan(getHomeGeoPoint().longitude) &&
        !std::isnan(getHomeGeoPoint().altitude)) {
        vector<Vector3r> path{nervosys::autonomylib::EarthUtils::GeodeticToNed(target, getHomeGeoPoint())};
...
bool MultirotorApiBase::moveToPosition(float x, float y, float z, float velocity, float timeout_sec,
                                       DrivetrainType drivetrain, const YawMode &yaw_mode, float lookahead,
                                       float adaptive_lookahead) {
    SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveToZ(float z, float velocity, float timeout_sec, const YawMode &yaw_mode, float lookahead,
                                float adaptive_lookahead) {
    SingleTaskCall lock(this);
    Vector2r cur_xy(getPosition().x(), getPosition().y());
    vector<Vector3r> path{Vector3r(cur_xy.x(), cur_xy.y(), z)};
    return moveOnPath(path, velocity, timeout_sec, DrivetrainType::MaxDegreeOfFreedom, yaw_mode, lookahead,
                      adaptive_lookahead);
}
...
bool MultirotorApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain,
    ...
    const float kMaxRCValue = 10000.0f;
    if (duration <= 0) {
        return true;
    }
    ...
    do {
        ...
        if (rc_data.is_valid &&
            (rc_data.timestamp == 0 || age <= kMaxMessageAge)) { // if rc message timestamp is not set OR is not too old
...
void MultirotorApiBase::setAngleLevelControllerGains(const vector<float> &kp, const vector<float> &ki,
                                                     const vector<float> &kd) {
    uint8_t controller_type = 2;
    setControllerGains(controller_type, kp, ki, kd);
}

void MultirotorApiBase::setAngleRateControllerGains(const vector<float> &kp, const vector<float> &ki,
                                                    const vector<float> &kd) {
    uint8_t controller_type = 3;
    setControllerGains(controller_type, kp, ki, kd);
}

void MultirotorApiBase::setVelocityControllerGains(const vector<float> &kp, const vector<float> &ki,
                                                   const vector<float> &kd) {
    uint8_t controller_type = 4;
    setControllerGains(controller_type, kp, ki, kd);
}

void MultirotorApiBase::setPositionControllerGains(const vector<float> &kp, const vector<float> &ki,
                                                   const vector<float> &kd) {
...
void MultirotorApiBase::moveByRollPitchYawrateThrottleInternal(float roll, float pitch, float yaw_rate,
                                                               float throttle) {
    if (safetyCheckVelocity(getVelocity()))
...
void MultirotorApiBase::moveByAngleRatesThrottleInternal(float roll_rate, float pitch_rate, float yaw_rate,
                                                         float throttle) {
    if (safetyCheckVelocity(getVelocity()))
...
void MultirotorApiBase::moveToPathPosition(const Vector3r &dest, float velocity, DrivetrainType drivetrain,
    ...
    if (!result.is_safe) {
        if (result.reason == SafetyEval::SafetyViolationType_::Obstacle) {
            ...
            if (safety_eval_ptr_->getObsAvoidanceStrategy() != SafetyEval::ObsAvoidanceStrategy::RaiseException) {
                ...
                Vector3r avoidance_vel =
                    getObsAvoidanceVelocity(result.cur_risk_dist, obs_avoidance_vel_) * result.suggested_vec;
...
float MultirotorApiBase::setNextPathPosition(const vector<Vector3r> &path, const vector<PathSegment> &path_segs,
    ...
    while (i < path.size() - 1) {
        ...
        // protect against duplicate points in path, normalized seg will have NaN
        ...
        if (&cur_path_loc == &next_path_loc) {
            Utils::log(
                Utils::stringf("segment %d done: x=%f, y=%f, z=%f", i, path.at(i).x(), path.at(i).y(), path.at(i).z()));
        }
        ++i;
    }
...
void MultirotorApiBase::adjustYaw(const Vector3r &heading, DrivetrainType drivetrain, YawMode &yaw_mode) {
    ...
    if (drivetrain == DrivetrainType::ForwardOnly && !yaw_mode.is_rate) {
        if (heading.norm() > getDistanceAccuracy()) {
            ...
            yaw_mode.setZeroRate(); // don't change existing yaw if heading is too small because that can generate
                                    // random result
    }
...
float MultirotorApiBase::getAutoLookahead(float velocity, float adaptive_lookahead, float max_factor,
