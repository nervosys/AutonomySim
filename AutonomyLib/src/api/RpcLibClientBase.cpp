...
namespace autonomylib {
    ...
    bool RpcLibClientBase::ping() { return pimpl_->client.call("ping").as<bool>(); }
    ...
    int RpcLibClientBase::getServerVersion() const { return pimpl_->client.call("getServerVersion").as<int>(); }
    ...
    void RpcLibClientBase::confirmConnection() {
        ...
        std::string ver_info = Utils::stringf("Client Ver:%i (Min Req:%i), Server Ver:%i (Min Req:%i)", client_ver,
                                              client_min_ver, server_ver, server_min_ver);

        if (server_ver < server_min_ver) {
            std::cerr << std::endl << ver_info << std::endl;
            std::cerr << std::endl
                      << "AutonomySim server is of older version and not supported by this client. Please upgrade!"
                      << std::endl;
        } else if (client_ver < client_min_ver) {
            std::cerr << std::endl << ver_info << std::endl;
            std::cerr << std::endl
                      << "AutonomySim client is of older version and not supported by this server. Please upgrade!"
                      << std::endl;
        } else
    ...
    nervosys::autonomylib::GeoPoint RpcLibClientBase::getHomeGeoPoint(const std::string &vehicle_name) const {
        return pimpl_->client.call("getHomeGeoPoint", vehicle_name).as<RpcLibAdaptorsBase::GeoPoint>().to();
    }

    nervosys::autonomylib::LidarData RpcLibClientBase::getLidarData(const std::string &lidar_name,
    ...
    nervosys::autonomylib::ImuBase::Output RpcLibClientBase::getImuData(const std::string &imu_name,
                                                                        const std::string &vehicle_name) const {
        return pimpl_->client.call("getImuData", imu_name, vehicle_name).as<RpcLibAdaptorsBase::ImuData>().to();
    }

    nervosys::autonomylib::BarometerBase::Output
    RpcLibClientBase::getBarometerData(const std::string &barometer_name, const std::string &vehicle_name) const {
        return pimpl_->client.call("getBarometerData", barometer_name, vehicle_name)
            .as<RpcLibAdaptorsBase::BarometerData>()
            .to();
    }

    nervosys::autonomylib::MagnetometerBase::Output
    RpcLibClientBase::getMagnetometerData(const std::string &magnetometer_name, const std::string &vehicle_name) const {
        return pimpl_->client.call("getMagnetometerData", magnetometer_name, vehicle_name)
            .as<RpcLibAdaptorsBase::MagnetometerData>()
            .to();
    }

    nervosys::autonomylib::GpsBase::Output RpcLibClientBase::getGpsData(const std::string &gps_name,
                                                                        const std::string &vehicle_name) const {
        return pimpl_->client.call("getGpsData", gps_name, vehicle_name).as<RpcLibAdaptorsBase::GpsData>().to();
    }

    nervosys::autonomylib::DistanceSensorData
    RpcLibClientBase::getDistanceSensorData(const std::string &distance_sensor_name,
                                            const std::string &vehicle_name) const {
        return pimpl_->client.call("getDistanceSensorData", distance_sensor_name, vehicle_name)
    ...
    void RpcLibClientBase::simAddDetectionFilterMeshName(const std::string &camera_name,
                                                         ImageCaptureBase::ImageType type, const std::string &mesh_name,
    ...
    void RpcLibClientBase::simSetDetectionFilterRadius(const std::string &camera_name, ImageCaptureBase::ImageType type,
                                                       const float radius_cm, const std::string &vehicle_name,
                                                       bool external) {
        pimpl_->client.call("simSetDetectionFilterRadius", camera_name, type, radius_cm, vehicle_name, external);
    }

    void RpcLibClientBase::simClearDetectionMeshNames(const std::string &camera_name, ImageCaptureBase::ImageType type,
                                                      const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simClearDetectionMeshNames", camera_name, type, vehicle_name, external);
    }

    vector<DetectionInfo> RpcLibClientBase::simGetDetections(const std::string &camera_name,
                                                             ImageCaptureBase::ImageType image_type,
                                                             const std::string &vehicle_name, bool external) {
        const auto &result = pimpl_->client.call("simGetDetections", camera_name, image_type, vehicle_name, external)
                                 .as<vector<RpcLibAdaptorsBase::DetectionInfo>>();
    ...
    void RpcLibClientBase::simSetKinematics(const Kinematics::State &state, bool ignore_collision,
                                            const std::string &vehicle_name) {
        pimpl_->client.call("simSetKinematics", RpcLibAdaptorsBase::KinematicsState(state), ignore_collision,
                            vehicle_name);
    }

    void RpcLibClientBase::simSetTraceLine(const std::vector<float> &color_rgba, float thickness,
                                           const std::string &vehicle_name) {
        pimpl_->client.call("simSetTraceLine", color_rgba, thickness, vehicle_name);
    }

    vector<ImageCaptureBase::ImageResponse>
    RpcLibClientBase::simGetImages(vector<ImageCaptureBase::ImageRequest> request, const std::string &vehicle_name,
                                   bool external) {
        const auto &response_adaptor =
            pimpl_->client.call("simGetImages", RpcLibAdaptorsBase::ImageRequest::from(request), vehicle_name, external)
    ...
    vector<uint8_t> RpcLibClientBase::simGetImage(const std::string &camera_name, ImageCaptureBase::ImageType type,
        ...
        vector<uint8_t> result =
            pimpl_->client.call("simGetImage", camera_name, type, vehicle_name, external).as<vector<uint8_t>>();
        return result;
    }

    // CinemAutonomySim
    std::vector<std::string> RpcLibClientBase::simGetPresetLensSettings(const std::string &camera_name,
                                                                        const std::string &vehicle_name,
                                                                        bool external) {
        return pimpl_->client.call("simGetPresetLensSettings", camera_name, vehicle_name, external)
    ...
    void RpcLibClientBase::simSetPresetLensSettings(const std::string &preset_lens_settings,
    ...
    std::vector<std::string> RpcLibClientBase::simGetPresetFilmbackSettings(const std::string &camera_name,
                                                                            const std::string &vehicle_name,
                                                                            bool external) {
        return pimpl_->client.call("simGetPresetFilmbackSettings", camera_name, vehicle_name, external)
            .as<vector<std::string>>();
    }

    void RpcLibClientBase::simSetPresetFilmbackSettings(const std::string &preset_filmback_settings,
                                                        const std::string &camera_name, const std::string &vehicle_name,
                                                        bool external) {
    ...
    std::string RpcLibClientBase::simGetFilmbackSettings(const std::string &camera_name,
                                                         const std::string &vehicle_name, bool external) {
        return pimpl_->client.call("simGetFilmbackSettings", camera_name, vehicle_name, external).as<std::string>();
    }

    float RpcLibClientBase::simSetFilmbackSettings(const float sensor_width, const float sensor_height,
                                                   const std::string &camera_name, const std::string &vehicle_name,
                                                   bool external) {
        return pimpl_->client
            .call("simSetFilmbackSettings", sensor_width, sensor_height, camera_name, vehicle_name, external)
            .as<float>();
    }
    ...
    void RpcLibClientBase::simSetFocalLength(const float focal_length, const std::string &camera_name,
                                             const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simSetFocalLength", focal_length, camera_name, vehicle_name, external);
    }

    void RpcLibClientBase::simEnableManualFocus(const bool enable, const std::string &camera_name,
                                                const std::string &vehicle_name, bool external) {
    ...
    void RpcLibClientBase::simSetFocusDistance(const float focus_distance, const std::string &camera_name,
                                               const std::string &vehicle_name, bool external) {
    ...
    void RpcLibClientBase::simSetFocusAperture(const float focus_aperture, const std::string &camera_name,
                                               const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simSetFocusAperture", focus_aperture, camera_name, vehicle_name, external);
    }

    void RpcLibClientBase::simEnableFocusPlane(const bool enable, const std::string &camera_name,
                                               const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simEnableFocusPlane", enable, camera_name, vehicle_name, external);
    }

    std::string RpcLibClientBase::simGetCurrentFieldOfView(const std::string &camera_name,
                                                           const std::string &vehicle_name, bool external) {
    ...
    bool RpcLibClientBase::simTestLineOfSightToPoint(const nervosys::autonomylib::GeoPoint &point,
                                                     const std::string &vehicle_name) {
        return pimpl_->client.call("simTestLineOfSightToPoint", RpcLibAdaptorsBase::GeoPoint(point), vehicle_name)
            .as<bool>();
    }

    bool RpcLibClientBase::simTestLineOfSightBetweenPoints(const nervosys::autonomylib::GeoPoint &point1,
                                                           const nervosys::autonomylib::GeoPoint &point2) {
        return pimpl_->client
            .call("simTestLineOfSightBetweenPoints", RpcLibAdaptorsBase::GeoPoint(point1),
                  RpcLibAdaptorsBase::GeoPoint(point2))
            .as<bool>();
    }

    vector<nervosys::autonomylib::GeoPoint> RpcLibClientBase::simGetWorldExtents() {
        vector<RpcLibAdaptorsBase::GeoPoint> raw_result =
            pimpl_->client.call("simGetWorldExtents").as<vector<RpcLibAdaptorsBase::GeoPoint>>();
        vector<nervosys::autonomylib::GeoPoint> final_result;
    ...
    vector<MeshPositionVertexBuffersResponse> RpcLibClientBase::simGetMeshPositionVertexBuffers() {
        const auto &response_adaptor = pimpl_->client.call("simGetMeshPositionVertexBuffers")
                                           .as<vector<RpcLibAdaptorsBase::MeshPositionVertexBuffersResponse>>();
        return RpcLibAdaptorsBase::MeshPositionVertexBuffersResponse::to(response_adaptor);
    }

    bool RpcLibClientBase::simAddVehicle(const std::string &vehicle_name, const std::string &vehicle_type,
                                         const Pose &pose, const std::string &pawn_path) {
        return pimpl_->client
            .call("simAddVehicle", vehicle_name, vehicle_type, RpcLibAdaptorsBase::Pose(pose), pawn_path)
            .as<bool>();
    }

    void RpcLibClientBase::simPrintLogMessage(const std::string &message, std::string message_param,
                                              unsigned char severity) {
        pimpl_->client.call("simPrintLogMessage", message, message_param, severity);
    }

    void RpcLibClientBase::simFlushPersistentMarkers() { pimpl_->client.call("simFlushPersistentMarkers"); }

    void RpcLibClientBase::simPlotPoints(const vector<Vector3r> &points, const vector<float> &color_rgba, float size,
    ...
    void RpcLibClientBase::simPlotLineStrip(const vector<Vector3r> &points, const vector<float> &color_rgba,
    ...
    void RpcLibClientBase::simPlotLineList(const vector<Vector3r> &points, const vector<float> &color_rgba,
                                           float thickness, float duration, bool is_persistent) {
        vector<RpcLibAdaptorsBase::Vector3r> conv_points;
        RpcLibAdaptorsBase::from(points, conv_points);
    ...
    void RpcLibClientBase::simPlotArrows(const vector<Vector3r> &points_start, const vector<Vector3r> &points_end,
        ...
        pimpl_->client.call("simPlotArrows", conv_points_start, conv_points_end, color_rgba, thickness, arrow_size,
                            duration, is_persistent);
    }

    void RpcLibClientBase::simPlotStrings(const vector<std::string> &strings, const vector<Vector3r> &positions,
                                          float scale, const vector<float> &color_rgba, float duration) {
    ...
    void RpcLibClientBase::simPlotTransforms(const vector<Pose> &poses, float scale, float thickness, float duration,
                                             bool is_persistent) {
    ...
    void RpcLibClientBase::simPlotTransformsWithNames(const vector<Pose> &poses, const vector<std::string> &names,
                                                      float tf_scale, float tf_thickness, float text_scale,
                                                      const vector<float> &text_color_rgba, float duration) {
        vector<RpcLibAdaptorsBase::Pose> conv_poses;
        RpcLibAdaptorsBase::from(poses, conv_poses);
        pimpl_->client.call("simPlotTransformsWithNames", conv_poses, names, tf_scale, tf_thickness, text_scale,
                            text_color_rgba, duration);
    }

    bool RpcLibClientBase::simIsPaused() const { return pimpl_->client.call("simIsPaused").as<bool>(); }

    void RpcLibClientBase::simPause(bool is_paused) { pimpl_->client.call("simPause", is_paused); }

    void RpcLibClientBase::simContinueForTime(double seconds) { pimpl_->client.call("simContinueForTime", seconds); }

    void RpcLibClientBase::simContinueForFrames(uint32_t frames) {
        pimpl_->client.call("simContinueForFrames", frames);
    }

    void RpcLibClientBase::simEnableWeather(bool enable) { pimpl_->client.call("simEnableWeather", enable); }
    ...
    void RpcLibClientBase::simSetTimeOfDay(bool is_enabled, const string &start_datetime, bool is_start_datetime_dst,
        ...
        pimpl_->client.call("simSetTimeOfDay", is_enabled, start_datetime, is_start_datetime_dst, celestial_clock_speed,
    ...
    std::vector<std::string> RpcLibClientBase::simSwapTextures(const std::string &tags, int tex_id, int component_id,
                                                               int material_id) {
    ...
    bool RpcLibClientBase::simSetObjectMaterialFromTexture(const std::string &object_name,
                                                           const std::string &texture_path, const int component_id) {
        return pimpl_->client.call("simSetObjectMaterialFromTexture", object_name, texture_path, component_id)
            .as<bool>();
    }
    ...
    std::string RpcLibClientBase::simSpawnObject(const std::string &object_name, const std::string &load_component,
                                                 const Pose &pose, const Vector3r &scale, bool physics_enabled) {
        return pimpl_->client
            .call("simSpawnObject", object_name, load_component, RpcLibAdaptorsBase::Pose(pose),
                  RpcLibAdaptorsBase::Vector3r(scale), physics_enabled)
            .as<std::string>();
    }
    ...
    nervosys::autonomylib::Vector3r RpcLibClientBase::simGetObjectScale(const std::string &object_name) const {
        return pimpl_->client.call("simGetObjectScale", object_name).as<RpcLibAdaptorsBase::Vector3r>().to();
    }

    nervosys::autonomylib::Pose RpcLibClientBase::simGetObjectPose(const std::string &object_name) const {
        return pimpl_->client.call("simGetObjectPose", object_name).as<RpcLibAdaptorsBase::Pose>().to();
    }

    bool RpcLibClientBase::simSetObjectPose(const std::string &object_name, const nervosys::autonomylib::Pose &pose,
                                            bool teleport) {
        return pimpl_->client.call("simSetObjectPose", object_name, RpcLibAdaptorsBase::Pose(pose), teleport)
            .as<bool>();
    }

    bool RpcLibClientBase::simSetObjectScale(const std::string &object_name,
                                             const nervosys::autonomylib::Vector3r &scale) {
        return pimpl_->client.call("simSetObjectScale", object_name, RpcLibAdaptorsBase::Vector3r(scale)).as<bool>();
    }

    CameraInfo RpcLibClientBase::simGetCameraInfo(const std::string &camera_name, const std::string &vehicle_name,
                                                  bool external) const {
        return pimpl_->client.call("simGetCameraInfo", camera_name, vehicle_name, external)
            .as<RpcLibAdaptorsBase::CameraInfo>()
            .to();
    }

    void RpcLibClientBase::simSetCameraPose(const std::string &camera_name, const Pose &pose,
                                            const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simSetCameraPose", camera_name, RpcLibAdaptorsBase::Pose(pose), vehicle_name, external);
    }

    void RpcLibClientBase::simSetCameraFov(const std::string &camera_name, float fov_degrees,
                                           const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simSetCameraFov", camera_name, fov_degrees, vehicle_name, external);
    }

    void RpcLibClientBase::simSetDistortionParam(const std::string &camera_name, const std::string &param_name,
                                                 float value, const std::string &vehicle_name, bool external) {
        pimpl_->client.call("simSetDistortionParam", camera_name, param_name, value, vehicle_name, external);
    }

    std::vector<float> RpcLibClientBase::simGetDistortionParams(const std::string &camera_name,
                                                                const std::string &vehicle_name, bool external) {
        return pimpl_->client.call("simGetDistortionParams", camera_name, vehicle_name, external)
            .as<std::vector<float>>();
    }

    nervosys::autonomylib::Kinematics::State
    RpcLibClientBase::simGetGroundTruthKinematics(const std::string &vehicle_name) const {
        return pimpl_->client.call("simGetGroundTruthKinematics", vehicle_name)
            .as<RpcLibAdaptorsBase::KinematicsState>()
            .to();
    }

    nervosys::autonomylib::Environment::State
    RpcLibClientBase::simGetGroundTruthEnvironment(const std::string &vehicle_name) const {
        return pimpl_->client.call("simGetGroundTruthEnvironment", vehicle_name)
            .as<RpcLibAdaptorsBase::EnvironmentState>()
            .to();
    }

    bool RpcLibClientBase::simCreateVoxelGrid(const nervosys::autonomylib::Vector3r &position, const int &x,
                                              const int &y, const int &z, const float &res,
                                              const std::string &output_file) {
        return pimpl_->client
            .call("simCreateVoxelGrid", RpcLibAdaptorsBase::Vector3r(position), x, y, z, res, output_file)
            .as<bool>();
    }
    ...
    void RpcLibClientBase::startRecording() { pimpl_->client.call("startRecording"); }

    void RpcLibClientBase::stopRecording() { pimpl_->client.call("stopRecording"); }

    bool RpcLibClientBase::isRecording() { return pimpl_->client.call("isRecording").as<bool>(); }
    ...
    vector<string> RpcLibClientBase::listVehicles() { return pimpl_->client.call("listVehicles").as<vector<string>>(); }
