...
SafetyEval::SafetyEval(MultirotorApiParams vehicle_params, shared_ptr<IGeoFence> fence_ptr,
                       shared_ptr<ObstacleMap> obs_xy_ptr)
...
void SafetyEval::checkFence(const Vector3r &dest_pos, const Vector3r &cur_pos, SafetyEval::EvalResult &appendToResult) {
    ...
    if (!allow) {
        ...
        appendToResult.message.append(common_utils::Utils::stringf(
            "Destination %s is outside geo fence and is worse off than current %s in geofence=[%s]",
            VectorMath::toString(dest_pos).c_str(), VectorMath::toString(cur_pos).c_str(),
            fence_ptr_->toString().c_str()));
    }
}

SafetyEval::EvalResult SafetyEval::isSafeDestination(const Vector3r &dest_pos, const Vector3r &cur_pos,
                                                     const Quaternionr &quaternion) {
...
void SafetyEval::isCurrentSafer(SafetyEval::EvalResult &result) {
    ...
    result.cur_risk_dist =
        adjustClearanceForPrStl(vehicle_params_.obs_clearance, result.cur_obs.confidence) - result.cur_obs.distance;

    if (!isThisRiskDistLess(result.cur_risk_dist, result.dest_risk_dist)) {
        result.is_safe = false;
        result.reason |= SafetyViolationType_::Obstacle;
        result.message.append(common_utils::Utils::stringf("Current position is safer"));
    }
    // else we are better of moving to dest
}

void SafetyEval::isSafeDestination(const Vector3r &dest_pos, const Vector3r &cur_pos, const Quaternionr &quaternion,
                                   SafetyEval::EvalResult &result) {
    ...
    if (cur_dest_norm < vehicle_params_.distance_accuracy) {
        ...
        result.dest_obs =
            obs_xy_ptr_->hasObstacle(point_tick - vehicle_params_.obs_window, point_tick + vehicle_params_.obs_window);

        // less risk distance is better
        result.dest_risk_dist = cur_dest_norm +
                                adjustClearanceForPrStl(vehicle_params_.obs_clearance, result.dest_obs.confidence) -
                                result.dest_obs.distance;
    ...
    if (!result.is_safe && result.reason & SafetyViolationType_::Obstacle) {
        ...
        Utils::log(Utils::stringf("isSafeDestination: cur_dest_norm=%f, result=%s, quaternion=%s", cur_dest_norm,
                                  result.toString().c_str(), VectorMath::toString(quaternion, true).c_str()));
    }
...
void SafetyEval::setSuggestedVelocity(SafetyEval::EvalResult &result, const Quaternionr &quaternion) {
    ...
    for (int i = 0; i <= ticks / 2; ++i) {
        ...
        float right_risk_dist =
            adjustClearanceForPrStl(vehicle_params_.obs_clearance, right_obs.confidence) - right_obs.distance;
        float left_risk_dist =
            adjustClearanceForPrStl(vehicle_params_.obs_clearance, left_obs.confidence) - left_obs.distance;
        ...
        if (right_risk_dist <= 0 || left_risk_dist <= 0) {
            ...
            const Vector3r suggested_body =
                Vector3r(std::cos(suggested_angle), std::sin(suggested_angle), 0).normalized();
            result.suggested_vec = VectorMath::transformToWorldFrame(suggested_body, quaternion, true);

            Utils::log(Utils::stringf("right_risk_dist=%f, left_risk_dist=%f, suggested_tick=%i, suggested_angle=%f",
                                      right_risk_dist, left_risk_dist, suggested_tick, suggested_angle,
                                      suggested_angle));
...
SafetyEval::EvalResult SafetyEval::isSafeVelocityZ(const Vector3r &cur_pos, float vx, float vy, float z,
                                                   const Quaternionr &quaternion) {
    SafetyEval::EvalResult result;
...
Vector3r SafetyEval::getDestination(const Vector3r &cur_pos, const Vector3r &velocity) const {
    ...
    float dest_pos_dist =
        std::max(velocity_mag * vehicle_params_.vel_to_breaking_dist, vehicle_params_.min_breaking_dist);

    // calculate dest_pos cur_pos we will be if we had to break suddenly
    return velocity_mag >= vehicle_params_.distance_accuracy ? (cur_pos + (velocity / velocity_mag) * dest_pos_dist)
                                                             : cur_pos;
}

SafetyEval::EvalResult SafetyEval::isSafeVelocity(const Vector3r &cur_pos, const Vector3r &velocity,
                                                  const Quaternionr &quaternion) {
    SafetyEval::EvalResult result;
...
void SafetyEval::setSafety(SafetyViolationType enable_reasons, float obs_clearance,
                           SafetyEval::ObsAvoidanceStrategy obs_strategy, const Vector3r &origin, float xy_length,
...
SafetyEval::ObsAvoidanceStrategy SafetyEval::getObsAvoidanceStrategy() { return obs_strategy_; }
