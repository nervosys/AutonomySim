...
#ifndef autonomylib_safety_SafetyEval_hpp
#define autonomylib_safety_SafetyEval_hpp
...
class SafetyEval {
  public:
    ...
    struct EvalResult {
        ...
        string toString() const {
            return Utils::stringf(
                "SafetyEval: is_safe=%i, reason=%X, cur_risk_dist=%f, dest_risk_dist=%f, suggested_vec=%s,"
                ...
                VectorMath::toString(cur_pos).c_str(), VectorMath::toString(dest_pos).c_str(),
                VectorMath::toString(cur_dest_body).c_str(), cur_obs.toString().c_str(), dest_obs.toString().c_str(),
                suggested_obs.toString().c_str(), message.c_str());
        }
    };
  ...
  public:
    ...
    void setSafety(SafetyViolationType enable_reasons, float obs_clearance,
                   SafetyEval::ObsAvoidanceStrategy obs_strategy, const Vector3r &origin, float xy_length, float max_z,
                   float min_z);
