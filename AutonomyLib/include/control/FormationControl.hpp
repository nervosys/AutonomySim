...
#ifndef autonomylib_control_FormationControl_hpp
#define autonomylib_control_FormationControl_hpp
...
public:
    ...
    struct FormationParams {
        ...
        FormationParams()
            ...
            , k_cohesion(0.3f)
            , k_alignment(0.2f)
            , formation_radius(10.0f)
            , formation_angle(static_cast<real_T>(M_PI / 6.0))
        {
        }
    };
    ...
    struct FormationCommand {
        ...
        FormationCommand()
            : desired_velocity(Vector3r::Zero())
            , desired_acceleration(Vector3r::Zero())
...
private:
    ...
    Vector3r computePositionError(
        const Vector3r& current_pos,
        const Vector3r& desired_pos) const;
    
    Vector3r computeVelocityError(
        const Vector3r& current_vel,
        const Vector3r& desired_vel) const;
    
    Vector3r computeSeparationForce(
        const VehicleState& vehicle,
        const std::vector<VehicleState>& neighbors) const;
    
    Vector3r computeCohesionForce(
        const VehicleState& vehicle,
        const std::vector<VehicleState>& neighbors) const;
    
    Vector3r computeAlignmentForce(
        const VehicleState& vehicle,
        const std::vector<VehicleState>& neighbors) const;
...
#endif // autonomylib_control_FormationControl_hpp
