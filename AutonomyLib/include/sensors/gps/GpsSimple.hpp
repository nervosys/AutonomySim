...
#ifndef autonomylib_sensors_GpsSimple_hpp
#define autonomylib_sensors_GpsSimple_hpp
...
class GpsSimple : public GpsBase {
  public: // methods
    GpsSimple(const AutonomySimSettings::GpsSetting &setting = AutonomySimSettings::GpsSetting())
        ...
        // initialize filters
        eph_filter.initialize(
            params_.eph_time_constant, params_.eph_final,
            params_.eph_initial); // starting dilution set to 100 which we will reduce over time to targeted 0.3f, with
                                  // 45% accuracy within 100 updates, each update occurring at 0.2s interval
  ...
  private:
    void addOutputToDelayLine(real_T eph, real_T epv) {
        ...
        output.gnss.fix_type = output.gnss.eph <= params_.eph_min_3d   ? GnssFixType::GNSS_FIX_3D_FIX
