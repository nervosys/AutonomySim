...
#ifndef autonomylib_physics_DebugPhysicsBody_hpp
#define autonomylib_physics_DebugPhysicsBody_hpp
...
class DebugPhysicsBody : public PhysicsBody {
  ...
  private:
    class WrenchVertex : public PhysicsBodyVertex {
      ...
      protected:
        virtual void setWrench(Wrench &wrench) override { wrench.force = force_; }
    ...
    static void createDragVertices(vector<PhysicsBodyVertex> &drag_vertices, real_T drag_coefficient, real_T body_x,
                                   real_T body_y, real_T body_z) {
        real_T top_bottom_area = body_x * body_y;
        real_T left_right_area = body_x * body_z;
        real_T front_back_area = body_y * body_z;
        Vector3r drag_factor_unit = Vector3r(front_back_area, left_right_area, top_bottom_area) * drag_coefficient /
                                    2; // Vector3r drag_factor_unit = Vector3r::Zero();
    ...
    static void createWrenchVertices(vector<WrenchVertex> &wrench_vertices, real_T body_x, real_T body_y, real_T body_z,
                                     real_T mass) {
  ...
  public:
    ...
    virtual void updateKinematics() override { PhysicsBody::updateKinematics(); }

    virtual real_T getRestitution() const override { return restitution_; }

    virtual real_T getFriction() const override { return friction_; }

    virtual uint wrenchVertexCount() const override { return static_cast<uint>(wrench_vertices_.size()); }
    virtual PhysicsBodyVertex &getWrenchVertex(uint index) override { return wrench_vertices_.at(index); }
    virtual const PhysicsBodyVertex &getWrenchVertex(uint index) const override { return wrench_vertices_.at(index); }
