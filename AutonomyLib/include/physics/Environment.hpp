...
#ifndef autonomylib_physics_Environment_hpp
#define autonomylib_physics_Environment_hpp
...
class Environment : public UpdatableObject {
  ...
  public:
    ...
    Environment(const State &initial) { initialize(initial); }
    ...
    GeoPoint getHomeGeoPoint() const { return home_geo_point_.home_geo_point; }

    // in local NED coordinates
    void setPosition(const Vector3r &position) { current_.position = position; }
