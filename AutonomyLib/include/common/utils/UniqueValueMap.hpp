...
template <class TKey, class TVal> class UniqueValueMap {
  public:
    ...
    typename std::map<TKey, TVal>::const_iterator find(const TKey &key) const { return map_.find(key); }

    const std::map<TKey, TVal> &getMap() const { return map_; }

    typename std::set<TVal>::const_iterator begin() const { return vals_.begin(); }

    typename std::set<TVal>::const_iterator end() const { return vals_.end(); }

    const TVal &at(const TKey &key) const { return map_.at(key); }
    ...
    size_t valsSize() const { return vals_.size(); }
