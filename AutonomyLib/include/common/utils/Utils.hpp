...
template <class T> inline void unused(T const &result) { static_cast<void>(result); }
...
class Utils {
  ...
  public:
    ...
    template <typename T> static T getRandomFromGaussian(T stddev = 1, T mean = 0) {
    ...
    static constexpr double degreesToRadians(double degrees) { return static_cast<double>(M_PIl * degrees / 180.0); }
    static constexpr float degreesToRadians(float degrees) { return static_cast<float>(M_PI * degrees / 180.0f); }
    static constexpr double radiansToDegrees(double radians) { return static_cast<double>(radians * 180.0 / M_PIl); }
    static constexpr float radiansToDegrees(float radians) { return static_cast<float>(radians * 180.0f / M_PI); }
    ...
    template <typename T> static T limitAbsValue(T val, T min_value, T max_value) {
    ...
    /// Limits absolute value whole preserving sign
    template <typename T> static T clip(T val, T min_value, T max_value) {
        return std::max(min_value, std::min(val, max_value));
    }

    template <typename Range>
    static const string printRange(Range &&range, const string &delim = ", ", const string &prefix = "(",
                                   const string &suffix = ")") {
    ...
    static inline int floorToInt(float x) { return static_cast<int>(std::floor(x)); }

    template <typename T> static constexpr T nan() { return std::numeric_limits<T>::quiet_NaN(); }
    template <typename T> static constexpr T max() { return std::numeric_limits<T>::max(); }
    template <typename T> static constexpr T min() { return std::numeric_limits<T>::min(); }

    template <typename T> static void setValue(T arr[], size_t length, const T &val) {
        std::fill(arr, arr + length, val);
    }

    template <typename T, size_t N> static void setValue(T (&arr)[N], const T &val) { std::fill(arr, arr + N, val); }

    template <class T, size_t N> static std::size_t length(const T (&)[N]) { return N; }
    ...
    static const char *to_string(time_point<steady_clock> t) {
        time_t tt = system_clock::to_time_t(
            std::chrono::time_point_cast<system_clock::duration>(system_clock::now() + (t - steady_clock::now())));
        return ctime(&tt);
    }

    static time_point<system_clock> now() { return system_clock::now(); }

    static std::time_t to_time_t(const std::string &str, bool is_dst = false,
    ...
    template <typename T> static void clear(std::queue<T> &q, size_t max_elements = SIZE_MAX) {
    ...
    template <typename T> static const std::vector<T> &emptyVector() {
    ...
    static constexpr float kelvinToCelcius(float kelvin) { return kelvin - 273.15f; }
    static constexpr float celciusToKelvin(float celcius) { return celcius + 273.15f; }

    template <typename TReal> static constexpr TReal epsilon() { return std::numeric_limits<TReal>::epsilon(); }

    // implements relative method - do not use for comparing with zero
    // use this most of the time, tolerance needs to be meaningful in your context
    template <typename TReal> static bool isApproximatelyEqual(TReal a, TReal b, TReal tolerance = epsilon<TReal>()) {
    ...
    template <typename TReal> static bool isApproximatelyZero(TReal a, TReal tolerance = epsilon<TReal>()) {
    ...
    template <typename TReal> static bool isDefinitelyLessThan(TReal a, TReal b, TReal tolerance = epsilon<TReal>()) {
        TReal diff = a - b;
        if (diff < tolerance)
            return true;
    ...
    template <typename TReal> static bool isWithinPrecisionInterval(TReal a, TReal b, unsigned int interval_size = 1) {
...
// TODO: Use GCC and Clang version from https://github.com/scottt/debugbreak
    ...
    static void writePFMfile(const float *const image_data, int width, int height, const std::string &path,
                             float scalef = 1) {
