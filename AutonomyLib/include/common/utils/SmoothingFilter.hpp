...
template <typename T> void SmoothingFilter<T>::initialize(int window_size, float outlier_factor) {
...
template <typename T> SmoothingFilter<T>::SmoothingFilter() {
    initialize(static_cast<int>(buffer_.capacity()), std::numeric_limits<float>::infinity());
}

template <typename T> SmoothingFilter<T>::SmoothingFilter(int window_size, float outlier_factor) {
    initialize(window_size, std::numeric_limits<float>::infinity());
}

template <typename T> std::tuple<double, double> SmoothingFilter<T>::filter(T value) {
