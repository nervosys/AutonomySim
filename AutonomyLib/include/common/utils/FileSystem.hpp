...
// This defines a default folder name for all the files created by AutonomyLib so they
// are all gathered nicely in one place in the user's documents folder.
#ifndef ProductFolderName
#define ProductFolderName "AutonomySim"
#endif
...
// Windows uses USERPROFILE, Linux uses HOME
    ...
    static std::string getAppDataFolder() { return ensureFolder(combine(getUserDocumentsFolder(), ProductFolderName)); }
    ...
    static std::string getLogFileNamePath(const std::string &fullPath, const std::string &prefix,
        ...
        std::string filename;
        filename.append(ensureFolder(fullPath)).push_back(kPathSeparator);
