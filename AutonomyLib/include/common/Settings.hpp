...
class Settings {
  ...
  public:
    ...
    bool isLoadSuccess() { return load_success_; }

    bool hasFileName() { return !getFullFilePath().empty(); }
    ...
    bool getChild(size_t index, Settings &child) const {
        if (doc_.size() > index && (doc_[index].type() == nlohmann::detail::value_t::object ||
    ...
    bool hasKey(const std::string &key) const { return doc_.find(key) != doc_.end(); }
    ...
    bool setDouble(const std::string &name, double value) {
        if (doc_.count(name) != 1 || doc_[name].type() != nlohmann::detail::value_t::number_float ||
    ...
    bool setBool(const std::string &name, bool value) {
        if (doc_.count(name) != 1 || doc_[name].type() != nlohmann::detail::value_t::boolean ||
    ...
    bool setInt(const std::string &name, int value) {
        if (doc_.count(name) != 1 || doc_[name].type() != nlohmann::detail::value_t::number_integer ||
    ...
    void setChild(const std::string &name, Settings &value) { doc_[name] = value.doc_; }
};
