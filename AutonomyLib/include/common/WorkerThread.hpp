...
#ifndef autonomylib_common_WorkerThread_hpp
...
class CancelableAction : public CancelToken {
  ...
  public:
    CancelableAction() : is_complete_(false) {}
    virtual ~CancelableAction();

    void reset() { is_complete_ = false; }
...
class WorkerThreadSignal {
  ...
  public:
    ...
    template <class _Predicate> void wait(_Predicate cancel) {
        // wait for signal or cancel predicate
        while (!signaled_) {
            std::unique_lock<std::mutex> lock(mutex_);
            cv_.wait_for(lock, std::chrono::milliseconds(1), [this, cancel] { return cancel(); });
        }
        signaled_ = false;
    }
