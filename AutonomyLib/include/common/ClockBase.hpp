...
#ifndef autonomylib_common_ClockBase_hpp
...
class ClockBase {
  ...
  public:
    ...
    ClockBase() { wall_clock_start_ = Utils::getTimeSinceEpochNanos(); }

    virtual ~ClockBase() = default;

    TTimeDelta elapsedSince(TTimePoint since) const { return elapsedBetween(nowNanos(), since); }
    static TTimeDelta elapsedBetween(TTimePoint second, TTimePoint first) { return (second - first) / 1.0E9; }
    TTimePoint addTo(TTimePoint t, TTimeDelta dt) { return static_cast<TTimePoint>(t + dt * 1.0E9); }
