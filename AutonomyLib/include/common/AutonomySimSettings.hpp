...
struct AutonomySimSettings {
  ...
  public: // methods
    ...
    static void createDefaultSettingsFile() {
        ...
        settings_json.setString("SeeDocsAt", "https://github.com/nervosys/AutonomySim/blob/main/docs/settings.md");
  ...
  private:
        ...
        if (upgrade_required) {
            ...
            if (has_default_settings) {
                ...
                if (settings_version_actual == 1) {
                    const std::vector<std::string> all_changed_keys = {"AdditionalCameras", "CaptureSettings",
                                                                       "NoiseSettings",     "UsageScenario",
                                                                       "SimpleFlight",      "PX4"};
                    ...
                    if (detected_keys.length()) {
                        std::string error_message =
                            "You are using newer version of AutonomySim with older version of settings.json. "
                            "You can either delete your settings.json and restart AutonomySim or use the upgrade "
            ...
            if (auto_upgrade) {
                warning_messages.push_back(
                    "You are using newer version of AutonomySim with older version of settings.json. "
                    "You should delete your settings.json file and restart AutonomySim.");
            }
        }
    ...
    static void loadRCSetting(const std::string &simmode_name, const Settings &settings_json, RCSettings &rc_setting) {
        ...
        if (settings_json.getChild("RC", rc_json)) {
            rc_setting.remote_control_id =
                rc_json.getInt("RemoteControlID", simmode_name == kSimModeTypeMultirotor ? 0 : -1);
            rc_setting.allow_api_when_disconnected =
                rc_json.getBool("AllowAPIWhenDisconnected", rc_setting.allow_api_when_disconnected);
        }
    }
    ...
    void loadDefaultRecordingSettings() {
        ...
        for (const auto &vehicle : vehicles) {
            recording_setting.requests[vehicle.first].push_back(
                ImageCaptureBase::ImageRequest("", ImageType::Scene, false, true));
        }
    }
    ...
    void loadRecordingSetting(const Settings &settings_json) {
        ...
        if (settings_json.getChild("Recording", recording_json)) {
            ...
            recording_setting.record_interval =
                recording_json.getFloat("RecordInterval", recording_setting.record_interval);
            ...
            if (recording_json.getChild("Cameras", req_cameras_settings)) {
                ...
                for (size_t child_index = 0; child_index < req_cameras_settings.size(); ++child_index) {
                    ...
                    if (req_cameras_settings.getChild(child_index, req_camera_settings)) {
                        ...
                        ImageType image_type = Utils::toEnum<ImageType>(req_camera_settings.getInt("ImageType", 0));
                        ...
                        recording_setting.requests[vehicle_name].push_back(
                            ImageCaptureBase::ImageRequest(camera_name, image_type, pixels_as_float, compress));
                    }
                }
            }
        }
    ...
    static std::unique_ptr<VehicleSetting> createMavLinkVehicleSetting(const Settings &settings_json) {
        ...
        std::unique_ptr<VehicleSetting> vehicle_setting_p =
            std::unique_ptr<VehicleSetting>(new MavLinkVehicleSetting());
        ...
        connection_info.vehicle_sysid =
            static_cast<uint8_t>(settings_json.getInt("VehicleSysID", connection_info.vehicle_sysid));
        connection_info.vehicle_compid = settings_json.getInt("VehicleCompID", connection_info.vehicle_compid);

        connection_info.offboard_sysid =
            static_cast<uint8_t>(settings_json.getInt("OffboardSysID", connection_info.offboard_sysid));
        connection_info.offboard_compid = settings_json.getInt("OffboardCompID", connection_info.offboard_compid);

        connection_info.logviewer_ip_address =
            settings_json.getString("LogViewerHostIp", connection_info.logviewer_ip_address);
        connection_info.logviewer_ip_port = settings_json.getInt("LogViewerPort", connection_info.logviewer_ip_port);
        connection_info.logviewer_ip_sport =
            settings_json.getInt("LogViewerSendPort", connection_info.logviewer_ip_sport);
        ...
        connection_info.control_port_local =
            settings_json.getInt("ControlPort", connection_info.control_port_local); // legacy
        connection_info.control_port_local =
            settings_json.getInt("ControlPortLocal", connection_info.control_port_local);
        connection_info.control_port_remote =
            settings_json.getInt("ControlPortRemote", connection_info.control_port_remote);
    ...
    createVehicleSetting(const std::string &simmode_name, const Settings &settings_json, const std::string vehicle_name,
                         std::map<std::string, std::shared_ptr<SensorSetting>> &sensor_defaults,
        ...
        vehicle_setting->allow_api_always = settings_json.getBool("AllowAPIAlways", vehicle_setting->allow_api_always);
        vehicle_setting->auto_create = settings_json.getBool("AutoCreate", vehicle_setting->auto_create);
        ...
        vehicle_setting->enable_trace = settings_json.getBool("EnableTrace", vehicle_setting->enable_trace);
        vehicle_setting->enable_collisions =
            settings_json.getBool("EnableCollisions", vehicle_setting->enable_collisions);
        vehicle_setting->is_fpv_vehicle = settings_json.getBool("IsFpvVehicle", vehicle_setting->is_fpv_vehicle);
    ...
    static void createDefaultVehicle(const std::string &simmode_name,
                                     std::map<std::string, std::unique_ptr<VehicleSetting>> &vehicles,
        ...
        if (simmode_name == kSimModeTypeMultirotor) {
            ...
            auto simple_flight_setting =
                std::unique_ptr<VehicleSetting>(new VehicleSetting("SimpleFlight", kVehicleTypeSimpleFlight));
            // TODO: we should be selecting remote if available else keyboard
            // currently keyboard is not supported so use rc as default
            ...
            auto physx_car_setting =
                std::unique_ptr<VehicleSetting>(new VehicleSetting("PhysXCar", kVehicleTypePhysXCar));
            ...
            auto cv_setting =
                std::unique_ptr<VehicleSetting>(new VehicleSetting("ComputerVision", kVehicleTypeComputerVision));
            cv_setting->sensors = sensor_defaults;
            vehicles[cv_setting->vehicle_name] = std::move(cv_setting);
        } else {
            throw std::invalid_argument(
                Utils::stringf("Unknown SimMode: %s, failed to set default vehicle settings", simmode_name.c_str())
                    .c_str());
        }
    }
    ...
    static void loadVehicleSettings(const std::string &simmode_name, const Settings &settings_json,
        ...
        nervosys::autonomylib::Settings vehicles_child;
        if (settings_json.getChild("Vehicles", vehicles_child)) {
            ...
            for (const auto &key : keys) {
                nervosys::autonomylib::Settings child;
    ...
    static void initializePawnPaths(std::map<std::string, PawnPath> &pawn_paths) {
        ...
        pawn_paths.emplace("BareboneCar",
                           PawnPath("Class'/AutonomySim/VehicleAdv/Vehicle/VehicleAdvPawn.VehicleAdvPawn_C'"));
        pawn_paths.emplace("DefaultCar", PawnPath("Class'/AutonomySim/VehicleAdv/SUV/SuvCarPawn.SuvCarPawn_C'"));
        pawn_paths.emplace("DefaultQuadrotor",
                           PawnPath("Class'/AutonomySim/Blueprints/BP_FlyingPawn.BP_FlyingPawn_C'"));
        pawn_paths.emplace("DefaultComputerVision",
                           PawnPath("Class'/AutonomySim/Blueprints/BP_ComputerVisionPawn.BP_ComputerVisionPawn_C'"));
    }

    static void loadPawnPaths(const Settings &settings_json, std::map<std::string, PawnPath> &pawn_paths) {
        initializePawnPaths(pawn_paths);

        nervosys::autonomylib::Settings pawn_paths_child;
    ...
    static void loadSegmentationSetting(const Settings &settings_json, SegmentationSetting &segmentation_setting) {
        ...
        if (settings_json.getChild("SegmentationSettings", json_parent)) {
            ...
            if (init_method == "" || init_method == "commonobjectsrandomids")
                ...
                throw std::invalid_argument(
                    std::string("SegmentationSetting init_method has invalid value in settings_json ") + init_method);
            ...
            if (mesh_naming_method == "" || mesh_naming_method == "ownername")
                ...
                throw std::invalid_argument(
                    std::string("SegmentationSetting MeshNamingMethod has invalid value in settings_json ") +
                    mesh_naming_method);
        }
    }
    ...
    static void loadNoiseSetting(const Settings &settings_json, NoiseSetting &noise_setting) {
        ...
        noise_setting.HorzWaveScreenSize =
            settings_json.getFloat("HorzWaveScreenSize", noise_setting.HorzWaveScreenSize);
        noise_setting.HorzNoiseLinesContrib =
            settings_json.getFloat("HorzNoiseLinesContrib", noise_setting.HorzNoiseLinesContrib);
        noise_setting.HorzNoiseLinesDensityY =
            settings_json.getFloat("HorzNoiseLinesDensityY", noise_setting.HorzNoiseLinesDensityY);
        noise_setting.HorzNoiseLinesDensityXY =
            settings_json.getFloat("HorzNoiseLinesDensityXY", noise_setting.HorzNoiseLinesDensityXY);
        noise_setting.HorzDistortionStrength =
            settings_json.getFloat("HorzDistortionStrength", noise_setting.HorzDistortionStrength);
        noise_setting.HorzDistortionContrib =
            settings_json.getFloat("HorzDistortionContrib", noise_setting.HorzDistortionContrib);
    }
            ...
            if (ue_settings_json.getChild("PixelFormatOverride", pixel_format_override_settings_json)) {
                for (size_t child_index = 0; child_index < pixel_format_override_settings_json.size(); ++child_index) {
                    ...
                    if (pixel_format_override_settings_json.getChild(child_index, pixel_format_child_json)) {
                        ...
                        PixelFormatOverrideSetting &pixel_format_setting =
                            ue_setting.pixel_format_override_settings.at(image_type);
                        pixel_format_setting.pixel_format =
                            pixel_format_child_json.getInt("PixelFormat", 0); // default to EXPixelformat::PF_Unknown
                    }
                }
            }
        }
    ...
    static void createCaptureSettings(const nervosys::autonomylib::Settings &settings_json, CaptureSetting &capture_setting) {
        ...
        capture_setting.auto_exposure_speed =
            settings_json.getFloat("AutoExposureSpeed", capture_setting.auto_exposure_speed);
        capture_setting.auto_exposure_bias =
            settings_json.getFloat("AutoExposureBias", capture_setting.auto_exposure_bias);
        capture_setting.auto_exposure_max_brightness =
            settings_json.getFloat("AutoExposureMaxBrightness", capture_setting.auto_exposure_max_brightness);
        ...
        capture_setting.motion_blur_amount =
            settings_json.getFloat("MotionBlurAmount", capture_setting.motion_blur_amount);
        capture_setting.image_type = settings_json.getInt("ImageType", 0);
        capture_setting.target_gamma = settings_json.getFloat(
            "TargetGamma", capture_setting.image_type == 0 ? CaptureSetting::kSceneTargetGamma : Utils::nan<float>());
        ...
        if (projection_mode == "" || projection_mode == "perspective")
            ...
            throw std::invalid_argument(
                std::string("CaptureSettings projection_mode has invalid value in settings_json ") + projection_mode);

        capture_setting.ortho_width = settings_json.getFloat("OrthoWidth", capture_setting.ortho_width);
    }

    static void loadSubWindowsSettings(const Settings &settings_json,
                                       std::vector<SubwindowSetting> &subwindow_settings) {
        ...
        if (settings_json.getChild("SubWindows", json_parent)) {
            for (size_t child_index = 0; child_index < json_parent.size(); ++child_index) {
                ...
                if (json_parent.getChild(child_index, json_settings_child)) {
                    ...
                    subwindow_setting.image_type = Utils::toEnum<ImageType>(json_settings_child.getInt("ImageType", 0));
        ...
        { // time of day settings_json
            ...
            if (settings_json.getChild("TimeOfDay", tod_settings_json)) {
                ...
                tod_setting.celestial_clock_speed =
                    tod_settings_json.getFloat("CelestialClockSpeed", tod_setting.celestial_clock_speed);
                tod_setting.is_start_datetime_dst =
                    tod_settings_json.getBool("StartDateTimeDst", tod_setting.is_start_datetime_dst);
                tod_setting.update_interval_secs =
                    tod_settings_json.getFloat("UpdateIntervalSecs", tod_setting.update_interval_secs);
    ...
    static void loadCameraDirectorSetting(const Settings &settings_json, CameraDirectorSetting &camera_director,
        ...
        if (clock_type == "") {
            ...
            if (simmode_name == kSimModeTypeMultirotor) {
                ...
                for (auto const &vehicle : vehicles) {
                    if (vehicle.second->auto_create && vehicle.second->vehicle_type == kVehicleTypePX4) {
                        clock_type = "ScalableClock";
                        break;
                    }
                }
    ...
    static std::shared_ptr<SensorSetting> createSensorSetting(SensorBase::SensorType sensor_type,
                                                              const std::string &sensor_name, bool enabled) {
...

    {
        ...
        nervosys::autonomylib::Settings sensors_child;
    ...
    static void loadDefaultSensorSettings(const std::string &simmode_name, const Settings &settings_json,
                                          std::map<std::string, std::shared_ptr<SensorSetting>> &sensors) {
        nervosys::autonomylib::Settings sensors_child;
    ...
    static void loadExternalCameraSettings(const Settings &settings_json, CameraSettingMap &external_cameras,
                                           const CameraSetting &camera_defaults) {
