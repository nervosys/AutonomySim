...
class StateReporter {
  public:
    ...
    string getOutput() const { return ss_.str(); }
    ...
    template <typename T> void writeValue(string name, const T &r) {
        writeNameOnly(name);
        writeValueOnly(r, true);
    }
    void writeNameOnly(string name) { ss_ << name << ": "; }
    template <typename T> void writeValueOnly(const T &r, bool end_line_or_tab = false) {
        ss_ << r;
