#ifndef autonomylib_vehicles_simpleflight_firmware_CascadeController_hpp
#define autonomylib_vehicles_simpleflight_firmware_CascadeController_hpp
    ...
    virtual void update() override {
        ...
        for (unsigned int axis = 0; axis < Axis4r::AxisCount(); ++axis) {
            ...
            if (axis_controllers_[axis] != nullptr) {
                ...
                comm_link_->log(std::string("Axis controller type is not set for axis ").append(std::to_string(axis)),
                                ICommLink::kLogLevelInfo);
        }
        params_->gains_changed = false;
    }

    virtual const Axis4r &getOutput() override { return output_; }
