#ifndef autonomylib_vehicles_simpleflight_firmware_VelocityController_hpp
#define autonomylib_vehicles_simpleflight_firmware_VelocityController_hpp
...
class VelocityController : public IAxisController, public IGoal { // for internal child controller
  ...
  public:
    VelocityController(Params *params, const IBoardClock *clock = nullptr) : params_(params), clock_(clock) {}
    ...
    virtual void initialize(unsigned int axis, const IGoal *goal, const IStateEstimator *state_estimator) override {
        ...
        PidConfig<float> pid_config(params_->velocity_pid.p[axis], params_->velocity_pid.i[axis],
        ...
        case 2:
            // we control yaw
            throw std::invalid_argument("axis must be 0, 1 or 3 but it was " + std::to_string(axis_) +
    ...
    virtual void update() override {
        ...
        const Axis3r &goal_velocity_world = Axis4r::axis4ToXyz(goal_->getGoalValue(), true);
        const Axis4r &goal_velocity_local =
            Axis4r::xyzToAxis4(state_estimator_->transformToBodyFrame(goal_velocity_world), true);
        pid_->setGoal(goal_velocity_local[axis_]);

        const Axis3r &measured_velocity_world = state_estimator_->getLinearVelocity();
        const Axis4r &measured_velocity_local =
            Axis4r::xyzToAxis4(state_estimator_->transformToBodyFrame(measured_velocity_world), true);
        ...
        case 0: //+vy is +ve roll
            ...
            //     nervosys::autonomylib::Utils::log(nervosys::autonomylib::Utils::stringf("VC: %i\t%f\t%f\t%f",
