#ifndef autonomylib_vehicles_simpleflight_firmware_RemoteControl_hpp
#define autonomylib_vehicles_simpleflight_firmware_RemoteControl_hpp
...
class RemoteControl : public IGoal, public IUpdatable {
  public:
    RemoteControl(const Params *params, const IBoardClock *clock, const IBoardInputPins *board_inputs,
        ...
        : params_(params), clock_(clock), board_inputs_(board_inputs), vehicle_state_(vehicle_state),
    ...
    virtual void update() override {
        ...
        for (unsigned int axis = 0; axis < Axis4r::AxisCount(); ++axis) {
            channels[axis] =
                board_inputs_->isRcConnected() ? board_inputs_->readChannel(params_->rc.channels[axis]) : 0;
        }
  ...
  private:
    enum class RcRequestType { None, ArmRequest, DisarmRequest, NeutralRequest };
    ...
    RcRequestType getActionRequest(const Axis4r &channels) {
        ...
        else if (isInTolerance(channels.roll(), tolerance) && isInTolerance(channels.pitch(), tolerance) &&
