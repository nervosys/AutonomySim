...
class AdaptiveController : public IController {
  ...
  private:
    ...
    const double Jr =
        0.00006f; // inertia of rotor, currently an estimate; make more accurate by getting a measured value
    ...
    void update_state_vals(double x_val, double vx, double y_val, double vy, double z_val, double vz, double roll,
                           double roll_rate, double pitch, double pitch_rate, double yaw, double yaw_rate) {
        x_in = x_val;
        xdot_in = vx;
        y_in = -y_val;
        ydot_in = -vy;
    ...
    void run() { rungeKutta(adaptive_y, adaptive_output, getTimeU(), 0.003f, array_length); }
    ...
    void Sliding_Iconfig(double zddot, double x_desired, double x_current, double xdot, double y_desired,
                         double y_current, double ydot, double yaw_current, double result[2][1]) {
    ...
    void inverse_3x3(double A[3][3], double result[3][3]) {
        ...
        det_A = A[0][0] * (A[1][1] * A[2][2] - A[1][2] * A[2][1]) +
                A[0][1] * (A[1][2] * A[2][0] - A[1][0] * A[2][2]) * A[2][0] * (A[1][0] * A[2][1] - A[1][1] * A[2][0]);
        if (det_A == 0) {
            result[0][0] = 0;
            result[0][1] = 0;
            result[0][2] = 0;
    ...
    void Angular_velocities_from_PQR(double PQR_val[3][1], double Angles_val[3][1], double result[3][1]) {
        result[0][0] = PQR_val[0][0] + PQR_val[1][0] * sin(Angles_val[0][0]) * tan(Angles_val[1][0]) +
                       PQR_val[2][0] * cos(Angles_val[0][0]) * tan(Angles_val[1][0]);
        result[1][0] = PQR_val[1][0] * cos(Angles_val[0][0]) - PQR_val[2][0] * sin(Angles_val[0][0]);
        result[2][0] = PQR_val[1][0] * (sin(Angles_val[0][0]) / cos(Angles_val[1][0])) +
                       PQR_val[2][0] * (cos(Angles_val[0][0]) / cos(Angles_val[1][0]));
    }
    ...
    void model(double *y, double last_time_, double *y_out) {
        ...
        // generate delta_dot which goes to integrator variable, sliding surface for 3 euler angles, can adjust sliding
        // surface slope as desired
        ...
        rollrate_ref = R_inverse[0][0] * S2_phi * k_phi + R_inverse[0][1] * S2_theta * k_theta +
                       R_inverse[0][2] * S2_psi * k_psi - delta_phi;
        pitchrate_ref = R_inverse[1][0] * S2_phi * k_phi + R_inverse[1][1] * S2_theta * k_theta +
                        R_inverse[1][2] * S2_psi * k_psi - delta_theta;
        yawrate_ref = R_inverse[2][0] * S2_phi * k_phi + R_inverse[2][1] * S2_theta * k_theta +
                      R_inverse[2][2] * S2_psi * k_psi - delta_psi;
  ...
  public:
    ...
    virtual const Axis4r &getOutput() override {
        ...
        update_state_vals(position[0], velocity[0], position[1], velocity[1], position[2], velocity[2], angle[0],
                          angle_rate[0], angle[1], angle_rate[1], angle[2], angle_rate[2]);
