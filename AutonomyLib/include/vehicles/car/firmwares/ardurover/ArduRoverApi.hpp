...
#ifndef autonomylib_vehicles_ArduRoverCarController_hpp
#define autonomylib_vehicles_ArduRoverCarController_hpp
...
class ArduRoverApi : public CarApiBase {
  ...
  public:
    ArduRoverApi(const AutonomySimSettings::VehicleSetting *vehicle_setting,
        ...
        connection_info_ =
            static_cast<const AutonomySimSettings::MavLinkVehicleSetting *>(vehicle_setting)->connection_info;
        sensors_ = &getSensors();

        connect();
    }

    ~ArduRoverApi() { closeConnections(); }
  ...
  public:
    virtual void setCarControls(const CarControls &controls) override {
        // Currently ArduPilot vehicles don't implement AutonomySim movement APIs
        unused(controls);
    }
