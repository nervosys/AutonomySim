...
int main(int argc, const char *argv[]) {
    ...
    if (argc != 2) {
        ...
        std::cout << "Start the DroneServer using the 'PX4' settings in ~/Documents/AutonomySim/settings.json."
                  << std::endl;
        return 1;
    }
    ...
    AutonomySimSettings::MavLinkConnectionInfo connection_info;
    ...
    if (settings.isLoadSuccess()) {
        ...
        connection_info.vehicle_sysid =
            static_cast<uint8_t>(child.getInt("VehicleSysID", connection_info.vehicle_sysid));
        connection_info.vehicle_compid = child.getInt("VehicleCompID", connection_info.vehicle_compid);

        connection_info.offboard_sysid =
            static_cast<uint8_t>(child.getInt("OffboardSysID", connection_info.offboard_sysid));
    ...
    nervosys::autonomylib::MultirotorRpcLibServer server(&api_provider, connection_info.local_host_ip);

    // start server in async mode
    server.start(false, 4);

    if (connection_info.use_tcp) {
        std::cout << "Server connected to MavLink TCP endpoint at " << connection_info.local_host_ip << ":"
                  << connection_info.tcp_port << std::endl;
    } else {
        std::cout << "Server connected to MavLink UDP endpoint at " << connection_info.local_host_ip << ":"
                  << connection_info.udp_port << std::endl;
    }
