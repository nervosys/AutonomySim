...
void CStateStereo::Initialize(SGMOptions &params, int m, int n) {
    ...
    sgmStereo = new SGMStereo(processingFrameWidth, processingFrameHeight, -maxDisp, -minDisp, params.numDirections,
                              params.sgmConfidenceThreshold, params.doSubPixRefinement, params.smoothness,
                              params.penalty1, params.penalty2, params.alpha, params.doSequential);
}
...
void CStateStereo::ProcessFrameAutonomySim(int frameCounter, float &dtime, const std::vector<uint8_t> &left_image,
    ...
    printf("Frame %06d:	%5.1f ms, Average fps: %lf\n", frameCounter, duration * 1000,
           1.0 / (dtime / double(frameCounter + 1)));
