...
#ifndef sgm_dsimage_h
#define sgm_dsimage_h
...
class DSI {
  public:
    ...
    short operator()(uint64_t x, uint64_t y, uint64_t z) const { return m_data[(x + y * m_cols) * m_planes + z]; }

    short &operator()(uint64_t x, uint64_t y, uint64_t z) { return m_data[(x + y * m_cols) * m_planes + z]; }

    short *operator()(uint64_t x, uint64_t y) const { return &(m_data[(x + y * m_cols) * m_planes]); }
...
#pragma omp parallel for schedule(dynamic, 1)
            ...
            for (int x = 1; x < m_cols - 1; x++) {
                ...
                float conf =
                    (float)__min(__max(20.0f * log(1.0f / (distinctiveness1 * distinctiveness1)), 0.0f), 255.0f);
