...
SGMStereo::SGMStereo(int _w, int _h, int minDisparity, int maxDisparity, int numDirections, int sgmConfidenceThreshold,
                     int doSubPixRefinement, float smoothness, float penalty1, float penalty2, float alpha,
                     int doSequential) {
    m_w = _w;
    m_h = _h;
        ...
        for (int x = 1; x < cols - 1; x++) {
            ...
            for (int disp = m_minDisparity; disp < m_maxDisparity; disp += 4) {
                if (x + disp - 1 >= 0 && x + disp + 5 < m_w) {
                        ...
                        _mm_add_ps(
                            ...
                            _mm_add_ps(
                                ...
                                _mm_add_ps(
                                    ...
                                    _mm_add_ps(
                                        v_01, _mm_add_ps(
                                                  v_11, _mm_add_ps(v_21, _mm_add_ps(v_02, _mm_add_ps(v_12, v_22)))))))),
                        _mm_set1_ps(9.0f));
                    ...
                    __m128 ncc1 = _mm_div_ps(
                        sum_v1_x_v2, _mm_sqrt_ps(_mm_max_ps(_mm_mul_ps(sum_v1_x_v1, sum_v2_x_v2), _mm_set1_ps(0.01f))));
                    __m128 score = _mm_min_ps(_mm_mul_ps(_mm_sub_ps(_mm_set1_ps(1.0f), ncc1), _mm_set1_ps(255.0f)),
                                              _mm_set1_ps(255.0f));
                    float scores[4];
...
void subtractMinVal(short *pMessage, int size) {
    ...
    x.m = minval;
    short val =
        __min(__min(__min(x.s[0], x.s[1]), __min(x.s[2], x.s[3])), __min(__min(x.s[4], x.s[5]), __min(x.s[6], x.s[7])));
...
void SGMStereo::messagePassing(short *pData, short *pBuffer1, short *pDMessage, int size, float weight,
                               short smoothness) {
...
void SGMStereo::scanlineOptimization(DSI &dv, DSI &msgs, unsigned char *img, float *lut, int dx_, int dy_) {
    ...
    for (int ys = 0; ys < rows; ys++) {
        for (int xs = 0; xs < cols; xs++) {
            if ((xs == 0 && dx_ == 1) || (ys == 0 && dy_ == 1) || (ys == rows - 1 && dy_ == -1) ||
                (xs == cols - 1 && dx_ == -1)) {
...
void SGMStereo::Run(unsigned char *iLeft, unsigned char *iRight, float *dispMap, unsigned char *confMap) {
