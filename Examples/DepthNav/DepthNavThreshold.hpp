...
class DepthNavThreshold : public DepthNav {
  ...
  protected:
    virtual Pose getNextPose(const std::vector<float> &depth_image, const Vector3r &goal, const Pose &current_pose,
                             real_T dt) override {
        ...
        if (true) {
            ...
            Vector2r planeSize = getPlaneSize(params_.max_allowed_obs_dist, params_.fov,
                                              hfov2vfov(params_.fov, params_.depth_height, params_.depth_width));
            ...
            Vector3r intersect_point =
                linePlaneIntersection(goal_body, VectorMath::front(), params_.max_allowed_obs_dist);
            // Check if intersection is valid
            if (VectorMath::hasNan(intersect_point)) {
                return rotateToGoal(current_pose, goal);
            } else if (intersect_point.z() < z_min || intersect_point.z() > z_max || intersect_point.y() < y_min ||
                       intersect_point.y() > y_max) {
                return rotateToGoal(current_pose, goal);
            } else {
                // 4. So now we have a rectangle and a point within it
                // 5. Then discretize the rectangle in M * N cells. This is simply truncation after division. For each
                // pixel we can now get cell coordinates.
            ...
            */
                ...
                for (int i = 0; i < cell_centers.size(); ++i) {
                    if (isCellFree(depth_image, cell_centers[spiral_idxs[i]])) {
                        // 8. We are here if we have found cell coordinates i, j as center of the free window from step
                        // #7
                        // 9. Compute i_x_center, j_y_center that would be center pixel of this cell in the plane for x
                        // = 1
                        ...
                        Vector3r goal_world = VectorMath::transformToWorldFrame(
                            Vector3r(params_.max_allowed_obs_dist, goal_y, goal_z).normalized(), current_pose);
                        ...
                        // 11. Compute next position along vector (i_x_center, j_y_center, 1) and transform it to world
                        // frame
                        goal_pose.position = VectorMath::transformToWorldFrame(
                            Vector3r(params_.max_allowed_obs_dist, goal_y, goal_z).normalized() * dt, goal_pose);
    ...
    bool isCellFree(const std::vector<float> &img, Vector2r cell_center) {
        ...
        for (int i = int(cell_center.y() - params_.vehicle_height_px / 2);
             i < int(cell_center.y() + params_.vehicle_height_px / 2); i++) {
            for (int j = int(cell_center.x() - params_.vehicle_width_px / 2);
                 j < int(cell_center.x() + params_.vehicle_width_px / 2); j++) {
