...
class DepthNavOptAStar {
    ...
    virtual void gotoGoal(const Pose &goal_pose, RpcLibClientBase &client) {
        ...
        do {
            ...
            if (VectorMath::hasNan(next_pose))
                ...
                Vector3r angular_vel = VectorMath::toAngularVelocity(current_pose.orientation, next_pose.orientation,
                                                                     params_.control_loop_period);
  ...
  protected:
    Pose getNextPose(const std::vector<float> &depth_image, const Vector3r &goal, const Pose &current_pose, real_T dt) {
        ...
        if (generate_debug_info_) {
            ...
            writeToBmpFile(bmp, params_.depth_width, params_.depth_height,
                           common_utils::FileSystem::combine(std::string("d:\\temp\\111\\"),
                                                             Utils::stringf("disparity_ % 06d.bmp", iteration_index_)));
        }
    ...
    static void writeToBmpFile(const std::vector<common_utils::bmp::rgb_t> &image, unsigned int width,
    ...
    void setupRay(SampleRay &sample_ray, const std::vector<float> &depth_image, const Vector3r &goal_body,
                  real_T goal_dist) {
    ...
    Vector3r pixel2ray(unsigned int x, unsigned y) {
        ...
        Vector3r ray(1.0f, std::tan(pixel_y_n * 2 * params_.tan_hfov_by_2 / params_.depth_width),
