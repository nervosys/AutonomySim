...
class DepthNav {
  public: // types
    ...
    class DepthNavException : public std::runtime_error {
      public:
        DepthNavException(const std::string &message) : std::runtime_error(message) {}
    };

  public:
    Params params_;

    DepthNav(const Params &params = Params()) : params_(params) {}
    ...
    void initialize(RpcLibClientBase &client, const std::vector<ImageCaptureBase::ImageRequest> &request) {
        ...
        params_.vehicle_height_px = int(ceil(params_.depth_height * params_.vehicle_height /
                                             (tan(params_.fov / 2) * params_.max_allowed_obs_dist * 2))); // height
        params_.vehicle_width_px =
            int(ceil(params_.depth_width * params_.vehicle_width /
                     (tan(hfov2vfov(params_.fov, params_.depth_height, params_.depth_width) / 2) *
                      params_.max_allowed_obs_dist * 2))); // width
    }
    ...
    virtual void gotoGoal(const Pose &goal_pose, RpcLibClientBase &client,
        ...
        do {
            ...
            const Pose next_pose = getNextPose(response.at(0).image_data_float, goal_pose.position, current_pose,
    ...
    virtual void gotoGoalSGM(const Pose &goal_pose, RpcLibClientBase &client,
                             const std::vector<ImageCaptureBase::ImageRequest> &request, CStateStereo *p_state) {
        ...
        if (response.at(0).height != params_.depth_height || response.at(0).width != params_.depth_height)
            throw DepthNavException("Image Dimension mismatch. Please check left camera in the AutonomySim config
        file.");

        if (response.at(1).height != params_.depth_height || response.at(1).width != params_.depth_height)
            throw DepthNavException("Image Dimension mismatch. Please check right camera in the AutonomySim config
        file.");
        */
            ...
            p_state->ProcessFrameAutonomySim(counter, dtime, left_image, right_image);
            ...
            const Pose next_pose =
                getNextPose(sgm_depth_image, goal_pose.position, current_pose, params_.control_loop_period);
