...
class StandALoneSensors {
  public:
    static void generateImuStaticData(std::ostream &output_stream, float period, float total_duration) {
        ...
        nervosys::autonomylib::Environment::State initial_environment(kinematics.pose.position, GeoPoint());
        nervosys::autonomylib::Environment environment(initial_environment);
        environment.reset();
        ...
        for (auto i = 0; i < interations; ++i) {
            ...
            output_stream << output.angular_velocity.x() << "\t" << output.angular_velocity.y() << "\t"
                          << output.angular_velocity.z() << "\t";
            output_stream << output.linear_acceleration.x() << "\t" << output.linear_acceleration.y() << "\t"
                          << output.linear_acceleration.z() << "\n";

            std::this_thread::sleep_for(
                std::chrono::duration<double>(static_cast<double>(period) - (Utils::getTimeSinceEpochSecs() - last)));
    ...
    static void generateBarometerStaticData(std::ostream &output_stream, float period, float total_duration,
                                            GeoPoint loc) {
        auto kinematics = Kinematics::State::zero();
        nervosys::autonomylib::Environment::State initial_environment(kinematics.pose.position, loc);
        nervosys::autonomylib::Environment environment(initial_environment);
        environment.reset();
    ...
    static void generateBarometerDynamicData(std::ostream &output_stream, float period, float total_duration,
        ...
        for (auto j = 0; j < 10; ++j) {
            for (auto i = 0; i < interations_20s; ++i) {
                ...
                std::this_thread::sleep_for(std::chrono::duration<double>(static_cast<double>(period) -
                                                                          (Utils::getTimeSinceEpochSecs() - last)));
    ...
    static void generateMagnetometer2D(std::ostream &output_stream, float period, float total_duration, GeoPoint loc,
                                       float yawStart, bool ccw = false) {
        output_stream << std::fixed;
        ...
        for (float direction = 0; direction < 5; direction++) {
            ...
            for (auto i = 0; i < interations; ++i) {
                ...
                output_stream << Utils::getTimeSinceEpochSecs() << "\t";
                output_stream << output.magnetic_field_body.x() << "\t" << output.magnetic_field_body.y() << "\t"
                              << output.magnetic_field_body.z();
    ...
    static void generateMagnetometer3D(std::ostream &output_stream, float period, float total_duration, GeoPoint loc,
                                       float yawStart = 0, bool ccw = false) {
        output_stream << std::fixed;
            ...
            for (float roll = 0; roll < 2.1 * M_PIf; roll += M_PIf / 2) {
                for (float direction = 0; direction < 5; direction++) {
                    ...
                    for (auto i = 0; i < interations; ++i) {
                        ...
                        output_stream << "\t" << kinematics.pose.orientation.w() << "\t"
                                      << kinematics.pose.orientation.x() << "\t" << kinematics.pose.orientation.y()
                                      << "\t" << kinematics.pose.orientation.z();
                        output_stream << std::endl;

                        std::this_thread::sleep_for(std::chrono::duration<double>(
                            static_cast<double>(period) - (Utils::getTimeSinceEpochSecs() - last)));
                        last = Utils::getTimeSinceEpochSecs();
                        environment.update();
                        mag.update();
                    }
