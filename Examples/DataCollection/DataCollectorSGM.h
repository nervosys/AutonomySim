...
#include "../../SemiGlobalMatching/src/sgmstereo/sgmstereo.h"
#include "../../SemiGlobalMatching/src/stereoPipeline/StateStereo.h"
...
class DataCollectorSGM {
  ...
  public:
    DataCollectorSGM(std::string storage_dir) : storage_dir_(storage_dir) {
    ...
    int generate(int num_samples) {
        ...
        nervosys::autonomylib::ClockBase *clock = nervosys::autonomylib::ClockFactory::get();
  ...
  private:
    ...
    typedef nervosys::autonomylib::ImageCaptureBase::ImageRequest ImageRequest;
    typedef nervosys::autonomylib::ImageCaptureBase::ImageResponse ImageResponse;
  ...
  private:
    struct ImagesResult {
        ...
        nervosys::autonomylib::TTimeDelta render_time;
        std::string storage_dir_;
    ...
    void processImages(ImagesResult *result) {
        ...
        p_state->ProcessFrameAutonomySim(result->sample, dtime, left_img, right_img);
        ...
        Utils::writePFMfile(gt_disparity_data.data(), w, h,
                            FileSystem::combine(result->storage_dir_, disparity_gt_file_name));

        // SGM depth disparity and confidence
        Utils::writePFMfile(sgm_depth_data.data(), w, h,
                            FileSystem::combine(result->storage_dir_, depth_sgm_file_name));
        Utils::writePFMfile(sgm_disparity_data.data(), w, h,
                            FileSystem::combine(result->storage_dir_, disparity_sgm_file_name));
        ...
        FILE *disparity_sgm =
            fopen(FileSystem::combine(result->storage_dir_, disparity_sgm_viz_file_name).c_str(), "wb");
        ...
        (*result->file_list) << left_file_name << "," << right_file_name << "," << depth_gt_file_name << ","
                             << disparity_gt_file_name << "," << depth_sgm_file_name << "," << disparity_sgm_file_name
                             << "," << confidence_sgm_file_name << std::endl;

        std::cout << "Image #" << result->sample << " pos:" << VectorMath::toString(result->position)
                  << " ori:" << VectorMath::toString(result->orientation) << " render time "
                  << result->render_time * 1E3f << "ms"
                  << " process time " << clock->elapsedSince(process_time) * 1E3f << " ms" << std::endl;
    }
