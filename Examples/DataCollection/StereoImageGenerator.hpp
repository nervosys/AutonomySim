...
class StereoImageGenerator {
  public:
    StereoImageGenerator(std::string storage_dir) : storage_dir_(storage_dir) { FileSystem::ensureFolder(storage_dir); }
  ...
  private:
    ...
    static void processImages(common_utils::ProsumerQueue<ImagesResult> *results) {
        while (!results->getIsDone()) {
            ...
            // convertToDisparity(depth_data, result.response.at(2).width, result.response.at(2).height, f, 25 /
            // 100.0f);
    ...
    static void convertToDisparity(std::vector<float> &image_data, int width, int height, float f = 320,
                                   float baseline_meters = 1) {
