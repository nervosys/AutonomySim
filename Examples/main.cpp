...
void runDataCollectorSGM(const int argc, const char *argv[]) {
    runDataCollectorSGM(
        argc < 2 ? 5000 : std::stoi(argv[1]),
        argc < 3 ? common_utils::FileSystem::combine(common_utils::FileSystem::getAppDataFolder(), "data_sgm")
                 : std::string(argv[2]));
}
...
void runStereoImageGenerator(const int argc, const char *argv[]) {
    runStereoImageGenerator(
        argc < 2 ? 50000 : std::stoi(argv[1]),
        argc < 3 ? common_utils::FileSystem::combine(common_utils::FileSystem::getAppDataFolder(), "stereo_gen")
                 : std::string(argv[2]));
}
...
void runDepthNavSGM() {
    ...
    p_state.Initialize(params, static_cast<int>(depthNav.params_.depth_height),
                       static_cast<int>(depthNav.params_.depth_width));
