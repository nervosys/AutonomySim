...
void ASimModeWorldBase::BeginPlay() { Super::BeginPlay(); }

void ASimModeWorldBase::initializeForPlay() {
    std::vector<nervosys::autonomylib::UpdatableObject *> vehicles;
...
void ASimModeWorldBase::startAsyncUpdator() { physics_world_->startAsyncUpdator(); }

void ASimModeWorldBase::stopAsyncUpdator() { physics_world_->stopAsyncUpdator(); }

long long ASimModeWorldBase::getPhysicsLoopPeriod() const // nanoseconds
{
    return physics_loop_period_;
}
void ASimModeWorldBase::setPhysicsLoopPeriod(long long period) { physics_loop_period_ = period; }
...
std::unique_ptr<ASimModeWorldBase::PhysicsEngineBase> ASimModeWorldBase::createPhysicsEngine() {
    ...
    if (physics_engine_name == "")
        ...
        nervosys::autonomylib::Settings fast_phys_settings;
        if (nervosys::autonomylib::Settings::singleton().getChild("FastPhysicsEngine", fast_phys_settings)) {
            physics_engine.reset(
                new nervosys::autonomylib::FastPhysicsEngine(fast_phys_settings.getBool("EnableGroundLock", true)));
        } else {
            physics_engine.reset(new nervosys::autonomylib::FastPhysicsEngine());
        }
        ...
        physics_engine.reset(new nervosys::autonomylib::ExternalPhysicsEngine());
    } else {
        physics_engine.reset();
...
bool ASimModeWorldBase::isPaused() const { return physics_world_->isPaused(); }
...
void ASimModeWorldBase::setWind(const nervosys::autonomylib::Vector3r &wind) const { physics_engine_->setWind(wind); }

void ASimModeWorldBase::setExtForce(const nervosys::autonomylib::Vector3r &ext_force) const {
    physics_engine_->setExtForce(ext_force);
}

void ASimModeWorldBase::updateDebugReport(nervosys::autonomylib::StateReporterWrapper &debug_reporter) {
...
void ASimModeWorldBase::reset() {
    UAutonomyBlueprintLib::RunCommandOnGameThread([this]() { physics_world_->reset(); }, true);

    // no need to call base reset because of our custom implementation
}

std::string ASimModeWorldBase::getDebugReport() { return physics_world_->getDebugReport(); }
