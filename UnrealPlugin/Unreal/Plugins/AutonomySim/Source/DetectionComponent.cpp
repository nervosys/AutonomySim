...
UDetectionComponent::UDetectionComponent() : max_distance_to_camera_(20000.f) {
        ...
        if (object_filter_.matchesActor(actor)) {
            if (FVector::Distance(actor->GetActorLocation(), GetComponentLocation()) <= max_distance_to_camera_) {
                ...
                if (texture_target_ && calcBoundingFromViewInfo(actor, box_2D_out)) {
                    ...
                    detection.RelativeTransform =
                        FTransform(getRelativeRotation(actor->GetActorLocation(), actor->GetActorRotation()),
...
bool UDetectionComponent::calcBoundingFromViewInfo(AActor *actor, FBox2D &box_out) {
    ...
    projection_data.ViewRotationMatrix =
        FInverseRotationMatrix(info.Rotation) *
    ...
    for (FVector &point : points) {
        ...
        is_in_camera_view |= (Pixel != screen_rect.Min) && (Pixel != screen_rect.Max) &&
                             screen_rect.Contains(FIntPoint(Pixel.X, Pixel.Y));
        points_2D.Add(Pixel);
    ...
    if (is_in_camera_view) {
        ...
        if (!is_visible) {
            for (int i = 0; i < 10; i++) {
                ...
                is_world_hit =
                    GetWorld()->LineTraceSingleByChannel(result, GetComponentLocation(), point, ECC_WorldStatic);
                if (is_world_hit) {
                    if (result.GetActor() == actor) {
                        is_visible = true;
                        break;
...
void UDetectionComponent::setFilterRadius(const float radius_cm) { max_distance_to_camera_ = radius_cm; }

void UDetectionComponent::clearMeshNames() { object_filter_.wildcard_mesh_names_.Empty(); }
