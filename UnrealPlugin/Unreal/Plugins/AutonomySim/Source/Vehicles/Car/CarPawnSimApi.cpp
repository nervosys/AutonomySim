...
void CarPawnSimApi::initialize() {
    ...
    std::shared_ptr<UnrealSensorFactory> sensor_factory =
        std::make_shared<UnrealSensorFactory>(getPawn(), &getNedTransform());

    vehicle_api_ = CarApiFactory::createApi(getVehicleSetting(), sensor_factory, *getGroundTruthKinematics(),
                                            *getGroundTruthEnvironment());
    pawn_api_ = std::unique_ptr<CarPawnApi>(
        new CarPawnApi(static_cast<ACarPawn *>(getPawn()), getGroundTruthKinematics(), vehicle_api_.get()));

    // TODO: should do reset() here?
    joystick_controls_ = nervosys::autonomylib::CarApiBase::CarControls();
}

std::string CarPawnSimApi::getRecordFileLine(bool is_header_line) const {
    std::string common_line = PawnSimApi::getRecordFileLine(is_header_line);
    if (is_header_line) {
        return common_line + "Throttle\tSteering\tBrake\tGear\tHandbrake\tRPM\tSpeed\t";
    }
...
void CarPawnSimApi::updateRendering(float dt) {
    ...
    try {
        ...
        UAutonomyBlueprintLib::LogMessage(FString(e.what()), TEXT(""), LogDebugLevel::Failure, 30);
    }
}
...
void CarPawnSimApi::updateCarControls() {
    ...
    if (rc_data.is_initialized) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Control Mode: ", "Keyboard", LogDebugLevel::Informational);
    ...
    if (!vehicle_api_->isApiControlEnabled()) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Control Mode: ", "API", LogDebugLevel::Informational);
