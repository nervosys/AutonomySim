...
std::unique_ptr<nervosys::autonomylib::ApiServerBase> ASimModeCar::createApiServer() const {
#ifdef AIRLIB_NO_RPC
...
bool ASimModeCar::isVehicleTypeSupported(const std::string &vehicle_type) const {
    return ((vehicle_type == AutonomySimSettings::kVehicleTypePhysXCar) ||
            (vehicle_type == AutonomySimSettings::kVehicleTypeArduRover));
}

std::string ASimModeCar::getVehiclePawnPathName(const AutonomySimSettings::VehicleSetting &vehicle_setting) const {
...
std::unique_ptr<PawnSimApi> ASimModeCar::createVehicleSimApi(const PawnSimApi::Params &pawn_sim_api_params) const {
    auto vehicle_pawn = static_cast<TVehiclePawn *>(pawn_sim_api_params.pawn);
    auto vehicle_sim_api =
        std::unique_ptr<PawnSimApi>(new CarPawnSimApi(pawn_sim_api_params, vehicle_pawn->getKeyBoardControls()));
