...
void UnrealImageCapture::getSceneCaptureImage(
    ...
    for (unsigned int i = 0; i < requests.size(); ++i) {
        ...
        visibilityChanged =
            const_cast<UnrealImageCapture *>(this)->updateCameraVisibility(camera, requests[i]) || visibilityChanged;
    }
    ...
    for (unsigned int i = 0; i < requests.size(); ++i) {
        ...
        response.image_data_uint8 = std::vector<uint8_t>(render_results[i]->image_data_uint8.GetData(),
                                                         render_results[i]->image_data_uint8.GetData() +
                                                             render_results[i]->image_data_uint8.Num());
        response.image_data_float = std::vector<float>(render_results[i]->image_data_float.GetData(),
                                                       render_results[i]->image_data_float.GetData() +
                                                           render_results[i]->image_data_float.Num());

        if (use_safe_method) {
            // Currently, we don't have a way to synthronize image capturing and camera pose when safe method is used,
            APIPCamera *camera = cameras_->at(request.camera_name);
            nervosys::autonomylib::Pose pose = camera->getPose();
...
bool UnrealImageCapture::updateCameraVisibility(APIPCamera *camera,
    ...
    if (!is_installed) {
        ...
        ViewportClient->OnScreenshotCaptured().AddLambda(
            [this](int32 SizeX, int32 SizeY, const TArray<FColor> &Bitmap) {
                ...
                last_compressed_png_ = std::vector<uint8_t>(last_compressed_png.GetData(),
                                                            last_compressed_png.GetData() + last_compressed_png.Num());
            });
