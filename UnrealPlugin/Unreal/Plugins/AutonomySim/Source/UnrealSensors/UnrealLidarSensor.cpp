...
// ctor
UnrealLidarSensor::UnrealLidarSensor(const AutonomySimSettings::LidarSetting &setting, AActor *actor,
...
void UnrealLidarSensor::createLasers() {
    const nervosys::autonomylib::LidarSimpleParams params = getParams();
    ...
    if (number_of_lasers > 1)
        delta_angle =
            (params.vertical_FOV_upper - (params.vertical_FOV_lower)) / static_cast<float>(number_of_lasers - 1);
...
void UnrealLidarSensor::getPointCloud(const nervosys::autonomylib::Pose &lidar_pose,
    ...
    if (total_points_to_scan > MAX_POINTS_IN_SCAN) {
        ...
        UAutonomyBlueprintLib::LogMessageString("Lidar: ", "Capping number of points to scan", LogDebugLevel::Failure);
    }

    // calculate number of points needed for each laser/channel
    const uint32 points_to_scan_with_one_laser =
        FMath::RoundHalfFromZero(total_points_to_scan / float(number_of_lasers));
    if (points_to_scan_with_one_laser <= 0) {
        // UAutonomyBlueprintLib::LogMessageString("Lidar: ", "No points requested this frame", LogDebugLevel::Failure);
        return;
    }
    ...
    ParallelFor(total_jobs, [&](int32 idx) {
        ...
        const float horizontal_angle = std::fmod(current_horizontal_angle_ + angle_distance_of_laser_measure *
                                                                                 (idx % points_to_scan_with_one_laser),
                                                 360.0f);
    ...
    segmentation_cloud.erase(std::remove(segmentation_cloud.begin(), segmentation_cloud.end(), -1),
                             segmentation_cloud.end());
...
bool UnrealLidarSensor::shootLaser(const nervosys::autonomylib::Pose &lidar_pose,
    ...
    nervosys::autonomylib::Quaternionr ray_q_b = VectorMath::coordOrientationAdd(ray_q_l, lidar_pose.orientation);

    // get ray quaternion in world frame
    nervosys::autonomylib::Quaternionr ray_q_w = VectorMath::coordOrientationAdd(ray_q_b, vehicle_pose.orientation);
    ...
    if (is_hit) {
        ...
        if (false && UAutonomyBlueprintLib::IsInGameThread()) {
            // Debug code for very specific cases.
            // Mostly shouldn't be needed. Use SimModeBase::drawLidarDebugPoints()
            DrawDebugPoint(actor_->GetWorld(), hit_result.ImpactPoint,
                           5, // size
                           FColor::Red,
        ...
        case AutonomySimSettings::LidarSetting::DataFrame::VehicleInertialFrame:
            ...
            break;
        case AutonomySimSettings::LidarSetting::DataFrame::SensorLocalFrame:
