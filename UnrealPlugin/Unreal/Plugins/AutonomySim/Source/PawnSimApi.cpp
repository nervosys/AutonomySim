...
PawnSimApi::PawnSimApi(const Params &params) : params_(params), ned_transform_(params.pawn, *params.global_transform) {}
    ...
    if (getRemoteControlID() >= 0) {
        ...
        if (rc_data_.is_initialized)
            UAutonomyBlueprintLib::LogMessageString(
                "RC Controller on USB: ", joystick_state_.pid_vid == "" ? "(Detected)" : joystick_state_.pid_vid,
                LogDebugLevel::Informational);
        else
            UAutonomyBlueprintLib::LogMessageString(
                "RC Controller on USB not detected: ", std::to_string(joystick_state_.connection_error_code),
                LogDebugLevel::Informational);
    }
}
...
void PawnSimApi::setupCamerasFromSettings(const common_utils::UniqueValueMap<std::string, APIPCamera *> &cameras) {
    ...
    const auto &camera_defaults = AutonomySimSettings::singleton().camera_defaults;
...
void PawnSimApi::createCamerasFromSettings() {
    // UStaticMeshComponent* bodyMesh = UAutonomyBlueprintLib::GetActorComponent<UStaticMeshComponent>(this,
    // TEXT("BodyMesh"));
    ...
    for (const auto &camera_setting_pair : getVehicleSetting()->cameras) {
        ...
        APIPCamera *camera = params_.pawn->GetWorld()->SpawnActor<APIPCamera>(params_.pip_camera_class,
...
void PawnSimApi::onCollision(class UPrimitiveComponent *MyComp, class AActor *Other,
    ...
    UPrimitiveComponent *comp = Cast<class UPrimitiveComponent>(
        Other ? (Other->GetRootComponent() ? Other->GetRootComponent() : nullptr) : nullptr);
    ...
    state_.collision_info.time_stamp = nervosys::autonomylib::ClockFactory::get()->nowNanos();
...
const NedTransform &PawnSimApi::getNedTransform() const { return ned_transform_; }

APawn *PawnSimApi::getPawn() { return params_.pawn; }
...
nervosys::autonomylib::RCData PawnSimApi::getRCData() const {
    joystick_.getJoyStickState(getRemoteControlID(), joystick_state_);
    ...
    if (rc_data_.is_valid) {
        ...
        // front-upper-left, front-upper-right, top-right-left, top-right-left, top-left-right, top-right-right,
...
void PawnSimApi::displayCollisionEffect(FVector hit_location, const FHitResult &hit) {
    if (params_.collision_display_template != nullptr &&
        Utils::isDefinitelyLessThan<double>(hit.ImpactNormal.Z, 0.0f)) {
        UParticleSystemComponent *particles = UGameplayStatics::SpawnEmitterAtLocation(
            params_.pawn->GetWorld(), params_.collision_display_template, FTransform(hit_location), true);
        particles->SetWorldScale3D(FVector(0.1f, 0.1f, 0.1f));
    }
}

int PawnSimApi::getRemoteControlID() const { return getVehicleSetting()->rc.remote_control_id; }
...
APIPCamera *PawnSimApi::getCamera(const std::string &camera_name) {
    return const_cast<APIPCamera *>(static_cast<const PawnSimApi *>(this)->getCamera(camera_name));
}

const UnrealImageCapture *PawnSimApi::getImageCapture() const { return image_capture_.get(); }

int PawnSimApi::getCameraCount() { return cameras_.valsSize(); }

bool PawnSimApi::testLineOfSightToPoint(const nervosys::autonomylib::GeoPoint &lla) const {
    bool hit;
    ...
    UAutonomyBlueprintLib::RunCommandOnGameThread(
        [this, lla, &hit]() {
            ...
            NedTransform zero_based_ned_transform(FTransform::Identity,
                                                  UAutonomyBlueprintLib::GetWorldToMetersScale(params_.pawn));
            ...
            hit = params_.pawn->GetWorld()->LineTraceTestByChannel(params_.pawn->GetActorLocation(), target_location,
                                                                   ECC_Visibility, collision_params);

            // KM911 remove logging
            //		common_utils::Utils::log("NED from LLA: " + std::to_string(target_location.X) + ", " +
            // std::to_string(target_location.Y) + ", " + std::to_string(target_location.Z),
            // common_utils::Utils::kLogLevelInfo);

            if (AutonomySimSettings::singleton().show_los_debug_lines_) {
                if (hit) {
                    // No LOS, so draw red line
                    FLinearColor color{1.0f, 0, 0, 0.4f};
                    params_.pawn->GetWorld()->LineBatcher->DrawLine(params_.pawn->GetActorLocation(), target_location,
                                                                    color, SDPG_World, 10, -1);
                } else {
...
void PawnSimApi::resetImplementation() {
    ...
    rc_data_ = nervosys::autonomylib::RCData();
    params_.pawn->SetActorLocationAndRotation(state_.start_location, state_.start_rotation, false, nullptr,
...
void PawnSimApi::reportState(nervosys::autonomylib::StateReporter &reporter) {
    nervosys::autonomylib::VehicleSimApiBase::reportState(reporter);
...
PawnSimApi::CollisionInfo PawnSimApi::getCollisionInfo() const { return state_.collision_info; }
...
FRotator PawnSimApi::getUUOrientation() const { return params_.pawn->GetActorRotation(); }
...
void PawnSimApi::plot(std::istream &s, FColor color, const Vector3r &offset) {
    ...
    while (s >> timestamp >> heading >> x >> y >> z) {
        ...
        if (!VectorMath::hasNan(last_point)) {
            DrawDebugLine(params_.pawn->GetWorld(), ned_transform_.fromLocalNed(last_point),
                          ned_transform_.fromLocalNed(current_point), color, true, -1.0F, 0, 3.0F);
        }
...
PawnSimApi::Pose PawnSimApi::getPose() const { return toPose(getUUPosition(), getUUOrientation().Quaternion()); }
...
void PawnSimApi::setPose(const Pose &pose, bool ignore_collision) {
    UAutonomyBlueprintLib::RunCommandOnGameThread(
        [this, pose, ignore_collision]() { setPoseInternal(pose, ignore_collision); }, true);
}
...
void PawnSimApi::setPoseInternal(const Pose &pose, bool ignore_collision) {
    ...
    if (enable_teleport)
        params_.pawn->SetActorLocationAndRotation(position, orientation, false, nullptr,
                                                  ETeleportType::TeleportPhysics);
    else
    ...
    if (state_.tracing_enabled && (state_.last_position - position).SizeSquared() > 0.25) {
        DrawDebugLine(params_.pawn->GetWorld(), state_.last_position, position, trace_color_, true, -1.0F, 0,
                      trace_thickness_);
...
bool PawnSimApi::canTeleportWhileMove() const {
    ...
    return !state_.collisions_enabled ||
           (state_.collision_info.has_collided && !state_.was_last_move_teleport && state_.passthrough_enabled);
}
...
void PawnSimApi::updateKinematics(float dt) {
    ...
    next_kinematics.accelerations.angular = dt > 0
                                                ? (next_kinematics.twist.angular - kinematics_->getTwist().angular) / dt
                                                : next_kinematics.accelerations.angular;
...
const nervosys::autonomylib::Environment *PawnSimApi::getGroundTruthEnvironment() const { return environment_.get(); }
nervosys::autonomylib::Kinematics *PawnSimApi::getKinematics() { return kinematics_.get(); }
nervosys::autonomylib::Environment *PawnSimApi::getEnvironment() { return environment_.get(); }
...
std::string PawnSimApi::getRecordFileLine(bool is_header_line) const {
    ...
    ss << getVehicleName() << "\t";
    ss << timestamp_millis << "\t";
    ss << kinematics->pose.position.x() << "\t" << kinematics->pose.position.y() << "\t"
...
nervosys::autonomylib::VehicleApiBase *PawnSimApi::getVehicleApiBase() const { return nullptr; }
