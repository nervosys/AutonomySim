#ifndef autonomylib_vehicles_simpleflight_firmware_AngleLevelController_hpp
#define autonomylib_vehicles_simpleflight_firmware_AngleLevelController_hpp
...
class AngleLevelController : public IAxisController, public IGoal { // for internal rate controller
  ...
  public:
    AngleLevelController(Params *params, const IBoardClock *clock = nullptr) : params_(params), clock_(clock) {}

    virtual void initialize(unsigned int axis, const IGoal *goal, const IStateEstimator *state_estimator) override {
        if (axis > 2)
            throw std::invalid_argument("AngleLevelController only supports axis 0-2 but it was " +
                                        std::to_string(axis));
        ...
        pid_.reset(new PidController<float>(clock_, PidConfig<float>(params_->angle_level_pid.p[axis],
    ...
    /********************  IGoal ********************/
    virtual const Axis4r &getGoalValue() const override { return rate_goal_; }

    virtual const GoalMode &getGoalMode() const override { return rate_mode_; }
};
