...
#ifndef autonomylib_vehicles_ArduCopterApi_hpp
#define autonomylib_vehicles_ArduCopterApi_hpp
...
class ArduCopterApi : public MultirotorApiBase {
  ...
  private:
    ...
    void sendSensors() {
        ...
        buf << "\"imu\": {" << std::fixed << std::setprecision(7) << "\"angular_velocity\": ["
            << imu_output.angular_velocity[0] << "," << imu_output.angular_velocity[1] << ","
            << imu_output.angular_velocity[2] << "]"
            << ","
            << "\"linear_acceleration\": [" << imu_output.linear_acceleration[0] << ","
            << imu_output.linear_acceleration[1] << "," << imu_output.linear_acceleration[2] << "]"
            << "}";
        ...
        if (count_gps_sensors != 0) {
            ...
            buf << ","
                   "\"gps\": {"
                << std::fixed << std::setprecision(7) << "\"lat\": " << gps_output.gnss.geo_point.latitude << ","
                << "\"lon\": " << gps_output.gnss.geo_point.longitude << "," << std::setprecision(3)
                << "\"alt\": " << gps_output.gnss.geo_point.altitude << "},"

                << "\"velocity\": {"
                << "\"world_linear_velocity\": [" << gps_output.gnss.velocity[0] << "," << gps_output.gnss.velocity[1]
        ...
        if (is_rc_connected_ && last_rcData_.is_valid) {
            buf << ","
                   "\"rc\": {"
                   "\"channels\": ["
                << (last_rcData_.roll + 1) * 0.5f << "," << (last_rcData_.yaw + 1) * 0.5f << ","
                << (last_rcData_.throttle + 1) * 0.5f << "," << (-last_rcData_.pitch + 1) * 0.5f;
        ...
        if (count_distance_sensors != 0) {
            ...
            for (uint i = 0; i < count_distance_sensors; ++i) {
                const auto *distance_sensor =
                    static_cast<const DistanceSimple *>(sensors_->getByType(SensorBase::SensorType::Distance, i));
        ...
        if (count_lidars != 0) {
            ...
            for (uint i = 0; i < count_lidars; ++i) {
                const auto *lidar =
                    static_cast<const LidarSimple *>(sensors_->getByType(SensorBase::SensorType::Lidar, i));

                if (lidar && lidar->getParams().external_controller) {
                    const auto &lidar_output = lidar->getOutput();
                    std::copy(lidar_output.point_cloud.begin(), lidar_output.point_cloud.end(),
                              std::ostream_iterator<real_T>(buf, ","));
    ...
    void recvRotorControl() {
        ...
        while (recv_ret != sizeof(pkt)) {
            if (recv_ret <= 0) {
                Utils::log(Utils::stringf("Error while receiving rotor control data - ErrorNo: %d", recv_ret),
                           Utils::kLogLevelInfo);
            } else {
                Utils::log(Utils::stringf("Received %d bytes instead of %zu bytes", recv_ret, sizeof(pkt)),
                           Utils::kLogLevelInfo);
            }
  ...
  protected:
    ...
    virtual const MultirotorApiParams &getMultirotorApiParams() const override { return safety_params_; }
    ...
    void connect() {
        ...
        Utils::log(Utils::stringf("Using UDP port %d, local IP %s, remote IP %s for sending sensor data", port_,
                                  connection_info_.local_host_ip.c_str(), ip_.c_str()),
                   Utils::kLogLevelInfo);
        Utils::log(Utils::stringf("Using UDP port %d for receiving rotor power", connection_info_.control_port_local,
                                  connection_info_.local_host_ip.c_str(), ip_.c_str()),
                   Utils::kLogLevelInfo);
  ...
  public:
    ArduCopterApi(const MultirotorParams *vehicle_params,
                  const AutonomySimSettings::MavLinkConnectionInfo &connection_info)
    ...
    ~ArduCopterApi() { closeConnections(); }
    ...
    virtual void getStatusMessages(std::vector<std::string> &messages) override { unused(messages); }

    virtual const SensorCollection &getSensors() const override { return vehicle_params_->getSensors(); }

    // TODO:MultirotorApiBase implementation
    virtual real_T getActuation(unsigned int rotor_index) const override { return rotor_controls_[rotor_index]; }

    virtual size_t getActuatorCount() const override { return vehicle_params_->getParams().rotor_count; }

    virtual void moveByRC(const RCData &rc_data) override { setRCData(rc_data); }
