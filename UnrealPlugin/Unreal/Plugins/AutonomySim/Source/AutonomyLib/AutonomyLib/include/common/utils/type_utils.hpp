...
#ifndef common_utils_type_utils_hpp
#define common_utils_type_utils_hpp
...
template <typename T> struct has_begin_end : private sfinae_base {
  ...
  private:
    template <typename C>
    static yes &
    f(typename std::enable_if<std::is_same<decltype(static_cast<typename C::const_iterator (C::*)() const>(&C::begin)),
    ...
    template <typename D>
    static yes &
    g(typename std::enable_if<std::is_same<decltype(static_cast<typename D::const_iterator (D::*)() const>(&D::end)),
...
template <typename T>
struct is_container
    : public std::integral_constant<bool, detail::has_const_iterator<T>::value && detail::has_begin_end<T>::beg_value &&
