#ifndef autonomylib_vehicles_simpleflight_firmware_OffboardApi_hpp
#define autonomylib_vehicles_simpleflight_firmware_OffboardApi_hpp
...
class OffboardApi : public IUpdatable, public IOffboardApi {
  ...
  private:
    ...
    bool isGreaterThanArmedThrottle(float throttle) { return throttle > params_->min_armed_throttle(); }
  ...
  public:
    ...
    virtual void update() override {
        ...
        if (!has_api_control_)
            ...
            if (takenoff_ && (clock_->millis() - goal_timestamp_ > params_->api_goal_timeout)) {
                if (!is_api_timedout_) {
    ...
    virtual const Axis4r &getGoalValue() const override { return goal_; }
    ...
    virtual bool hasApiControl() override { return has_api_control_; }
    ...
    virtual VehicleStateType getVehicleState() const override { return vehicle_state_.getState(); }

    virtual const IStateEstimator &getStateEstimator() override { return *state_estimator_; }

    virtual GeoPoint getHomeGeoPoint() const override { return state_estimator_->getHomeGeoPoint(); }

    virtual GeoPoint getGeoPoint() const override { return state_estimator_->getGeoPoint(); }

    virtual bool getLandedState() const override { return landed_; }
};
