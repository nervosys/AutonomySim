...
struct GeoPoint {
    ...
    std::string to_string() const {
        return std::to_string(latitude) + string(", ") + std::to_string(longitude) + string(", ") +
...
struct HomeGeoPoint {
    ...
    HomeGeoPoint() {}
    HomeGeoPoint(const GeoPoint &home_geo_point_val) { initialize(home_geo_point_val); }
...
struct CollisionInfo {
    ...
    CollisionInfo(bool has_collided_val, const Vector3r &normal_val, const Vector3r &impact_point_val,
                  const Vector3r &position_val, real_T penetration_depth_val, TTimePoint time_stamp_val,
                  const std::string &object_name_val, int object_id_val)
        : has_collided(has_collided_val), normal(normal_val), impact_point(impact_point_val), position(position_val),
          penetration_depth(penetration_depth_val), time_stamp(time_stamp_val), object_name(object_name_val),
...
struct Box2D {
    ...
    Box2D(Vector2r min_val, Vector2r max_val) : min(min_val), max(max_val) {}
};
...
struct Box3D {
    ...
    Box3D(Vector3r min_val, Vector3r max_val) : min(min_val), max(max_val) {}
};
...
struct DetectionInfo {
    ...
    DetectionInfo(const std::string &name_val, const GeoPoint &geo_point_val, const Box2D &box2D_val,
                  const Box3D &box3D_val, const Pose &relative_pose_val)
        : name(name_val), geo_point(geo_point_val), box2D(box2D_val), box3D(box3D_val),
          relative_pose(relative_pose_val) {}
};
...
struct RCData {
    ...
    unsigned int getSwitch(uint16_t index) const { return switches & (1 << index) ? 1 : 0; }
...
struct CameraDetails {
    ...
    std::string to_string() const {
        return Utils::stringf("CameraDetails: camera_name=%s, vehicle_name=%s, external=%d", camera_name.c_str(),
                              vehicle_name.c_str(), external);
    }
};
