...
#ifndef autonomylib_vehicles_SimpleFlightBoard_hpp
#define autonomylib_vehicles_SimpleFlightBoard_hpp
...
class SimpleFlightBoard : public simple_flight::IBoard {
  ...
  public:
    ...
    void setGroundTruthKinematics(const Kinematics::State *kinematics) { kinematics_ = kinematics; }
    ...
    void setInputChannel(uint index, real_T val) { input_channels_[index] = static_cast<float>(val); }
    ...
    virtual uint64_t millis() const override { return clock()->nowNanos() / 1000000; }

    virtual float readChannel(uint16_t index) const override { return input_channels_[index]; }

    virtual float getAvgMotorOutput() const override {
        return ((getMotorControlSignal(0) + getMotorControlSignal(1) + getMotorControlSignal(2) +
                 getMotorControlSignal(3)) /
                4);
    }

    virtual bool isRcConnected() const override { return is_connected_; }

    virtual void writeOutput(uint16_t index, float value) override { motor_output_[index] = value; }
    ...
    virtual void readAccel(float accel[3]) const override {
        const auto &linear_accel =
            VectorMath::transformToBodyFrame(kinematics_->accelerations.linear, kinematics_->pose.orientation);
