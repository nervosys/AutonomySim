...
class EarthUtils {
  ...
  private:
    ...
    static constexpr int DECLINATION_TABLE[13][37] = {
        {46,  45,  44,  42,  41,  40,  38,  36,  33,  28,  23,  16,  10,  4,  -1, -5, -9, -14, -19,
         -26, -33, -40, -48, -55, -61, -66, -71, -74, -75, -72, -61, -25, 22, 40, 45, 47, 46},
        {30,  30,  30,  30,  29,  29,  29,  29,  27,  24,  18,  11,  3, -3, -9, -12, -15, -17, -21,
         -26, -32, -39, -45, -51, -55, -57, -56, -53, -44, -31, -14, 0, 13, 21, 26,  29,  30},
        {21,  22,  22,  22,  22,  22,  22,  22,  21,  18,  13,  5,  -3, -11, -17, -20, -21, -22, -23,
         -25, -29, -35, -40, -44, -45, -44, -40, -32, -22, -12, -3, 3,  9,   14,  18,  20,  21},
        {16,  17,  17,  17,  17,  17,  16,  16,  16,  13, 8,  0, -9, -16, -21, -24, -25, -25, -23,
         -20, -21, -24, -28, -31, -31, -29, -24, -17, -9, -3, 0, 4,  7,   10,  13,  15,  16},
        {12,  13, 13,  13,  13,  13,  12,  12,  11, 9,  3, -4, -12, -19, -23, -24, -24, -22, -17,
         -12, -9, -10, -13, -17, -18, -16, -13, -8, -3, 0, 1,  3,   6,   8,   10,  12,  12},
        {10, 10, 10, 10, 10, 10, 10, 9,  9,  6, 0, -6, -14, -20, -22, -22, -19, -15, -10,
         -6, -2, -2, -4, -7, -8, -8, -7, -4, 0, 1, 1,  2,   4,   6,   8,   10,  10},
        {9,  9, 9, 9, 9,  9,  8,  8,  7,  4, -1, -8, -15, -19, -20, -18, -14, -9, -5,
         -2, 0, 1, 0, -2, -3, -4, -3, -2, 0, 0,  0,  1,   3,   5,   7,   8,   9},
        {8, 8, 8, 9, 9, 9,  8,  8,  6,  2, -3, -9, -15, -18, -17, -14, -10, -6, -2,
         0, 1, 2, 2, 0, -1, -1, -2, -1, 0, 0,  0,  0,   1,   3,   5,   7,   8},
        ...
        {4, 8, 12, 15, 17, 18, 16, 12, 5, -3, -12, -18, -20, -19, -16, -13, -8, -4, -1,
         1, 4, 6,  8,  9,  9,  9,  7,  3, -1, -6,  -10, -12, -11, -9,  -5,  0,  4},
  ...
  public:
    ...
    static real_T getMagDeclination(real_T latitude, real_T longitude) {
        ...
        if (latitude < -90.0f || latitude > 90.0f || longitude < -180.0f || longitude > 180.0f) {
            throw std::out_of_range(
                Utils::stringf("invalid latitude (%f) or longitude (%f) value", latitude, longitude));
        }
        ...
        real_T declination_min =
            ((longitude - min_lon) / MAG_SAMPLING_RES) * (declination_se - declination_sw) + declination_sw;
        real_T declination_max =
            ((longitude - min_lon) / MAG_SAMPLING_RES) * (declination_ne - declination_nw) + declination_nw;
    ...
    static Vector3r getMagField(const GeoPoint &geo_point, double &declination, double &inclination) { // return Tesla
        ...
        */
        ...
        double mag_clat = acos(cos_lat * cos_MagPoleLat + sin_lat * sin_MagPoleLat * cos(lon - MagPoleLon));
        ...
        double field_mag = MeanMagField * pow(EARTH_RADIUS / altitude, 3) * sqrt(1 + 3 * cos_mag_clat * cos_mag_clat);
        ...
        return Vector3r(static_cast<real_T>(field_xy * cos(declination)),
    ...
    static GeoPoint nedToGeodetic(const Vector3r &v, const HomeGeoPoint &home_geo_point) {
        ...
        if (!Utils::isApproximatelyZero(c)) { // avoids large changes?
            ...
            lon_rad = (home_geo_point.lon_rad + atan2(y_rad * sin_c, c * home_geo_point.cos_lat * cos_c -
    ...
    static Vector3r GeodeticToEcef(const GeoPoint &geo) {
        ...
        real_T z =
            static_cast<real_T>((EARTH_RADIUS / xi * (1 - (6.69437999014 * 0.001)) + geo.altitude) * sin(lat_rad));
    ...
    static Vector3r GeodeticToNedFast(const GeoPoint &geo, const GeoPoint &home) {
        ...
        real_T y = static_cast<real_T>(Utils::degreesToRadians(d_lon) * EARTH_RADIUS *
                                       cos(Utils::degreesToRadians(geo.latitude)));
    ...
    static constexpr double MeanMagField =
        3.12E-5; // Tesla, https://en.wikipedia.org/wiki/Dipole_model_of_the_Earth's_magnetic_field
