...
#ifndef autonomylib_vehicles_SimpleFlightApi_hpp
#define autonomylib_vehicles_SimpleFlightApi_hpp

#include "SimpleFlightBoard.hpp"
#include "SimpleFlightCommLink.hpp"
#include "SimpleFlightCommon.hpp"
#include "SimpleFlightEstimator.hpp"
...
class SimpleFlightApi : public MultirotorApiBase {
  ...
  private:
    ...
    unique_ptr<SimpleFlightBoard> board_;
    unique_ptr<SimpleFlightCommLink> comm_link_;
    unique_ptr<SimpleFlightEstimator> estimator_;
    unique_ptr<simple_flight::IFirmware> firmware_;
    MultirotorApiParams safety_params_;
    RCData last_rcData_;

    // convert pitch, roll, yaw from -1 to 1 to PWM
    static uint16_t angleToPwm(float angle) { return static_cast<uint16_t>(angle * 500.0f + 1500.0f); }
    ...
    void readSettings(const AutonomySimSettings::VehicleSetting &vehicle_setting) {
  ...
  protected:
    virtual Kinematics::State getKinematicsEstimated() const override {
        return SimpleFlightCommon::toKinematicsState3r(
    ...
    virtual Vector3r getPosition() const override {
        ...
        return SimpleFlightCommon::toVector3r(val);
    }
    ...
    virtual Quaternionr getOrientation() const override {
        ...
        return SimpleFlightCommon::toQuaternion(val);
    }
    ...
    virtual GeoPoint getGpsLocation() const override {
        return SimpleFlightCommon::toGeoPoint(firmware_->offboardApi().getGeoPoint());
    }
    ...
    virtual void commandMotorPWMs(float front_right_pwm, float rear_left_pwm, float front_left_pwm,
                                  float rear_right_pwm) override {
        // Utils::log(Utils::stringf("commandMotorPWMs %f, %f, %f, %f", front_right_pwm, rear_left_pwm, front_left_pwm,
        // rear_right_pwm));

        typedef simple_flight::GoalModeType GoalModeType;
        simple_flight::GoalMode mode(GoalModeType::Passthrough, GoalModeType::Passthrough, GoalModeType::Passthrough,
    ...
    virtual void commandRollPitchYawZ(float roll, float pitch, float yaw, float z) override {
        ...
        typedef simple_flight::GoalModeType GoalModeType;
        simple_flight::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleLevel,
    ...
    virtual void commandRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle) override {
        ...
        typedef simple_flight::GoalModeType GoalModeType;
        simple_flight::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleRate,
                                     GoalModeType::Passthrough);
    ...
    virtual void commandAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate, float z) override {
        ...
        typedef simple_flight::GoalModeType GoalModeType;
        simple_flight::GoalMode mode(GoalModeType::AngleRate, GoalModeType::AngleRate, GoalModeType::AngleRate,
                                     GoalModeType::PositionWorld);
    ...
    virtual void setControllerGains(uint8_t controller_type, const vector<float> &kp, const vector<float> &ki,
  ...
  public:
    SimpleFlightApi(const MultirotorParams *vehicle_params, const AutonomySimSettings::VehicleSetting *vehicle_setting)
        ...
        board_.reset(new SimpleFlightBoard(&params_));
        comm_link_.reset(new SimpleFlightCommLink());
        estimator_.reset(new SimpleFlightEstimator());
    ...
    virtual bool isApiControlEnabled() const override { return firmware_->offboardApi().hasApiControl(); }
    ...
    virtual GeoPoint getHomeGeoPoint() const override {
        return SimpleFlightCommon::toGeoPoint(firmware_->offboardApi().getHomeGeoPoint());
    }
    ...
    // MultirotorApiBase implementation
