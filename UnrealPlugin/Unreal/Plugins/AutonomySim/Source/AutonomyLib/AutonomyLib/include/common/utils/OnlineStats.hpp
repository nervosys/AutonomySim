...
class OnlineStats {
  ...
  public:
    ...
    double variance() const { return m2 / (n - 1.0f); }

    double standardDeviation() const { return sqrt(variance()); }

    double skewness() const { return sqrt(double(n)) * m3 / pow(m2, 1.5f); }

    double kurtosis() const { return double(n) * m4 / (m2 * m2) - 3.0f; }
    ...
    OnlineStats operator+(const OnlineStats &rhs) {
        ...
        combined.m2 = this->m2 + rhs.m2 + delta2 * this->n * rhs.n / combined.n;

        combined.m3 = this->m3 + rhs.m3 + delta3 * this->n * rhs.n * (this->n - rhs.n) / (combined.n * combined.n);
        combined.m3 += 3.0f * delta * (this->n * rhs.m2 - rhs.n * this->m2) / combined.n;

        combined.m4 = this->m4 + rhs.m4 +
                      delta4 * this->n * rhs.n * (this->n * this->n - this->n * rhs.n + rhs.n * rhs.n) /
                          (combined.n * combined.n * combined.n);
        combined.m4 +=
            6.0f * delta2 * (this->n * this->n * rhs.m2 + rhs.n * rhs.n * this->m2) / (combined.n * combined.n) +
