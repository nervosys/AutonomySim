#ifndef autonomylib_vehicles_simpleflight_firmware_Mixer_hpp
#define autonomylib_vehicles_simpleflight_firmware_Mixer_hpp
  ...
  public:
    ...
    void getMotorOutput(const Axis4r &controls, std::vector<float> &motor_outputs) const {
        ...
        for (int motor_index = 0; motor_index < kMotorCount; ++motor_index) {
            motor_outputs[motor_index] = controls.throttle() * mixerQuadX[motor_index].throttle +
