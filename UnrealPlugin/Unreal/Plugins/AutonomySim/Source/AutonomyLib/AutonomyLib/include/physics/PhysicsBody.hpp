...
#ifndef autonomylib_physics_PhysicsBody_hpp
#define autonomylib_physics_PhysicsBody_hpp
...
class PhysicsBody : public UpdatableObject {
  ...
  public:                            // interface
    TTimePoint last_kinematics_time; // for use in physics engine: //TODO: use getter/setter or friend method?

    virtual real_T getRestitution() const = 0;
    virtual real_T getFriction() const = 0;

    // derived class may return covariant type
    virtual uint wrenchVertexCount() const { return 0; }
    ...
    virtual uint dragVertexCount() const { return 0; }
    ...
    virtual void setCollisionInfo(const CollisionInfo &collision_info) { collision_info_ = collision_info; }
     ...
     */
    virtual void updateKinematics() { kinematics_->update(); }
    ...
    virtual UpdatableObject *getPhysicsBody() override { return this; }
    ...
    real_T getMassInv() const { return mass_inv_; }

    const Matrix3x3r &getInertia() const { return inertia_; }
    const Matrix3x3r &getInertiaInv() const { return inertia_inv_; }

    const Pose &getPose() const { return kinematics_->getPose(); }
    void setPose(const Pose &pose) { return kinematics_->setPose(pose); }

    const Twist &getTwist() const { return kinematics_->getTwist(); }
    void setTwist(const Twist &twist) { return kinematics_->setTwist(twist); }

    const Kinematics::State &getKinematics() const { return kinematics_->getState(); }
    const Kinematics::State &getInitialKinematics() const { return kinematics_->getInitialState(); }
    ...
    const Wrench &getWrench() const { return wrench_; }
    void setWrench(const Wrench &wrench) { wrench_ = wrench; }

    const CollisionInfo &getCollisionInfo() const { return collision_info_; }
    const CollisionResponse &getCollisionResponseInfo() const { return collision_response_; }
    CollisionResponse &getCollisionResponseInfo() { return collision_response_; }
