...
#ifndef autonomylib_vehicles_MultirotorApiBase_hpp
#define autonomylib_vehicles_MultirotorApiBase_hpp
...
class MultirotorApiBase : public VehicleApiBase {
  ...
  protected:
    ...
    virtual float getTakeoffZ() const = 0; // the height above ground for the drone after successful takeoff (Z above
                                           // ground is negative due to NED coordinate system).
    // noise in difference of two position coordinates. This is not GPS or position accuracy which can be very low such
    // as 1m. the difference between two position cancels out transitional errors. Typically this would be 0.1m or
    // lower.
  ...
  public:
    ...
    virtual bool moveToGPS(float latitude, float longitude, float altitude, float velocity, float timeout_sec,
                           DrivetrainType drivetrain, const YawMode &yaw_mode, float lookahead,
                           float adaptive_lookahead);
    virtual bool moveByVelocityBodyFrame(float vx, float vy, float vz, float duration, DrivetrainType drivetrain,
                                         const YawMode &yaw_mode);
    virtual bool moveByVelocityZBodyFrame(float vx, float vy, float z, float duration, DrivetrainType drivetrain,
                                          const YawMode &yaw_mode);
    virtual bool moveByMotorPWMs(float front_right_pwm, float rear_left_pwm, float front_left_pwm, float rear_right_pwm,
                                 float duration);
    ...
    virtual bool moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle,
                                                float duration);
    ...
    virtual bool moveByAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle,
                                          float duration);
    virtual bool moveByVelocity(float vx, float vy, float vz, float duration, DrivetrainType drivetrain,
                                const YawMode &yaw_mode);
    virtual bool moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain,
                                 const YawMode &yaw_mode);
    virtual bool moveOnPath(const vector<Vector3r> &path, float velocity, float timeout_sec, DrivetrainType drivetrain,
    ...
    virtual bool moveToPosition(float x, float y, float z, float velocity, float timeout_sec, DrivetrainType drivetrain,
                                const YawMode &yaw_mode, float lookahead, float adaptive_lookahead);
    virtual bool moveToZ(float z, float velocity, float timeout_sec, const YawMode &yaw_mode, float lookahead,
                         float adaptive_lookahead);
    virtual bool moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain,
                              const YawMode &yaw_mode);
    ...
    /************************* set angle gain APIs *********************************/
    virtual void setAngleLevelControllerGains(const vector<float> &kp, const vector<float> &ki,
    ...
    virtual bool setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance,
    ...
    const RotorStates &getRotorStates() const { return rotor_states_; }
    ...
    virtual void cancelLastTask() override { token_.cancel(); }
