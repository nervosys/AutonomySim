...
#ifndef autonomylib_vehicles_MultirotorParams_hpp
#define autonomylib_vehicles_MultirotorParams_hpp
...
class MultirotorParams {
  ...
  protected:
    // must override by derived class
    virtual void setupParams() = 0;
    virtual const SensorFactory *getSensorFactory() const = 0;

    // static utility functions for derived classes to use
    ...
    /// the second is placed bottom left, and also flies CCW.  The third in the array is placed top left and flies
    /// clockwise (CW) while the last is placed bottom right and also flies clockwise.  This is how the 4 items in the
    /// arm_lengths and arm_angles arrays will be used.  So arm_lengths is 4 numbers (in meters) where four arm lengths,
    /// 0 is top right, 1 is bottom left, 2 is top left and 3 is bottom right.  arm_angles is 4 numbers (in degrees)
    /// relative to forward vector (0,1), provided in the same order where 0 is top right, 1 is bottom left, 2 is top
    /// left and 3 is bottom right, so for example, the angles for a regular symmetric X pattern would be 45, 225, 315,
    /// 135.  The rotor_z is the offset of each motor upwards relative to the center of mass (in meters).
    static void initializeRotorQuadX(vector<RotorPose> &rotor_poses /* the result we are building */,
                                     uint rotor_count /* must be 4 */, real_T arm_lengths[],
    ...
    static void initializeRotorHexX(vector<RotorPose> &rotor_poses /* the result we are building */,
                                    uint rotor_count /* must be 6 */, real_T arm_lengths[],
                                    real_T rotor_z /* z relative to center of gravity */) {
        Vector3r unit_z(0, 0, -1); // NED frame
        if (rotor_count == 6) {
            rotor_poses.clear();
    ...
    static void initializeRotorOctoX(vector<RotorPose> &rotor_poses /* the result we are building */,
                                     uint rotor_count /* must be 8 */, real_T arm_lengths[],
                                     real_T rotor_z /* z relative to center of gravity */) {
        Vector3r unit_z(0, 0, -1); // NED frame
        if (rotor_count == 8) {
            rotor_poses.clear();
        ...
        */
            ...
            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(0, arm_lengths[0], rotor_z), octo_rot67, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(0, -arm_lengths[1], rotor_z), octo_rot67, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(0, arm_lengths[2], rotor_z), octo_rot22, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(-arm_lengths[3], 0, rotor_z), octo_rot22, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(arm_lengths[4], 0, rotor_z), octo_rot22, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(0, -arm_lengths[5], rotor_z), octo_rot22, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(arm_lengths[6], 0, rotor_z), octo_rot67, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCW);

            rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(-arm_lengths[7], 0, rotor_z), octo_rot67, true),
                                     unit_z, RotorTurningDirection::RotorTurningDirectionCW);
        } else
            throw std::invalid_argument("Rotor count other than 8 is not supported by this method!");
    }

    /// Initialize the rotor_poses given the rotor_count, the arm lengths and the arm angles (relative to forwards
    /// vector). Also provide the direction you want to spin each rotor and the z-offset of the rotors relative to the
    /// center of gravity.
    static void initializeRotors(vector<RotorPose> &rotor_poses, uint rotor_count, real_T arm_lengths[],
                                 real_T arm_angles[], RotorTurningDirection rotor_directions[],
                                 real_T rotor_z /* z relative to center of gravity */) {
        Vector3r unit_z(0, 0, -1); // NED frame
        rotor_poses.clear();
    ...
    static void computeInertiaMatrix(Matrix3x3r &inertia, const Vector3r &body_box,
                                     const vector<RotorPose> &rotor_poses, real_T box_mass,
    ...
    void setupFrameGenericQuad(Params &params) {
        // set up arm lengths
        // dimensions are for F450 frame:
        // http://artofcircuits.com/product/quadcopter-frame-hj450-with-power-distribution
        ...
        // (idle throttle percentage)*max_thrust*rotor_count/10 (0.8kg using default parameters and SimpleFlight) any
        // value above the maximum would result in the motors not being able to lift the body even at max thrust, and
        // any value below the minimum would cause the drone to fly upwards on idling throttle (50% of the max throttle
        // if using SimpleFlight) Note that the default idle throttle percentage is 50% if you are using SimpleFlight
        params.mass = 1.0f;

        real_T motor_assembly_weight = 0.055f; // weight for MT2212 motor for F450 frame
        real_T box_mass = params.mass - params.rotor_count * motor_assembly_weight;

        // using rotor_param default, but if you want to change any of the rotor_params, call calculateMaxThrust() to
        // recompute the max_thrust given new thrust coefficients, motor max_rpm and propeller diameter.
    ...
    void setupFrameGenericHex(Params &params) {
        ...
        // set up mass
        // this has to be between max_thrust*rotor_count/10 (2.5kg using default parameters in RotorParams.hpp) and
        // (idle throttle percentage)*max_thrust*rotor_count/10 (1.25kg using default parameters and SimpleFlight) any
        // value above the maximum would result in the motors not being able to lift the body even at max thrust, and
        // any value below the minimum would cause the drone to fly upwards on idling throttle (50% of the max throttle
        // if using SimpleFlight)
        params.mass = 1.0f;
    ...
    */
        ...
        // note: the Forward vector is actually the "x" axis, and the AngleAxisr rotation is pointing down and is left
        // handed, so this means the rotation is counter clockwise, so the vector (arm_lengths[i], 0) is the X-axis, so
        // the CCW rotations to position each arm correctly are listed below: See measurements here:
        // http://diydrones.com/profiles/blogs/arducopter-tbs-discovery-style (angles reversed because we are doing CCW
        // rotation)
        ...
        for (uint i = 0; i < 4; i++) {
            Quaternionr angle(AngleAxisr(arm_angles[i] * M_PIf / 180, unit_z));
            params.rotor_poses.emplace_back(VectorMath::rotateVector(Vector3r(arm_lengths[i], 0, rotor_z), angle, true),
                                            unit_z, rotor_directions[i]);
        };
  ...
  public:
    ...
    virtual void initialize(const AutonomySimSettings::VehicleSetting *vehicle_setting) {
    ...
    const SensorCollection &getSensors() const { return sensors_; }

    void addSensorsFromSettings(const AutonomySimSettings::VehicleSetting *vehicle_setting) {
