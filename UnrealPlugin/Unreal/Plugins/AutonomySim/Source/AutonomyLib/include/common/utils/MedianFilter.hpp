...
template <typename T> void MedianFilter<T>::initialize(int window_size, float outlier_factor) {
...
template <typename T> MedianFilter<T>::MedianFilter() {
    initialize(buffer_.capacity(), std::numeric_limits<float>::infinity());
}

template <typename T> MedianFilter<T>::MedianFilter(int window_size, float outlier_factor) {
    initialize(window_size, std::numeric_limits<float>::infinity());
}

template <typename T> std::tuple<double, double> MedianFilter<T>::filter(T value) {
