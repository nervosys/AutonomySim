...
#ifndef autonomylib_sensors_MagnetometerSimpleParams_hpp
#define autonomylib_sensors_MagnetometerSimpleParams_hpp
...
struct MagnetometerSimpleParams {
    ...
    Vector3r noise_sigma =
        Vector3r(0.005f, 0.005f, 0.005f); // 5 mgauss as per specs sheet (RMS is same as stddev) https://goo.gl/UOz6FT
    real_T scale_factor = 1.0f;
    Vector3r noise_bias =
        Vector3r(0.0f, 0.0f, 0.0f);    // no offset as per specsheet (zero gauss level) https://goo.gl/UOz6FT
    ...
    void initializeFromSettings(const AutonomySimSettings::MagnetometerSetting &settings) {
