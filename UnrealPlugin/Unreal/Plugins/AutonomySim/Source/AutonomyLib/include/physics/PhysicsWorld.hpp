...
#ifndef autonomylib_physics_PhysicsWorld_hpp
#define autonomylib_physics_PhysicsWorld_hpp
...
class PhysicsWorld : UpdatableObject {
  public:
    ...
    void lock() { world_.lock(); }

    void unlock() { world_.unlock(); }
    ...
    uint64_t getUpdatePeriodNanos() const { return update_period_nanos_; }

    void startAsyncUpdator() { world_.startAsyncUpdator(update_period_nanos_); }
    void stopAsyncUpdator() { world_.stopAsyncUpdator(); }

    void enableStateReport(bool is_enabled) { reporter_.setEnable(is_enabled); }
    ...
    std::string getDebugReport() { return reporter_.getOutput(); }

    void pause(bool is_paused) { world_.pause(is_paused); }

    bool isPaused() const { return world_.isPaused(); }

    void continueForTime(double seconds) { world_.continueForTime(seconds); }

    void continueForFrames(uint32_t frames) { world_.continueForFrames(frames); }

    void setFrameNumber(uint32_t frameNumber) { world_.setFrameNumber(frameNumber); }
