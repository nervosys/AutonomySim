#ifndef autonomylib_vehicles_simpleflight_firmware_CommonStructures_hpp
#define autonomylib_vehicles_simpleflight_firmware_CommonStructures_hpp
...
template <typename T> class Axis4 : public Axis3<T> {
  public:
    ...
    Axis4(const Axis3<T> &axis3_val, const T &val4_val = T()) : Axis3<T>(axis3_val), val4_(val4_val) {}
  ...
  public:
    ...
    static VehicleStateType fromString(const std::string &val) {
        ...
        throw std::invalid_argument(
            std::string("The value cannot be converted to VehicleStateType enum: ").append(val));
    }
...
class GoalMode : public Axis4<GoalModeType> {
  public:
    ...
    static const GoalMode &getVelocityXYPosZMode() {
        static const GoalMode mode = GoalMode(GoalModeType::VelocityWorld, GoalModeType::VelocityWorld,
                                              GoalModeType::AngleRate, GoalModeType::PositionWorld);
        return mode;
    }

    static const GoalMode &getVelocityMode() {
        static const GoalMode mode = GoalMode(GoalModeType::VelocityWorld, GoalModeType::VelocityWorld,
                                              GoalModeType::AngleRate, GoalModeType::VelocityWorld);
        return mode;
    }

    static const GoalMode &getPositionMode() {
        static const GoalMode mode = GoalMode(GoalModeType::PositionWorld, GoalModeType::PositionWorld,
                                              GoalModeType::AngleRate, GoalModeType::PositionWorld);
        return mode;
    }
    ...
    static const GoalMode &getUnknown() {
        static const GoalMode mode =
            GoalMode(GoalModeType::Unknown, GoalModeType::Unknown, GoalModeType::Unknown, GoalModeType::Unknown);
        return mode;
    }
};

// config params for PID controller
template <class T> struct PidConfig {
    PidConfig(float kp_val = 0.01f, float ki_val = 0.0f, float kd_val = 0.0f, T min_output_val = -1,
              T max_output_val = 1, float time_scale_val = 1.0f / 1000, bool enabled_val = true,
              T output_bias_val = T(), float iterm_discount_val = 1)
        : kp(kp_val), ki(ki_val), kd(kd_val), time_scale(time_scale_val), min_output(min_output_val),
          max_output(max_output_val), enabled(enabled_val), output_bias(output_bias_val),
    ...
    enum class IntegratorType { Standard, RungKutta };
