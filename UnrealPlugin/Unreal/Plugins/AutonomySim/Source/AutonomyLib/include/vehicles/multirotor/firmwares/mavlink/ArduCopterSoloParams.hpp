...
#ifndef autonomylib_vehicles_ArduCopterSolo_hpp
#define autonomylib_vehicles_ArduCopterSolo_hpp
...
class ArduCopterSoloParams : public MultirotorParams {
  ...
  public:
    ArduCopterSoloParams(const AutonomySimSettings::MavLinkVehicleSetting &vehicle_settings,
    ...
    virtual void setupParams() override {
        // TODO consider whether we want to make us of the 'connection_info_.model' field (perhaps to indicate different
        // sensor configs, say?  not sure...)
  ...
  private:
    void setupSolo(Params &params) {
        ...
        // This is in meters (3DR spec indicates that motor-to-motor is 18.1 inches, so we're just dividing that by 2,
        // which looks about right in real life)
        ...
        // params.mass = 1.5f;
        // And this is what works currently - TODO sort out why ArduPilot isn't providing sufficient rotor power for
        // default Solo weight - presumably some param setting?
        params.mass = 0.8f;
        ...
        // Dimensions of core body box or abdomen, in meters (not including arms).  KM measures the Solo at 9.5" x 4.5"
        // x 3"
    ...
    getConnectionInfo(const AutonomySimSettings::MavLinkVehicleSetting &vehicle_setting) {
        AutonomySimSettings::MavLinkConnectionInfo result = vehicle_setting.connection_info;
        return result;
    }
