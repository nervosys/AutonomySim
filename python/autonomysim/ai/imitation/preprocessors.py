...
# upper bound for normalizing the car's speed between 0 and 1
MAX_SPEED = 70.0
...
def generateDataMapAutonomySim(folders, velocity_max=MAX_SPEED):
    """
    Data map generator for simulator(AutonomySim) data. Reads the driving_log csv file and
    returns a list of 'center camera image name - label(s)' tuples.
    ...
    for folder in folders:
        ...
        for i in range(1, current_df.shape[0] - 1):
            ...
            norm_speed = [
                float(current_df.iloc[i - 1][["Speed (kmph)"]]) / velocity_max
...
def preprocess_data(
    ...
    # ...
    if all:
        data_folders = [name for name in os.listdir(input_dir)]

    full_path_raw_folders = [os.path.join(input_dir, f) for f in data_folders]
    cook(full_path_raw_folders, output_dir, train_eval_test_split, chunk_size)
    return None
