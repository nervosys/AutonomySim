...
class DriveIterator(image.Iterator):
    ...
    def __init__(
        ...
        super(DriveIterator, self).__init__(
            x_images.shape[0], batch_size, shuffle, seed
        )

    def next(self):
        """For python 2.x. DEPRECATE.
    ...
    def __get_indexes(self, index_array):
        ...
        if self.x_prev_states is not None:
            batch_x_images = np.zeros(
                tuple([self.cmdch_size] + list(self.x_images.shape)[1:]),
                dtype=K.floatx(),
            )
            batch_x_prev_states = np.zeros(
                tuple([self.cmdch_size] + list(self.x_prev_states.shape)[1:]),
                dtype=K.floatx(),
            )
        else:
            batch_x_images = np.zeros(
