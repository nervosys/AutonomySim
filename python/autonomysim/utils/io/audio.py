...
class WavReader:
    ...
    def get_requested_channels(self, data):
        if self.requested_channels > self.actual_channels:
            raise Exception(
                "Cannot add channels, actual is {}, requested is {}".format(
    ...
    def read(self):
        """Reads the next chunk of audio data.

        It returns the data converted to floating point numbers between -1 and 1,
        scaled by the range of values possible for the given audio format.
