...
def main():
    ...
    wait_key("Press any key to set all object IDs to 0")
    found = client.simSetSegmentationObjectID("[\w]*", 0, True)
    print("Done: %r" % (found))

    # for block environment

    wait_key("Press any key to change one ground object ID")
    found = client.simSetSegmentationObjectID("Ground", 20)
    print("Done: %r" % (found))

    # regex are case insensitive
    wait_key("Press any key to change all ground object ID")
    ...
    wait_key("Press any key to set Landscape object ID to 128")
