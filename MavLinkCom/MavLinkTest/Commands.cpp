    ...
    while (log.read(msg, log_timestamp)) {
        ...
        case MavLinkStatustext::kMessageId: {
            ...
            if (std::strstr(status_msg.text, kCommandLogPrefix) == status_msg.text) {
                ...
                std::string command_text_file_name =
                    FileSystem::combine(out_folder, Utils::stringf("cmd_%03d_%s.txt", command_index, line.c_str()));
        ...
        case MavLinkLocalPositionNed::kMessageId: {
            if (pos_log_file.is_open()) {
                ...
                if (std::isnan(x_start)) {
                    ...
                    pos_log_file << pos_msg.x - x_start << "\t" << pos_msg.y - y_start << "\t" << pos_msg.z - z_start
                                 << std::endl;
                }
            }
            break;
...
void DumpLogCommandsCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    ...
    for (directory_iterator next(dirPath), end; next != end; ++next) {
        ...
        if (ext == ".mavlink") {
            auto out_folder =
                FileSystem::createDirectory(FileSystem::combine(log_folder_, path.filename().stem().generic_string()));
...
void SyncParamValue(std::shared_ptr<MavLinkVehicle> com, std::vector<MavLinkParameter> &params,
...
void PlayLogCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    ...
    while (log_.read(msg, log_timestamp)) {
        ...
        if (waitMicros > 0) {
            ...
            // our clock fell behind somehow (debug breakpoint?) So fix it by moving our start time forwards by this
            // amount.
            playback_start_timestamp -= waitMicros;
        }
        ...
        case MavLinkHeartbeat::kMessageId: {
            ...
            if (heartbeat_msg.base_mode != last_basemode || heartbeat_msg.custom_mode != last_custommode) {
                ...
                // com->setMode(last_basemode | static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED),
                // last_custommode);
            }
...
void LandCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    ...
    bool rc = false;
    if (com->land(state.global_est.heading, state.home.global_pos.lat, state.home.global_pos.lon,
                  state.home.global_pos.alt)
            .wait(3000, &rc)) {
        if (rc) {
            printf("ok\n");
        } else {
...
void PositionCommand::Execute(std::shared_ptr<MavLinkVehicle> com) { Command::Execute(com); }
...
void CapabilitiesCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    ...
    if (com->getCapabilities().wait(10000, &ver)) {
        ...
        if ((ver.capabilities &
             static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_GLOBAL_INT)) != 0) {
        ...
        if ((ver.capabilities &
             static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_ACTUATOR_TARGET)) != 0) {
            printf("      MAV_PROTOCOL_CAPABILITY_SET_ACTUATOR_TARGET\n");
        }
        if ((ver.capabilities &
             static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_FLIGHT_TERMINATION)) != 0) {
            printf("      MAV_PROTOCOL_CAPABILITY_FLIGHT_TERMINATION\n");
        }
        if ((ver.capabilities &
             static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_COMPASS_CALIBRATION)) != 0) {
        ...
        if (targetPosition) {
            ...
            if (this->hasLocalPosition) {
                ...
                if (targetReached && !settled &&
                    (fabs(this->vx) + std::abs(this->vy) + std::abs(this->vz) < almostStationery)) {
                    settled = true;
...
void OrbitCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    ...
    if (!com->isLocalControlSupported()) {
        throw std::runtime_error(Utils::stringf(
            "Your drone does not support the MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED capability."));
    }
...
void OrbitCommand::UpdateTarget() {
    ...
    if (flyingToRadius) {
        ...
        if (logViewer.get() != nullptr) {
            ...
            est.usec = static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::microseconds>(
                                                 std::chrono::system_clock::now().time_since_epoch())
                                                 .count());
...
void RotateCommand::HasLocalPosition() {
    if (!start_) {
        // in case we are flying fast, we first do a Goto to get to a fixed stationary position before we try and start
        // rotating.
        Goto(x, y, z, -1, yaw);
    }
}
...
void WiggleCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
    if (!com->isAttitudeControlSupported()) {
        throw std::runtime_error(
            Utils::stringf("Your drone does not support the MAV_PROTOCOL_CAPABILITY_SET_ATTITUDE_TARGET capability."));
    }
    started_ = false;
...
void FtpCommand::doList() {
    ...
    if (progress.average_rate != 0) {
        printf("%d msgs received, %f milliseconds per packet, longest delay=%f\n", progress.message_count,
               progress.average_rate, progress.longest_delay);
    }
}
