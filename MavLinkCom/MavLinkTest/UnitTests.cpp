...
void UnitTests::SerialPx4Test() {
    ...
    if (!received.timed_wait(5000)) {
        throw std::runtime_error("MavLink heartbeat is not being received over serial, please make sure PX4 is plugged "
                                 "in and the unit test is using the right COM port.");
    }
...
void UnitTests::SendImageTest() {
    ...
    printf("    Received image %d bytes, width %d and height %d ok\n", static_cast<int>(image.data.size()), image.width,
           image.height);

    return;
}

void UnitTests::VerifyFile(MavLinkFtpClient &ftp, const std::string &dir, const std::string &name, bool exists,
                           bool isdir) {
    MavLinkFtpProgress progress;
    std::vector<MavLinkFileInfo> files;
    ftp.list(progress, dir, files);
    if (progress.error != 0) {
        throw std::runtime_error(Utils::stringf("unexpected error %d: '%s' from ftp list '%s' command", progress.error,
                                                progress.message.c_str(), dir.c_str()));
    }
    ...
    try {
        ...
        if (progress.error != 0) {
            throw std::runtime_error(Utils::stringf(
                "unexpected error %d: '%s' from ftp list '/fs/microsd' command - does your pixhawk have an sd card?",
...
#if defined(_WIN32)
        // I wish there was a cleaner way to do this, but I can't use tempPath.native() because on windows that is a
        // wstring and on our linux build it is a string.
        ...
        if (progress.error != 0) {
            throw std::runtime_error(Utils::stringf("unexpected error %d: '%s' from ftp put command", progress.error,
        ...
        if (progress.error != 0) {
            throw std::runtime_error(Utils::stringf("unexpected error %d: '%s' from ftp get command", progress.error,
                                                    progress.message.c_str()));
        }
        ...
        if (progress.error != 0) {
            throw std::runtime_error(Utils::stringf("unexpected error %d: '%s' from ftp remove command", progress.error,
                                                    progress.message.c_str()));
        } else {
        ...
        if (progress.error != 0) {
            throw std::runtime_error(
                Utils::stringf("unexpected error %d: '%s' from ftp mkdir '/fs/microsd/testrmdir' command",
                               progress.error, progress.message.c_str()));
        }
        ...
        if (progress.error != 0) {
            throw std::runtime_error(
                Utils::stringf("unexpected error %d: '%s' from ftp rmdir '/fs/microsd/testrmdir' command",
                               progress.error, progress.message.c_str()));
        }
