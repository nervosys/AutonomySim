...
#include <vector>
STRICT_MODE_OFF
#include "json.hpp" // "common/utils/json.hpp"
STRICT_MODE_ON
#include "UnitTests.h"
...
void OpenLogFiles() {
    if (logDirectory.size() > 0) {
        ...
        std::string input =
            Utils::stringf("%02d-%02d-%02d-input.%s", local->tm_hour, local->tm_min, local->tm_sec, ext);
        ...
        std::string output =
            Utils::stringf("%02d-%02d-%02d-output.%s", local->tm_hour, local->tm_min, local->tm_sec, ext);
...
const static FlagName MavSysSensorFlags[] = {
    ...
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE),
     "MAV_SYS_STATUS_SENSOR_ABSOLUTE_PRESSURE - 0x08 absolute pressure"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE),
    ...
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_OPTICAL_FLOW),
     "MAV_SYS_STATUS_SENSOR_OPTICAL_FLOW - 0x40 optical flow"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_VISION_POSITION),
     "MAV_SYS_STATUS_SENSOR_VISION_POSITION - 0x80 computer vision position"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_LASER_POSITION),
     "MAV_SYS_STATUS_SENSOR_LASER_POSITION - 0x100 laser based position"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_EXTERNAL_GROUND_TRUTH),
     "MAV_SYS_STATUS_SENSOR_EXTERNAL_GROUND_TRUTH - 0x200 external ground truth (Vicon or Leica)"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_ANGULAR_RATE_CONTROL),
     "MAV_SYS_STATUS_SENSOR_ANGULAR_RATE_CONTROL - 0x400 3D angular rate control"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_ATTITUDE_STABILIZATION),
     "MAV_SYS_STATUS_SENSOR_ATTITUDE_STABILIZATION - 0x800 attitude stabilization"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_YAW_POSITION),
     "MAV_SYS_STATUS_SENSOR_YAW_POSITION - 0x1000 yaw position"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_Z_ALTITUDE_CONTROL),
     "MAV_SYS_STATUS_SENSOR_Z_ALTITUDE_CONTROL - 0x2000 z/altitude control"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_XY_POSITION_CONTROL),
     "MAV_SYS_STATUS_SENSOR_XY_POSITION_CONTROL - 0x4000 x/y position control"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_MOTOR_OUTPUTS),
...
const static char *AutoPilotNames[]{
    ...
    "MAV_AUTOPILOT_GENERIC_WAYPOINTS_AND_SIMPLE_NAVIGATION_ONLY, Generic autopilot supporting waypoints and other "
...
const static FlagName ModeFlagNames[]{
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_TEST_ENABLED),
     "MAV_MODE_FLAG_TEST_ENABLED - system has a test mode enabled. This flag is intended for temporary system tests "
     "and should not be used for stable implementations."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_AUTO_ENABLED),
     "MAV_MODE_FLAG_AUTO_ENABLED - autonomous mode enabled, system finds its own goal positions. Guided flag can be "
    ...
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_STABILIZE_ENABLED),
     "MAV_MODE_FLAG_STABILIZE_ENABLED - system stabilizes electronically its attitude (and optionally position). It "
     "needs however further control inputs to move around."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED),
     "MAV_MODE_FLAG_HIL_ENABLED - hardware in the loop simulation. All motors / actuators are blocked, but internal "
...
const static char *MavStateNames[]{
    ...
    "MAV_STATE_EMERGENCY - System is in a non-normal flight mode. It lost control over parts or over the whole "
...
const static FlagName CustomModeNames[]{{PX4_CUSTOM_MAIN_MODE_MANUAL, "PX4_CUSTOM_MAIN_MODE_MANUAL"},
...
void PrintSystemStatus(MavLinkSysStatus &status) {
    ...
    printf("    battery_remaining = %d\n", static_cast<int>(status.cmdtery_remaining));
}
...
void PrintUsage() {
    ...
    printf("Options: \n");
    printf("    -udp[:ipaddr[:port]]                   - connect to remote drone at this udp address (default port "
           "localhost:14550)\n");
    printf("    -tcp[:ipaddr[:port]]                   - connect to remote drone at this tcp address (default port "
           "localhost:4560)\n");
    printf("    -local[:ipaddr]                        - connect to remote drone via this local address (default "
           "localhost)\n");
    printf(
        "    -server                                - start mavlink server on specified -udp or -tcp local address\n");
    printf("    -serial[:comPortName][, baudrate]]     - open serial port\n");
    printf("    -sitl[:ipaddr[:port]]                  - when talking to a SITL PX4 provide socket port number here "
           "for control channel (default localhost:14580)\n");
    ...
    printf("    -logdir:filename                       - specify local directory where mavlink logs are stored "
           "(default is no log files)\n");
    printf("    -logformat:json                        - the default is binary .mavlink, if you specify this option "
           "you will get mavlink logs in json\n");
    printf("    -convert:[json,csv]                    - convert all existing .mavlink log files in the logdir to the "
           "specified -logformat\n");
    printf("    -filter:msid,msgid,...                 - while converting .mavlink log extract only the given mavlink "
           "message ids\n");
    ...
    printf("    -wifi:iface                            - add wifi rssi to the telemetry using given wifi interface "
           "name (e.g. wplsp0)\n");
    printf("If no arguments it will find a COM port matching the name 'PX4'\n");
    printf("You can specify -proxy multiple times with different port numbers to proxy drone messages out to multiple "
           "listeners\n");
}
...
std::shared_ptr<MavLinkConnection> connectProxy(const PortAddress &endPoint, std::string name) {
    ...
    std::shared_ptr<MavLinkConnection> proxyConnection =
        MavLinkConnection::connectRemoteUdp(name, localEndPoint.addr, endPoint.addr, endPoint.port);
...
std::string findPixhawk() {
    ...
    for (auto iter = result.begin(); iter != result.end(); iter++) {
        ...
        if (info.vid == pixhawkVendorId) {
            if (info.pid == pixhawkFMUV4ProductId || info.pid == pixhawkFMUV2ProductId ||
...
std::shared_ptr<MavLinkConnection> connectOffboard() {
    ...
    return MavLinkConnection::connectRemoteUdp("drone", defaultLocalAddress, offboardEndPoint.addr,
...
std::shared_ptr<MavLinkConnection> connectServer(std::string name) {
    ...
    if (droneConnection != nullptr) {
        // then we have a serial connection as the primary droneConnection, so publish all PX4 messages out to the
        // server
