...
class SerialPort::serialport_impl {
  ...
  public:
    ...
    int setAttributes(int baudRate, Parity parity, int dataBits, StopBits bits, Handshake hs, int readTimeout,
                      int writeTimeout) {
        DCB dcb;
...
class SerialPort::serialport_impl {
  ...
  public:
    ...
    int setAttributes(int baudRate, Parity parity, int dataBits, StopBits stopBits, Handshake handshake,
        ...
        tty.c_lflag &=
            ~(ECHO | ECHONL | ICANON | ISIG | IEXTEN); // no signaling chars, no echo, no canonical processing
        tty.c_oflag &= ~OPOST;
...
int SerialPort::connect(const char *portName, int baudRate) { return impl_->connect(portName, baudRate); }

int SerialPort::write(const uint8_t *ptr, int count) { return impl_->write(ptr, count); }

int SerialPort::read(uint8_t *buffer, int bytesToRead) { return impl_->read(buffer, bytesToRead); }
