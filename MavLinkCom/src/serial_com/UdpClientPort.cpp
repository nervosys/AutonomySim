...
class UdpClientPort::UdpSocketImpl {
  ...
  public:
    ...
    int getRssi(const char *ifaceName) { return getWifiRssi(static_cast<int>(sock), ifaceName); }
    ...
    static void resolveAddress(const std::string &ipAddress, int port, sockaddr_in &addr) {
        ...
        if (!found) {
            auto msg =
                Utils::stringf("UdpClientPort could not resolve ip address for '%s:%d'\n", ipAddress.c_str(), port);
            throw std::runtime_error(msg);
        }
    }
    ...
    int connect(const std::string &localHost, int localPort, const std::string &remoteHost, int remotePort) {
        ...
        // AutonomySim to shutdown properly when drone is not connected.
        struct timeval tv;
        tv.tv_sec = 1;
        tv.tv_usec = 0;
    ...
    int write(const uint8_t *ptr, int count) {
        if (closed_ || remoteaddr.sin_port == 0) {
            // well if we are creating a server, we don't know when the client is going to connect, so skip this
            // exception for now.
            // throw std::runtime_error("UdpClientPort cannot send until we've received something first so we can find
            // out what port to send to.\n");
            return 0;
        }
...
#else
        int hr = sendto(sock, reinterpret_cast<const char *>(ptr), count, 0, reinterpret_cast<sockaddr *>(&remoteaddr),
                        addrlen);
#endif
        if (hr == SOCKET_ERROR) {
            hr = checkerror();
            if (hr != 0) {
                // perhaps the client is gone, and may want to come back on a different port, in which case let's reset
                // our remote port to allow that.
                remoteaddr.sin_port = 0;
...
void UdpClientPort::connect(const std::string &localHost, int localPort, const std::string &remoteHost,
                            int remotePort) {
    impl_->connect(localHost, localPort, remoteHost, remotePort);
}

int UdpClientPort::write(const uint8_t *ptr, int count) { return impl_->write(ptr, count); }

int UdpClientPort::read(uint8_t *buffer, int bytesToRead) { return impl_->read(buffer, bytesToRead); }
...
int UdpClientPort::getRssi(const char *ifaceName) { return impl_->getRssi(ifaceName); }