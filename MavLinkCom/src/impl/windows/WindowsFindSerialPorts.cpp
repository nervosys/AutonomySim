...
std::vector<SerialPortInfo> MavLinkConnection::findSerialPorts(int vid, int pid) {
    ...
    for (int devIndex = 0; SetupDiEnumDeviceInfo(classInfo, devIndex, &deviceInfo); devIndex++) {
        ...
        if (hr == CR_SUCCESS) {
            ...
            int dvid = 0, dpid = 0;
            if (parseVidPid(buffer, &dvid, &dpid) && ((dvid == vid && dpid == pid) || (vid == 0 && pid == 0))) {
                DWORD keyCount = 0;
                ...
                if (SetupDiGetDevicePropertyKeys(classInfo, &deviceInfo, keyArray, keyCount, &keyCount, 0)) {
                    ...
                    for (DWORD j = 0; j < keyCount; j++) {
                        ...
                        bool isItemNameProperty =
                            (key->fmtid == PKEY_ItemNameDisplay.fmtid && key->pid == PKEY_ItemNameDisplay.pid);
                        if (isItemNameProperty) {
                            ULONG bufferSize = 0;
                            DEVPROPTYPE propertyType;
                            CM_Get_DevNode_Property(deviceInfo.DevInst, &keyArray[j], &propertyType, NULL, &bufferSize,
                                                    0);
                            if (bufferSize > 0) {
                                BYTE *propertyBuffer = new BYTE[bufferSize];
                                hr = CM_Get_DevNode_Property(deviceInfo.DevInst, &keyArray[j], &propertyType,
                                                             propertyBuffer, &bufferSize, 0);
                                if (hr == CR_SUCCESS) {
