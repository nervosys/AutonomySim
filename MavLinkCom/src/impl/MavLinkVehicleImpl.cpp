...
AsyncResult<MavLinkHomePosition> MavLinkVehicleImpl::waitForHomePosition() {
    ...
    AsyncResult<MavLinkHomePosition> result([=](int subscription) { con->unsubscribe(subscription); });
...
void MavLinkVehicleImpl::handleMessage(std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage &msg) {
    ...
    // status messages should usually be only sent by actual PX4. However if someone else is sending it to, we should
    // listen it. in future it would be good to have ability to add system IDs we are interested in if (msg.sysid !=
    // getTargetSystemId())
    //{
        ...
        if (heartbeat.autopilot == static_cast<uint8_t>(MAV_AUTOPILOT::MAV_AUTOPILOT_PX4)) {
            ...
            if (!isOffboard && (requested_mode_ != custom) && (custom != previous_mode_)) {
                if (control_request_sent_) {
                    ...
                    Utils::log(Utils::stringf("MavLinkVehicle: detected mode change (mode=%d, submode=%d), will stop "
                                              "trying to do offboard control\n",
                                              mode, submode));
                }
            }
...
AsyncResult<bool> MavLinkVehicleImpl::waitForAltitude(float z, float dz, float dvz) {
    std::shared_ptr<MavLinkConnection> con = ensureConnection();
    AsyncResult<bool> result([=](int subscription) { con->unsubscribe(subscription); });
...
void MavLinkVehicleImpl::checkOffboard() {
    ...
    if (control_requested_ && !vehicle_state_.controls.offboard) {
        // Ok, now's the time to actually request it since the caller is about to send
        // MavLinkSetPositionTargetGlobalInt, but PX4 will reject this thinking 'offboard_control_loss_timeout' because
        // we haven't actually sent any offboard messages yet.  I know the PX4 protocol is kind of weird.  So we prime
        // the pump here with some dummy messages that tell the drone to stay where it is, this will reset the
        // 'offboard_control_loss_timeout', then we should be able to get control.
...
void MavLinkVehicleImpl::offboardIdle() {
    ...
    msg.type_mask = MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_POSITION |
                    ...
                    MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_POSITION | MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_YAW_RATE |
                    MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_YAW_ANGLE | MAVLINK_MSG_SET_POSITION_TARGET_IDLE;
...
void MavLinkVehicleImpl::moveToGlobalPosition(float lat, float lon, float alt, bool isYaw, float yawOrRate) {
    ...
    msg.type_mask =
        MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_VELOCITY | MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_ACCELERATION;
...
AsyncResult<bool> MavLinkVehicleImpl::setMissionMode() {
    return setMode(static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED) |
                   ...
                   static_cast<int>(PX4_CUSTOM_MAIN_MODE_AUTO), static_cast<int>(PX4_CUSTOM_SUB_MODE_AUTO_MISSION));
}

AsyncResult<bool> MavLinkVehicleImpl::loiter() {
    return setMode(static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
                   static_cast<int>(PX4_CUSTOM_MAIN_MODE_AUTO), static_cast<int>(PX4_CUSTOM_SUB_MODE_AUTO_LOITER));
}
...
bool MavLinkVehicleImpl::isLocalControlSupported() {
    ...
    if (!getCapabilities().wait(2000, &cap)) {
        throw std::runtime_error(
            "Two second timeout waiting for response to mavlink command MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES");
    }
    return (cap.capabilities &
            static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED)) != 0;
}
...
void MavLinkVehicleImpl::moveByLocalVelocity(float vx, float vy, float vz, bool isYaw, float yawOrRate) {
    ...
    msg.type_mask =
        MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_POSITION | MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_ACCELERATION;
...
void MavLinkVehicleImpl::moveByLocalVelocityWithAltHold(float vx, float vy, float z, bool isYaw, float yawOrRate) {
    ...
    msg.type_mask = MAVLINK_MSG_SET_POSITION_TARGET_ALT_HOLD | MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_ACCELERATION;
...
void MavLinkVehicleImpl::moveToLocalPosition(float x, float y, float z, bool isYaw, float yawOrRate) {
    ...
    msg.type_mask =
        MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_ACCELERATION | MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_VELOCITY;
    if (isYaw) {
        msg.type_mask |= MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_YAW_RATE;
        msg.yaw = yawOrRate;
        msg.yaw_rate = 0;
...
bool MavLinkVehicleImpl::isAttitudeControlSupported() {
    ...
    if (!getCapabilities().wait(5000, &cap)) {
        throw std::runtime_error(
            "Five second timeout waiting for response to mavlink command MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES");
    }
    return (cap.capabilities &
            static_cast<int>(MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_ATTITUDE_TARGET)) != 0;
}

void MavLinkVehicleImpl::moveByAttitude(float roll, float pitch, float yaw, float rollRate, float pitchRate,
