...
std::vector<SerialPortInfo> ListDevices() {
    ...
    ULONG length;
    CONFIGRET cr = CM_Get_Device_Interface_List_SizeW(&length, const_cast<GUID *>(&GUID_DEVINTERFACE_COMPORT),
                                                      nullptr, // pDeviceID
    ...
    cr =
        CM_Get_Device_Interface_ListW(const_cast<GUID *>(&GUID_DEVINTERFACE_COMPORT),
                                      nullptr, // pDeviceID
                                      buf.data(), static_cast<ULONG>(buf.size()), CM_GET_DEVICE_INTERFACE_LIST_PRESENT);
    ...
    *buf.rbegin() = UNICODE_NULL;
    ...
    for (PCWSTR deviceInterface = buf.data(); *deviceInterface; deviceInterface += wcslen(deviceInterface) + 1) {
        ...
        cr = CM_Get_Device_Interface_PropertyW(deviceInterface, &propkey, &propertyType,
...
// suppress
// OneCoreFindSerialPorts.obj : warning LNK4221: This object file does not define any previously undefined public
