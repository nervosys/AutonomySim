...
void MavLinkNodeImpl::connect(std::shared_ptr<MavLinkConnection> connection) {
    ...
    subscription_ = connection_->subscribe(
        [=](std::shared_ptr<MavLinkConnection> con, const MavLinkMessage &msg) { handleMessage(con, msg); });
}
...
AsyncResult<MavLinkAutopilotVersion> MavLinkNodeImpl::getCapabilities() {
    if (has_cap_) {
        AsyncResult<MavLinkAutopilotVersion> nowait([=](int state) { unused(state); });
    ...
    AsyncResult<MavLinkAutopilotVersion> result([=](int state) { con->unsubscribe(state); });
...
AsyncResult<MavLinkHeartbeat> MavLinkNodeImpl::waitForHeartbeat() {
    ...
    AsyncResult<MavLinkHeartbeat> result([=](int state) { con->unsubscribe(state); });
...
std::vector<MavLinkParameter> MavLinkNodeImpl::getParamList() {
    ...
    int subscription =
        con->subscribe([&](std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage &message) {
            unused(connection);
    ...
    // note that UDP does not guarantee delivery of messages, so we have to also check if some parameters are missing
    ...
    std::sort(result.begin(), result.end(),
              [&](const MavLinkParameter &p1, const MavLinkParameter &p2) { return p1.name.compare(p2.name) < 0; });
...
MavLinkParameter MavLinkNodeImpl::getCachedParameter(const std::string &name) {
    if (this->parameters_.size() == 0) {
        throw std::runtime_error(
            "Error: please call getParamList during initialization so we have cached snapshot of the parameter values");
    }
...
AsyncResult<MavLinkParameter> MavLinkNodeImpl::getParameter(const std::string &name) {
    ...
    if (size > 16) {
        throw std::runtime_error(
            Utils::stringf("Error: parameter name '%s' is too long, must be <= 16 chars", name.c_str()));
    } else if (size < 16) {
        size++; // we can include the null terminator.
    }
    auto con = ensureConnection();
    AsyncResult<MavLinkParameter> asyncResult([=](int state) { con->unsubscribe(state); });
...
AsyncResult<bool> MavLinkNodeImpl::setParameter(MavLinkParameter p) {
    ...
    if (size > 16) {
        throw std::runtime_error(
            Utils::stringf("Error: parameter name '%s' is too long, must be <= 16 chars", p.name.c_str()));
    } else if (size < 16) {
        size++; // we can include the null terminator.
    }
    auto con = ensureConnection();
