...
struct FtpPayload {
    ...
    uint8_t burst_complete; ///< Only used if req_opcode=kCmdBurstreadFile - 1: set of burst packets complete, 0: More
                            ///< burst packets coming.
...
void MavLinkFtpClientImpl::subscribe() {
    if (subscription_ == 0) {
        subscription_ =
            getConnection()->subscribe([=](std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage &msg) {
                unused(connection);
                handleResponse(msg);
            });
    }
...
void MavLinkFtpClientImpl::list(MavLinkFtpProgress &progress, const std::string &remotePath,
...
void MavLinkFtpClientImpl::get(MavLinkFtpProgress &progress, const std::string &remotePath,
...
void MavLinkFtpClientImpl::put(MavLinkFtpProgress &progress, const std::string &remotePath,
                               const std::string &localPath) {
    local_file_ = localPath;
    remote_file_ = remotePath;
...
bool MavLinkFtpClientImpl::openSourceFile() {
    ...
    if (file_ptr_ == nullptr) {
        if (progress_ != nullptr) {
            ...
            progress_->message =
                Utils::stringf("Error opening file '%s' for reading, rc=%d", remote_file_.c_str(), progress_->error);
        }
        return false;
    }
    ...
    if (file_ptr_ == nullptr) {
        ...
        if (FileSystem::isDirectory(path)) {
            ...
            if (FileSystem::isDirectory(path)) {
                ...
                if (progress_ != nullptr) {
                    progress_->error = errno;
                    progress_->message =
                        Utils::stringf("Error opening file because '%s' is not a directory", path.c_str());
                }
                return false;
            } else {
                if (progress_ != nullptr) {
                    ...
                    progress_->message = Utils::stringf(
                        "Error opening file because '%s' should be a directory but it was not found", path.c_str());
                }
                return false;
            }
        }
        ...
        if (file_ptr_ == nullptr) {
            if (progress_ != nullptr) {
                progress_->error = errno;
                progress_->message =
                    Utils::stringf("Error opening file '%s' for writing, rc=%d", local_file_.c_str(), errno);
            }
            return false;
        }
        file_size_ = 0;
