...
std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::connectSerial(const std::string &nodeName,
...
void MavLinkConnectionImpl::startLoggingSendMessage(std::shared_ptr<MavLinkLog> log) { sendLog_ = log; }

void MavLinkConnectionImpl::stopLoggingSendMessage() { sendLog_ = nullptr; }

// log every message that is "sent" using sendMessage.
void MavLinkConnectionImpl::startLoggingReceiveMessage(std::shared_ptr<MavLinkLog> log) { receiveLog_ = log; }

void MavLinkConnectionImpl::stopLoggingReceiveMessage() { receiveLog_ = nullptr; }
...

    {
        ...
        try {
            ...
            throw std::runtime_error(
                Utils::stringf("MavLinkConnectionImpl: Error sending message on connection '%s', details: %s",
                               name.c_str(), e.what()));
        }
    }
    {
...
int MavLinkConnectionImpl::prepareForSending(MavLinkMessage &msg) {
    ...
    bool signing =
        !mavlink1 && mavlink_status_.signing && (mavlink_status_.signing->flags & MAVLINK_SIGNING_FLAG_SIGN_OUTGOING);
    ...
    if (signing) {
        mavlink_sign_packet(mavlink_status_.signing, reinterpret_cast<uint8_t *>(msg.signature),
...
void MavLinkConnectionImpl::joinLeftSubscriber(std::shared_ptr<MavLinkConnection> remote,
...
void MavLinkConnectionImpl::joinRightSubscriber(std::shared_ptr<MavLinkConnection> connection,
                                                const MavLinkMessage &msg) {
    unused(connection);
...
void MavLinkConnectionImpl::join(std::shared_ptr<MavLinkConnection> remote, bool subscribeToLeft,
                                 bool subscribeToRight) {
    if (subscribeToLeft)
        this->subscribe(std::bind(&MavLinkConnectionImpl::joinLeftSubscriber, this, remote, std::placeholders::_1,
                                  std::placeholders::_2));

    if (subscribeToRight)
        remote->subscribe(
            std::bind(&MavLinkConnectionImpl::joinRightSubscriber, this, std::placeholders::_1, std::placeholders::_2));
}
...
void MavLinkConnectionImpl::readPackets() {
    ...
    while (hr == 0 && con_ != nullptr && !closed) {
        ...
        for (int i = 0; i < count; i++) {
            uint8_t frame_state =
                mavlink_frame_char_buffer(&msgBuffer, &mavlink_intermediate_status_, buffer[i], &msg, &mavlink_status_);
    ...
    while (hasMsg) {
        ...
        for (auto ptr = snapshot.begin(); ptr != end; ptr++) {
            try {
                ...
                Utils::log(
                    Utils::stringf("MavLinkConnectionImpl: Error handling message %d on connection '%s', details: %s",
...
bool MavLinkConnectionImpl::isPublishThread() const { return std::this_thread::get_id() == publish_thread_id_; }
