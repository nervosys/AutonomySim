...
template <class T> class AsyncResult {
  public:
    ...
    AsyncResult(CompletionHandler owner) { state_ = std::make_shared<AsyncResultState<T>>(owner); }
  ...
  private:
    static T getResult(std::shared_ptr<AsyncResultState<T>> state) { return state->getResult(); }
