...
class MavLinkConnection : public std::enable_shared_from_this<MavLinkConnection> {
  public:
    ...
    static std::shared_ptr<MavLinkConnection> connectSerial(const std::string &nodeName, const std::string &portName,
    ...
    // it will remember the remote address of the sender so that subsequend sendMessage calls will go back to that
    // sender. This is useful if the remote sender already knows which local port you plan to listen on. The localAddr
    // can also a specific local ip address if you need to specify which network interface to use, for example, a
    // corporate wired ethernet usually does not transmit UDP packets to a wifi connected device, so in that case the
    // localAddress needs to be the IP address of a specific wifi internet adapter rather than 127.0.0.1.
    static std::shared_ptr<MavLinkConnection> connectLocalUdp(const std::string &nodeName, const std::string &localAddr,
                                                              int localPort);
    ...
    // to a wifi connected device, so in that case the localAddress needs to be the IP address of a specific wifi
    // internet adapter rather than 127.0.0.1.
    static std::shared_ptr<MavLinkConnection> connectRemoteUdp(const std::string &nodeName,
    ...
    static std::shared_ptr<MavLinkConnection> connectTcp(const std::string &nodeName, const std::string &localAddr,
    ...
    // Advanced method that create a bridge between two connections.  For example, if you use connectRemoteUdp to
    // connect to QGroundControl port 14550, and connectSerial to connect to PX4, then you can call this method to join
    // the two so that all messages from PX4 are sent to QGroundControl and vice versa.
    ...
    // Compute crc checksums, and pack according to mavlink1 or mavlink2 (depending on what target node supports) and do
    // optional message signing according to the target node we are communicating with, and return the message length.
    int prepareForSending(MavLinkMessage &msg);

    // Returns true if we are on the publishing thread.  Certain blocing operations that wait for messages from mavlin
    // vehicle are not allowed on this thread.
